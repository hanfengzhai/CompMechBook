#!/usr/bin/env bash
# Verify narrative intermission hinges at Part III.4 and VI.4 (continuous-book device).
set -euo pipefail
...
if [[ $MISSING -ne 0 ]]; then
  echo "FAIL: narrative intermission hinges incomplete (III.4 and VI.4 required)"
  exit 1
fi

echo "OK: narrative intermission hinges present (III.4 analysis→assembly, VI.4 ascent→descent)"
