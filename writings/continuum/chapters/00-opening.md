...
## Writings canonical landing (V.4 → VI.0) {#writings-canonical-landing-v4-to-vi0}

You may have arrived from [`writings/fvm`](../../writings/fvm/chapters/SUMMARY.md) (Part V) after [V.4's Bridge](../part05-fvm/04-navier-stokes-cfd.md#bridge-to-part-vi), from [`writings/fem`](../../writings/fem/chapters/SUMMARY.md) via [IV.5 Door B](../part04-fem/05-convergence.md#bridge-two-doors-from-here), or from the unified book ([`SUMMARY.md`](../SUMMARY.md)) — either path should land here **after** the conjugate heat transfer handshake and [V.4 Writings canonical hinge](../part05-fvm/04-navier-stokes-cfd.md#writings-canonical-hinge-v4-to-vi0), not at VI.1 kinematics cold. The [`writings/continuum`](../../writings/continuum/chapters/SUMMARY.md) subtree repeats the same reading contract as Part II's [Functional Analysis Notes](https://hanfengzhai.github.io/file/teaching/notes/ME412_CourseSummary.pdf) template; only the upstream note PDF and the state variable change — from cell fluxes and face balances to \(\mathbf{F}\), \(\boldsymbol{\sigma}\), and virtual work. When a new `book.toml` feels like a course break, read [row 47](../appendix/sources.md#v4-vi0-writings-canonical-reunion-index-row-47) and recite both plot spines: [V.4](../part05-fvm/04-navier-stokes-cfd.md#plot-spine-one-line) ("Navier–Stokes and CHT set \(T_w\)") then [VI.0](#plot-spine-one-line) ("FEM and FVM reunite on Cauchy stress").
...
| CHT export ([V.4 parser checkpoint](../part05-fvm/04-navier-stokes-cfd.md#parser-checkpoint-archive-cht-export-yaml)) | `cht_export.yaml` from [`parse_cht.sh`](../../scripts/parse_cht.sh); canonical \(T_w = 311.48\,\text{K}\) (Ra/Nu Lab act teaches \(\approx 379\,\text{K}\) — pick one) | Single wall temperature for both twins; \(\varepsilon_{\text{th}} = \alpha(T_w)\Delta T\) in virtual work | [VI.3](03-variational-elasticity.md); Part VII \(M(\tau, T_w)\) |
| Two doors ([IV.5](../part04-fem/05-convergence.md#bridge-two-doors-from-here)) | Door A → Part V → here; Door B → here directly | Both doors arrive at the same \(\boldsymbol{\sigma}\) vocabulary | This opening |
...
**CHT outer-loop reunion (row 30).** If row 29 restored thermoelastic assembly on the **solid mesh** but Part IV's \(\mathbf{K}_{TT}\) and Part V's face fluxes still feel like separate solvers, read the [CHT outer-loop reunion index](../appendix/sources.md#cht-outer-loop-reunion-index-row-30) — the meta stitch when the Picard table converged in the chapter but `cht_export.yaml` never sat beside both decks. The converged \(T_w\) from [V.4's parser checkpoint](../part05-fvm/04-navier-stokes-cfd.md#parser-checkpoint-archive-cht-export-yaml) is the **single temperature** both twins carry into [VI.2 thermal strain](02-stress-balance.md) and [VI.4 return-mapping at the heated grip](04-nonlinear-plasticity-preview.md#lab-act-return-mapping-on-the-load-cell-knee-act-iv-hardening). Do not open Part VII with a room-temperature default while `cht_export.yaml` documents \(T_w = 311.48\,\text{K}\) from the outer loop you already ran.

**Twin-ladder → virtual work reunion (row 31).** If row 30 restored one \(T_w\) but Part IV's \(\mathbf{K}\mathbf{U}=\mathbf{F}\) and this part's virtual work still feel like separate subjects, read the [twin-ladder → virtual work reunion index](../appendix/sources.md#twin-ladder-virtual-work-reunion-index-row-31) — the meta stitch when the twins converged but energy never closed the load cell story. Start at [VI.1's Act II overlay](01-kinematics.md#lab-act-act-ii-overlay-thermal-eigenstrain-from-cht-export) (superpose \(\varepsilon_{\text{th}}\) from `cht_export.yaml`), then [VI.3's virtual work Lab act](03-variational-elasticity.md#lab-act-virtual-work-equals-load-cell-reading-act-iii--pulling) (\(\delta\Pi = 0\) equals the load cell slope). Row 31 does not replace row 30 — it reunites **assembly with variational physics** when one wall temperature is archived but \(\Pi[\mathbf{u}]\) still feels distant from Part IV.

**Virtual work → plasticity preview reunion (row 32).** If row 31 restored \(\delta\Pi = 0\) as force balance but [VI.4](04-nonlinear-plasticity-preview.md) return-mapping still feels like a separate numerical trick, read the [virtual work → plasticity preview reunion index](../appendix/sources.md#virtual-work-plasticity-preview-reunion-index-row-32) — the meta stitch when the elastic load cell story is clear but the **yield knee** breaks energy minimization. Start at [VI.3's opening hinge to VI.4](03-variational-elasticity.md#opening-hinge-vi3-to-vi4), run the [return-mapping Lab act](04-nonlinear-plasticity-preview.md#lab-act-return-mapping-on-the-load-cell-knee-act-iv-hardening), then read [VI.4's intermission](04-nonlinear-plasticity-preview.md#intermission-ascent-ends-descent-begins) before [Part VII's descent hinge](../part07-defects/00-opening.md#descent-hinge-from-vi4-energy-break-forest-begins). Row 32 does not replace row 31 — it reunites **variational elasticity with phenomenological plasticity** when energy closed the elastic climb but history at the knee still feels arbitrary.
