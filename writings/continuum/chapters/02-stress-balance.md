...
| Balance-law output (this chapter) | Variational consumer ([VI.3](03-variational-elasticity.md)) | Wire act | Failure mode |
|-----------------------------------|---------------------------------------------------------------|----------|--------------|
| Momentum balance \(\nabla\cdot\boldsymbol{\sigma}+\mathbf{f}=\mathbf{0}\) | Stationarity of \(\Pi[\mathbf{u}]\) — total potential energy | Act III linear elastic climb on load cell | Treating \(\boldsymbol{\sigma}\) as post-process only |
| Cauchy stress; symmetry from angular balance | Virtual work = first variation of \(\Pi\) | Grip traction vs body weight split | Missing thermal eigenstrain in coupled run |
| Hooke's law \(\boldsymbol{\sigma}=\mathbb{C}:\boldsymbol{\varepsilon}\) | Why Part IV's \(\mathbf{K}\) integrates \(\mathbb{C}:\boldsymbol{\varepsilon}(\mathbf{N}_I):\boldsymbol{\varepsilon}(\mathbf{N}_J)\) | Micrometer + load cell consistency (Lab act) | Handbook \(E\) without DFT/MD pedigree |
| Thermal balance; \(\varepsilon_{\text{th}}=\alpha\Delta T\) | Coupled energy functional when \(T\) feeds moduli | Act II Joule heating + fixed grips | Wrong \(\alpha\) pedigree (epilogue Handshake 3) |

The virtual work equation in this chapter is the same balance Part IV assembled — continuum mechanics **names** the tensors the FEM code already integrated. The load cell's linear elastic climb (prologue **Act III — Pulling**) measures stress derived here; when the curve bends (**Act IV — Hardening**), smooth fields and isotropic \(\mathbb{C}\) stop being enough.

[VI.3](03-variational-elasticity.md) opens on stored elastic energy in the stretch — the scene where \(\Pi(\mathbf{u})\) makes explicit why equilibrium is a minimum and why Part IV's assembly was Rayleigh–Ritz all along. Turn the page when \(\boldsymbol{\sigma}=\mathbb{C}:\boldsymbol{\varepsilon}\) feels like a constitutive plug-in rather than the consequence of minimizing elastic energy — variational elasticity reunifies the story.
