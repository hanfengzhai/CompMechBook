...
| 32 | Intermission → descent reunion (VI.4 → Part VII) | [Intermission → descent reunion index](#intermission-descent-reunion-index-row-32) · [preface row 32](../preface.md#skill-navigation-row-32) · [prologue row 32 preview](../prologue/00-many-scales.md#prologue-preview-row-32) · [prologue row 32 closing stitch](../prologue/00-many-scales.md#row-32-closing-stitch) · [epilogue row 32 closing loop](../epilogue/multiscale.md#row-32-closing-loop) · [memory sheet row 32 baby picture](memory-sheet.md#row-32-baby-picture-intermission-descent-reunion) | Rows 17–31 all read correctly but **\(J_2\) phenomenology and dislocation forest still feel like separate subjects** — read VI.4 intermission → return-mapping Lab act → three descent signals → VII.0 Scene and name why \(H\) is a placeholder for \(\rho\) |
...
## Intermission → descent reunion index (row 32) {#intermission-descent-reunion-index-row-32}

Row 17 names **Scene → Bridge** chapter rhythm; rows 18–20 name **plot spine** one-liners; row 21 names **Lab act** one-line moves; rows 22–27 name sensory through symbolic reunion; row 28 names **Scale-boundary reunion** export pedigree; row 29 names **Thermoelastic assembly reunion** on one solid mesh; row 30 names **CHT outer-loop reunion** at the wall; row 31 names **twin-ladder → virtual work reunion** in Part VI. Row 32 names the **intermission → descent reunion** — when every meta-stitch layer reads correctly but **Part VI's \(J_2\) phenomenology and Part VII's dislocation forest still feel like separate subjects** despite [VI.4's intermission](../part06-continuum/04-nonlinear-plasticity-preview.md#intermission-ascent-ends-descent-begins), return to the ascent → descent hinge before Part VII opens.

Each row below is a **representative anchor** on the VI.4 → VII.0 path — the mandatory midpoint gate where smooth Cauchy fields exhaust their honest limit and line defects become the state variable. Read aloud when row 31 restored virtual work but the load cell knee still reads like curve-fitting without a forest.

| Chapter | Descent move | One-line audit on the wire (read aloud) | Failure mode |
|---------|--------------|----------------------------------------|--------------|
| [VI.4 intermission](../part06-continuum/04-nonlinear-plasticity-preview.md#intermission-ascent-ends-descent-begins) | Ascent ends; descent begins | "Smooth fields and \(J_2\) fit the knee; pedigree begins in VII" | Part VII feels like a new course after continuum |
| [VI.4 plot spine](../part06-continuum/04-nonlinear-plasticity-preview.md#plot-spine-one-line) | Midpoint gate (row 20) | "Plasticity preview names where continuum fields fail" | Skipped mandatory pause before VII |
| [VI.4 return-mapping Lab act](../part06-continuum/04-nonlinear-plasticity-preview.md#lab-act-return-mapping-on-the-load-cell-knee-act-iv-hardening) | Phenomenological \(H\), \(\sigma_{y0}\) | "Return-mapping reproduces the knee with fitted parameters" | \(H\) treated as fundamental, not placeholder |
| [VI.4 three descent signals](../part06-continuum/04-nonlinear-plasticity-preview.md#three-signals-to-descend-from-part-vi-to-part-vii) | History, mesh failure, notch geometry | "Name which signal forces descent — cold-drawn history is signal 1" | Annealed and drawn wire fit with same \(H\) |
| [VI.4 Bridge to Part VII](../part06-continuum/04-nonlinear-plasticity-preview.md#bridge-to-part-vii) | Scale-boundary handshake VI.4 → VII | "\(\sigma_y = \sigma_{y0} + H\alpha\) exports to Taylor \(\tau \propto \sqrt{\rho}\)" | Mesh converged but hardening slope still wrong |
| [VI.4 Perzyna Lab act](../part06-continuum/04-nonlinear-plasticity-preview.md#lab-act-perzyna-viscoplasticity-and-the-strain-rate-knee-act-iv-rate-sensitivity) | Rate sensitivity \(m\) | "Perzyna \((N,\eta)\) are FEM placeholders until DDD mobility" | Rate-independent \(J_2\) at changing grip speed |
| [VII.0 Scene](../part07-defects/00-opening.md#scene) | Forest under the mesh | "Lines carry Burgers vectors; cold drawing stored \(\rho\) before the test" | DDD disconnected from FEM stress field |
| [VII.0 descent paragraph](../part07-defects/00-opening.md#the-descent-in-one-paragraph) | Mesoscale export contract | "Forest density \(\rho\) replaces fitted \(H\)" | OpenDiS run without Peach–Köhler link to Part IV |
| [VII.2 forest Lab act](../part07-defects/02-dislocation-dynamics.md#lab-act-read-the-hardening-bend-from-forest-density-act-iv) | \(\tau(\gamma)\), \(\rho(\gamma)\) export | "Forest density explains the knee without magic \(H\)" | Bulk calibration without link statistics |

**Spot audit (VI.4 → VII.0 boundary).** When row 31 restored virtual work but Part VII feels like a topic change, open the [VI.4 intermission](../part06-continuum/04-nonlinear-plasticity-preview.md#intermission-ascent-ends-descent-begins) — recite the plot spine one line aloud (row 20 midpoint gate), run the [return-mapping Lab act](../part06-continuum/04-nonlinear-plasticity-preview.md#lab-act-return-mapping-on-the-load-cell-knee-act-iv-hardening), then read the [three descent signals](../part06-continuum/04-nonlinear-plasticity-preview.md#three-signals-to-descend-from-part-vi-to-part-vii) and name which signal applies to cold-drawn copper. Row 32 does not replace row 31 (assembly ↔ energy) — it reunites **phenomenological plasticity with mesoscale physics** when the load cell knee is computable but unexplained.

**Baby picture:** row 31 asks whether \(\mathbf{K}\mathbf{U}=\mathbf{F}\) is force balance on \(\Pi[\mathbf{u}]\); row 32 asks whether **fitted \(H\) is a placeholder for forest density \(\rho\) the cold-drawn wire already stored**. The [preface row 32 skill checkpoint](../preface.md#skill-navigation-row-32) closes the competence loop; the [memory sheet row 32 baby picture](memory-sheet.md#row-32-baby-picture-intermission-descent-reunion) draws the intermission → return-mapping → forest chain for index-card review.

## Forest → Handshake 4a reunion index (row 33) {#forest-handshake-4a-reunion-index-row-33}

Row 17 names **Scene → Bridge** chapter rhythm; rows 18–20 name **plot spine** one-liners; row 21 names **Lab act** one-line moves; rows 22–27 name sensory through symbolic reunion; row 28 names **Scale-boundary reunion** export pedigree; row 29 names **Thermoelastic assembly reunion** on one solid mesh; row 30 names **CHT outer-loop reunion** at the wall; row 31 names **twin-ladder → virtual work reunion** in Part VI; row 32 names **intermission → descent reunion** at VI.4 → VII.0. Row 33 names the **forest → Handshake 4a reunion** — when every meta-stitch layer reads correctly but **OpenDiS forest exports and lab-rate hardening still feel like separate subjects** despite row 32 naming \(\rho\), return to the mobility pedigree and rate-extrapolation chain before Part VII.3 or the epilogue.

Each row below is a **representative anchor** on the VII.2 → VII.3 → Handshake 4a path — the mesoscale contract where forest density, segment mobility at \(T_w\), and DDD timestep strain rate must reunite before the load cell imports \(\tau_{\text{lab}}\). Read aloud when row 32 restored \(\rho\) but OpenDiS still cites 300 K mobility or imports \(\tau(\gamma)\) at DDD rate without power-law \(m\).

| Chapter | Forest / rate move | One-line audit on the wire (read aloud) | Failure mode |
|---------|-------------------|----------------------------------------|--------------|
| [VII.2 forest Lab act](../part07-defects/02-dislocation-dynamics.md#lab-act-read-the-hardening-bend-from-forest-density-act-iv) | Taylor \(\tau \propto \sqrt{\rho}\) | "Forest density explains the knee — not fitted \(H\) alone" | \(\rho\) on paper but no OpenDiS archive |
| [VII.2 mobility Lab act](../part07-defects/02-dislocation-dynamics.md#lab-act-calibrate-screw-mobility-from-md-shear-act-iv--mobility-prelude) | \(M(\tau, T_w)\) from MD shear | "Mobility yaml cites \(T_w\) from `cht_export.yaml`, not 300 K default" | Literature Cu table beside Joule-heated wire |
| [VIII.0 descent hinge](../part08-md/00-opening.md#descent-hinge-cores-mobility-and-tw-pedigree) | CHT → MD → DDD temperature | "Act II warmed the wire before Act IV forest moved" | OpenDiS at 300 K after converged CHT |
| [VII.2 OpenDiS workflow](../part07-defects/02-dislocation-dynamics.md#opendis-workflow) | Multi-rate \(\dot\gamma\) runs | "Export \(\tau(\gamma)\) at \(\dot\varepsilon \in \{10^2, 10^3, 10^4\}\,\text{s}^{-1}\)" | Single DDD rate pasted into FEM deck |
| [VII.2 reproducibility checklist](../part07-defects/02-dislocation-dynamics.md#reproducibility-checklist) | Mobility provenance audit | "Archive `mobility_cu_screw_{T_w}K.yaml` beside `ddd_cu_fr/`" | Segment mesh converged but mobility folklore |
| [VII.3 rate handshake](../part07-defects/03-polycrystal-and-fem-handoff.md#scale-boundary-handshake-ddd-strain-rate-to-quasi-static-fem) | \(\tau_{\text{flow}}\) → \(\tau_{\text{lab}}\) | "Power-law \(m\) bridges DDD timestep and lab grip speed" | Direct import at \(10^3\,\text{s}^{-1}\) to \(10^{-3}\,\text{s}^{-1}\) load cell |
| [`parse_rate.sh`](../../scripts/parse_rate.sh) | `ddd_tau_vs_rate.dat` → `rate_export.yaml` | "Parser archives extrapolated \(\tau_{\text{lab}}\), not raw DDD curve" | Hand calculation never exported |
| [Epilogue Handshake 4a](../epilogue/multiscale.md#4a--ddd-strain-rate-to-quasi-static-load-cell-act-iv--hardening) | Bulk hardening at lab rate | "Yield within 5% when \(\tau_{\text{lab}}\) replaces \(\tau_{\text{flow}}\)" | Early yield from rate mismatch |
| [Preface row 14](../preface.md#skill-navigation-row-14) | Workflow-time mirror | "Row 33 is reading time; row 14 is competence time — same chain" | Epilogue 4a feels like a new course after VII.2 |
| [VI.4 Perzyna](../part06-continuum/04-nonlinear-plasticity-preview.md#lab-act-perzyna-viscoplasticity-and-the-strain-rate-knee-act-iv-rate-sensitivity) | FEM placeholder \(m\) | "Perzyna \((N,\eta)\) await DDD power-law from VII.2" | Rate-independent \(J_2\) at changing grip speed |

**Spot audit (VII.2 → VII.3 boundary).** When row 32 restored forest density but the load cell yield still arrives early, open the [VII.2 mobility Lab act](../part07-defects/02-dislocation-dynamics.md#lab-act-calibrate-screw-mobility-from-md-shear-act-iv--mobility-prelude) — verify `temperature_K` in the mobility yaml matches [`cht_export.yaml`](../../fixtures/cht_export.yaml), then run OpenDiS at three strain rates and [`parse_rate.sh`](../../scripts/parse_rate.sh) before opening [VII.3's rate handshake](../part07-defects/03-polycrystal-and-fem-handoff.md#scale-boundary-handshake-ddd-strain-rate-to-quasi-static-fem). Row 33 does not replace row 32 (phenomenology ↔ forest) — it reunites **moving forest with documented mobility and lab-rate exports** when \(\rho\) explains the knee but segment velocity still reads like folklore.

**Baby picture:** row 32 asks why the knee bent; row 33 asks whether **lines moved at the right temperature and strain rate before \(\tau(\gamma)\) crossed the scale boundary**. The [preface row 33 skill checkpoint](../preface.md#skill-navigation-row-33) closes the competence loop; the [memory sheet row 33 baby picture](memory-sheet.md#row-33-baby-picture-forest-handshake-4a-reunion) draws the forest → mobility at \(T_w\) → multi-rate DDD → Handshake 4a chain for index-card review.

## Polycrystal handoff → Handshake 4b reunion index (row 34) {#polycrystal-handshake-4b-reunion-index-row-34}

Row 17 names **Scene → Bridge** chapter rhythm; rows 18–20 name **plot spine** one-liners; row 21 names **Lab act** one-line moves; rows 22–27 name sensory through symbolic reunion; row 28 names **Scale-boundary reunion** export pedigree; row 29 names **Thermoelastic assembly reunion** on one solid mesh; row 30 names **CHT outer-loop reunion** at the wall; row 31 names **twin-ladder → virtual work reunion** in Part VI; row 32 names **intermission → descent reunion** at VI.4 → VII.0; row 33 names **forest → Handshake 4a reunion** on the VII.2 → VII.3 rate path. Row 34 names the **polycrystal handoff → Handshake 4b reunion** — when every meta-stitch layer reads correctly but **bulk crystal plasticity (VII.3 Steps 1–3) and notch localization (FE² / Handshake 4b) still feel like separate subjects** despite row 33 closing bulk \(\tau_{\text{lab}}\), return to the OpenDiS → DAMASK → polycrystal FEM → FE² chain before Act V activates.

Each row below is a **representative anchor** on the VII.3 Steps 1–4 → Handshake 4b path — the mesoscale contract where homogenized hardening must match the load cell in bulk **and** be audited at the notch root before scalar \(H\) is trusted at \(K_t \approx 3\). Read aloud when row 33 restored bulk yield but crystal plasticity and FE² still feel like unrelated workflows.

| Chapter | Polycrystal / FE² move | One-line audit on the wire (read aloud) | Failure mode |
|---------|------------------------|----------------------------------------|--------------|
| [VII.3 Step 1](../part07-defects/03-polycrystal-and-fem-handoff.md#step-1--opendis-rve-simulation) | OpenDiS RVE statistics | "Forest and link statistics archived before homogenization" | DDD restart missing beside DAMASK yaml |
| [VII.3 Step 2](../part07-defects/03-polycrystal-and-fem-handoff.md#step-2--homogenize-ddd-to-crystal-plasticity-parameters) | DAMASK single-element replay | "Taylor fit reproduces \(\tau(\gamma)\) on one Gauss point" | Polycrystal run before single-element audit |
| [VII.3 rate handshake](../part07-defects/03-polycrystal-and-fem-handoff.md#scale-boundary-handshake-ddd-strain-rate-to-quasi-static-fem) | \(\tau_{\text{lab}}\) from [`parse_rate.sh`](../../scripts/parse_rate.sh) | "Bulk deck inherits lab-rate hardening, not raw DDD curve" | FE² inherits wrong rate pedigree from Step 2 |
| [VII.3 Step 3](../part07-defects/03-polycrystal-and-fem-handoff.md#step-3--polycrystal-fem-of-the-wire-damask--mesh) | Polycrystal FEM bulk match | "Force–displacement within 10% of Act IV load cell" | Bulk match declared while notch never tested |
| [VII.3 Lab act](../part07-defects/03-polycrystal-and-fem-handoff.md#lab-act-archive-the-opendis--damask--fem-handoff-act-ivv) | Handoff bundle discipline | "One git commit: opendis.restart + damask.yaml + fem.inp" | Texture or unit mismatch between scales |
| [VII.3 Step 4 FE²](../part07-defects/03-polycrystal-and-fem-handoff.md#step-4--when-offline-calibration-fails-fe-at-the-notch) | DDD-active Gauss points at notch | "48 Gauss points within 50 µm embed OpenDiS RVE" | Scalar \(H\) at notch without pile-up physics |
| [`parse_fe2.sh`](../../scripts/parse_fe2.sh) | `fe2_notch_comparison.dat` → `fe2_export.yaml` | "Parser reports 10.7% uplift: crystal plasticity 215 → FE² 238 MPa" | Hand calculation never archived |
| [Epilogue Handshake 4b](../epilogue/multiscale.md#4b--when-continuum-fails-at-the-notch-md-subdomain-act-v--notch) | Notch-root localization audit | "FE² root stress within 5% or concurrent DDD required" | Bulk adequacy mistaken for notch adequacy |
| [Preface row 15](../preface.md#skill-navigation-row-15) | Workflow-time mirror | "Row 34 is reading time; row 15 is competence time — same chain" | Epilogue 4b feels like a new course after VII.3 Step 3 |
| [VI.4 three descent signals](../part06-continuum/04-nonlinear-plasticity-preview.md#three-signals-to-descend-from-part-vi-to-part-vii) | Notch geometry signal 3 | "Stress concentrator forces descent below homogenization" | Act V skipped because bulk hardening matched |

**Spot audit (VII.3 bulk → notch boundary).** When row 33 restored bulk \(\tau_{\text{lab}}\) but the notch root under-predicts peak stress by 10–15%, open [VII.3 Step 4 FE²](../part07-defects/03-polycrystal-and-fem-handoff.md#step-4--when-offline-calibration-fails-fe-at-the-notch) — verify `rate_export.yaml` from row 33 feeds the same `hardening.yaml`, run crystal plasticity and FE² on the same notch mesh, then [`parse_fe2.sh`](../../scripts/parse_fe2.sh) on [`fixtures/fe2_notch_comparison.dat`](../../fixtures/fe2_notch_comparison.dat). Row 34 does not replace row 33 (forest ↔ rate pedigree) — it reunites **bulk polycrystal homogenization with notch localization physics** when Steps 1–3 pass but Step 4 and Handshake 4b still feel like epilogue-only material.

**Baby picture:** row 33 asks whether bulk yield is credible at lab rate; row 34 asks whether **homogenized crystal plasticity suffices at the notch root or FE² must embed DDD at selected Gauss points**. The [preface row 34 skill checkpoint](../preface.md#skill-navigation-row-34) closes the competence loop; the [memory sheet row 34 baby picture](memory-sheet.md#row-34-baby-picture-polycrystal-handshake-4b-reunion) draws the OpenDiS → DAMASK → polycrystal FEM → FE² → Handshake 4b chain for index-card review.

## VII.3 → VIII descent handoff reunion index (row 35) {#vii3-viii-descent-handoff-reunion-index-row-35}

Row 17 names **Scene → Bridge** chapter rhythm; rows 18–20 name **plot spine** one-liners; row 21 names **Lab act** one-line moves; rows 22–27 name sensory through symbolic reunion; row 28 names **Scale-boundary reunion** export pedigree; row 29 names **Thermoelastic assembly reunion** on one solid mesh; row 30 names **CHT outer-loop reunion** at the wall; row 31 names **twin-ladder → virtual work reunion** in Part VI; row 32 names **intermission → descent reunion** at VI.4 → VII.0; row 33 names **forest → Handshake 4a reunion** on the VII.2 → VII.3 rate path; row 34 names **polycrystal handoff → Handshake 4b reunion** on the bulk → notch path. Row 35 names the **VII.3 → VIII descent handoff reunion** — when every meta-stitch layer reads correctly but **mesoscale export discipline (VII.3 Bridge) and atomistic descent (Part VIII) still feel like separate subjects** despite row 34 closing bulk and notch workflows, return to the Bridge → descent hinge → opening hinge → EAM foundation chain before `mobility.yaml` cites Part VIII on trust alone.

Each row below is a **representative anchor** on the VII.3 Bridge → VIII.0 → VIII.1 path — the scale descent contract where line cores, Peierls thresholds, and mobility tables borrowed from Part VIII must reunite with atomic coordinates, EAM forces, and the \(T_w\) pedigree from Act II. Read aloud when row 34 restored mesoscale handoffs but Peierls stress, core cutoffs, and `MD_NVT_shear_PartVIII` still feel like magic numbers without an atomic box.

| Chapter | Descent handoff move | One-line audit on the wire (read aloud) | Failure mode |
|---------|---------------------|----------------------------------------|--------------|
| [VII.3 Bridge](../part07-defects/03-polycrystal-and-fem-handoff.md#bridge-to-part-viii) | Mesoscale → atomistic export table | "Mobility, \(\gamma_{\text{sf}}\), Peierls — Part VIII must supply the ink" | Lines archived but parameters cite folklore |
| [VII.3 scale-boundary handshake](../part07-defects/03-polycrystal-and-fem-handoff.md#bridge-to-part-viii) | Peierls threshold → core width | "Core cutoff \(r_c\) in OpenDiS awaits atomic \(w\) from MD" | Linear elasticity inside the core |
| [VIII.0 descent hinge](../part08-md/00-opening.md#descent-hinge-cores-mobility-and-tw-pedigree) | \(T_w\) pedigree from CHT | "NVT shear at \(T_w \approx 379\,\text{K}\), not 300 K default" | `mobility.yaml` beside handbook table |
| [VIII.0 plot spine](../part08-md/00-opening.md#plot-spine-one-line) | Act III descent rung 2 | "Line cores and notch tips need atoms on a potential surface" | Part VIII feels like a new course after DDD |
| [VIII.1 opening hinge](../part08-md/01-potentials-phase-space.md#opening-hinge-vii3-to-viii1) | `mobility.yaml` → \(\mathbf{F}_i = -\nabla V\) | "Same copper wire — coordinates replace line singularities" | Bridge read but phase space still feels new |
| [VIII.1 EAM Lab act](../part08-md/01-potentials-phase-space.md#lab-act-eam-lattice-constant-from-energy-minimization-act-v--notch-prelude) | Foundation archive \(a_0\), \(E_{\text{coh}}\) | "256-atom minimization archives Burgers \(b = a_0/\sqrt{2}\)" | DDD yaml with literature \(a_0\) |
| [VIII.1 scale-boundary handshake](../part08-md/01-potentials-phase-space.md#bridge) | VII.3 → VIII.1 → VIII.2 chain | "0 K minimization before NVT at \(T_w\)" | Mobility fit from unminimized lattice |
| [Preface descent hinge row 7](../preface.md#descent-continuity-hinges) | Continuity hinge mirror | "Row 35 is meta-stitch; row 7 is the first descent hinge" | Skipped Bridge before Part VIII opening |
| [Preface row 8](../preface.md#skill-navigation-row-8) | Workflow-time \(T_w\) mirror | "Row 35 is reading time; row 8 is competence time — same \(T_w\)" | MD mobility at 300 K after CHT |
| [VII.2 mobility Lab act](../part07-defects/02-dislocation-dynamics.md#lab-act-calibrate-screw-mobility-from-md-shear-act-iv--mobility-prelude) | Upstream MD shear contract | "`MD_NVT_shear_PartVIII` must exist before OpenDiS inherits table" | Mobility yaml cites missing MD run |

**Spot audit (VII.3 → VIII.1 boundary).** When row 34 restored bulk and notch mesoscale workflows but Part VIII still feels like a new subject, open [VII.3's Bridge](../part07-defects/03-polycrystal-and-fem-handoff.md#bridge-to-part-viii) — read the export/downward table aloud, then the [VIII.0 descent hinge](../part08-md/00-opening.md#descent-hinge-cores-mobility-and-tw-pedigree) and [VIII.1 opening hinge](../part08-md/01-potentials-phase-space.md#opening-hinge-vii3-to-viii1). Verify `mobility.yaml` in the VII.3 handoff bundle cites `MD_NVT_shear_PartVIII` **and** that the [VIII.1 EAM Lab act](../part08-md/01-potentials-phase-space.md#lab-act-eam-lattice-constant-from-energy-minimization-act-v--notch-prelude) archives `cu_eam_a0.txt` beside `cht_export.yaml`. Row 35 does not replace row 34 (bulk ↔ notch) or row 7 (continuity hinge) — it reunites **mesoscale parameter exports with atomistic foundation** when the Bridge named the ink but no atomic box yet grounds Peierls stress, core width, or \(\gamma_{\text{sf}}\).

**Baby picture:** row 34 asks whether homogenization suffices at the notch; row 35 asks whether **the ink behind dislocation lines has an atomic address** — coordinates, EAM forces, and \(a_0\) before NVT shear at \(T_w\). The [preface row 35 skill checkpoint](../preface.md#skill-navigation-row-35) closes the competence loop; the [memory sheet row 35 baby picture](memory-sheet.md#row-35-baby-picture-vii3-viii-descent-handoff-reunion) draws the VII.3 Bridge → VIII.0 descent hinge → VIII.1 opening hinge → EAM foundation chain for index-card review.

## Potentials → ensembles reunion index (row 36) {#potentials-ensembles-reunion-index-row-36}

Row 17 names **Scene → Bridge** chapter rhythm; rows 18–20 name **plot spine** one-liners; row 21 names **Lab act** one-line moves; rows 22–27 name sensory through symbolic reunion; row 28 names **Scale-boundary reunion** export pedigree; row 29 names **Thermoelastic assembly reunion** on one solid mesh; row 30 names **CHT outer-loop reunion** at the wall; row 31 names **twin-ladder → virtual work reunion** in Part VI; row 32 names **intermission → descent reunion** at VI.4 → VII.0; row 33 names **forest → Handshake 4a reunion** on the VII.2 → VII.3 rate path; row 34 names **polycrystal handoff → Handshake 4b reunion** on the bulk → notch path; row 35 names **VII.3 → VIII descent handoff reunion**. Row 36 names the **potentials → ensembles opening hinge reunion** — when every meta-stitch layer reads correctly but **0 K EAM minimization and NVT/NPT dynamics still feel like separate subjects** despite the same foundation archive and the same \(T_w\) from `cht_export.yaml`, return to the VIII.1 Bridge → VIII.2 opening hinge chain before exporting mobility or moduli upward.

Each row below is a **representative anchor** on the VIII.1 → VIII.2 path — the second descent stitch within Part VIII where energy minima become canonical samples of phase space. Read aloud when row 35 restored core-cutoff pedigree but `MD_NVT_shear_PartVIII` still has not run at \(T_w\).
...
**Spot audit (VIII.1 → VIII.2 gate).** When [VIII.1](../part08-md/01-potentials-phase-space.md) minimized the lattice but [VIII.2](../part08-md/02-ensembles-integrators.md) feels like standalone statistical mechanics, open the [EAM Lab act](../part08-md/01-potentials-phase-space.md#lab-act-eam-lattice-constant-from-energy-minimization-act-v--notch-prelude) — verify `cu_eam_a0.txt` sits in the foundation folder. Then read [VIII.1's Bridge](../part08-md/01-potentials-phase-space.md#bridge) and the [VIII.2 opening hinge](../part08-md/02-ensembles-integrators.md#opening-hinge-viii1-to-viii2) — confirm [`fixtures/cht_export.yaml`](../../fixtures/cht_export.yaml) gives \(T_w = 311.48\,\text{K}\) before NPT equilibration. Row 36 does not replace row 35 (mesoscale ↔ atomistic pedigree) — it reunites **static potential surface with dynamic sampling** when cores are clear but trajectories still feel disconnected from the EAM archive.

**Baby picture:** row 35 asks whether MD measures DDD mobility; row 36 asks whether **NVT/NPT is what happens after 0 K minimization** — same potential, same \(T_w\), but phase space instead of energy minimum. The [preface row 36 skill checkpoint](../preface.md#skill-navigation-row-36) closes the competence loop; the [memory sheet row 36 baby picture](memory-sheet.md#row-36-baby-picture-potentials-ensembles-reunion) draws the VIII.1 → VIII.2 opening hinge chain for index-card review.

## Ensembles → coarse-graining reunion index (row 37) {#ensembles-coarse-graining-reunion-index-row-37}

Row 17 names **Scene → Bridge** chapter rhythm; rows 18–20 name **plot spine** one-liners; row 21 names **Lab act** one-line moves; rows 22–27 name sensory through symbolic reunion; row 28 names **Scale-boundary reunion** export pedigree; row 29 names **Thermoelastic assembly reunion** on one solid mesh; row 30 names **CHT outer-loop reunion** at the wall; row 31 names **twin-ladder → virtual work reunion** in Part VI; row 32 names **intermission → descent reunion** at VI.4 → VII.0; row 33 names **forest → Handshake 4a reunion** on the VII.2 → VII.3 rate path; row 34 names **polycrystal handoff → Handshake 4b reunion** on the bulk → notch path; row 35 names **VII.3 → VIII descent handoff reunion**; row 36 names **potentials → ensembles opening hinge reunion**. Row 37 names the **ensembles → coarse-graining opening hinge reunion** — when every meta-stitch layer reads correctly but **NVT/NPT trajectories and coarse-grained handoff tables still feel like separate subjects** despite audited integrators and `mobility_cu_screw_{T_w}K.yaml`, return to the VIII.2 Bridge → VIII.3 opening hinge → pedigree checklist chain before opening Part IX.

Each row below is a **representative anchor** on the VIII.2 → VIII.3 path — the third descent stitch within Part VIII where phase-space movies compress into yaml tables with named consumers and DFT audit gates. Read aloud when row 36 restored dynamics pedigree but raw LAMMPS dumps still export without a handoff bundle or pedigree checklist.

| Chapter | Ensembles → coarse-graining move | One-line audit on the wire (read aloud) | Failure mode |
|---------|----------------------------------|----------------------------------------|--------------|
| [VIII.2 NPT Lab act](../part08-md/02-ensembles-integrators.md#lab-act-npt-tension-on-a-copper-nanowire-segment) | Audited \(E\), \(\nu\) at \(T_w\) | "NVE drift flat; moduli within 10% before export" | Transient thermostat exported as yield |
| [VIII.2 NVT shear](../part08-md/02-ensembles-integrators.md#opening-hinge-viii1-to-viii2) | \(M(\tau, T_w)\) converged | "`mobility_cu_screw_311K.yaml` beside thermostat log" | Mobility yaml cites missing shear run |
| [VIII.2 VACF phonon audit](../part08-md/02-ensembles-integrators.md#phonon-density-of-states-from-velocity-autocorrelation) | Phonon peaks archived | "`cu.phonon/` beside `cu.elastic/` foundation folders" | Bulk-fit EAM wrong at dislocation core |
| [VIII.2 Bridge](../part08-md/02-ensembles-integrators.md#bridge) | Export chapter named | "Trajectories stop being movies when handoff tables exist" | Skipping Bridge to VIII.3 |
| [VIII.2 Scale-boundary handshake](../part08-md/02-ensembles-integrators.md#bridge) | MD export → audit gate → consumer | "Each row names filename, consumer, failure mode" | Raw dump files without yaml |
| [VIII.3 opening hinge from VIII.2](../part08-md/03-ab-initio-and-coarse-graining.md#opening-hinge-viii2-to-viii3) | Compression layer vocabulary | "Coarse-grained numbers replace raw trajectories" | Part VIII.3 feels like standalone AIMD coursework |
| [VIII.3 pedigree checklist](../part08-md/03-ab-initio-and-coarse-graining.md#pedigree-checklist-before-the-epilogue) | DFT audit gates named | "Every upward export has minimum QE evidence" | EAM on trust without `pw.x` log |
| [VIII.3 EAM-fit audit Lab act](../part08-md/03-ab-initio-and-coarse-graining.md#lab-act-eam-fit-audit-before-the-notch-md-run-act-v--notch) | Act V gate before notch MD | "Five-row acceptance table passes before Act V" | Notch nucleation on un-audited potential |
| [VIII.3 Bridge to Part IX](../part08-md/03-ab-initio-and-coarse-graining.md#bridge-to-part-ix) | Downward audit request | "EAM matches bulk but cite the DFT input deck" | Multiscale folklore without SCF pedigree |

**Spot audit (VIII.2 → VIII.3 gate).** When [VIII.2](../part08-md/02-ensembles-integrators.md) proved NVE drift and NPT moduli but [VIII.3](../part08-md/03-ab-initio-and-coarse-graining.md) feels like standalone ab initio MD theory, open the [VIII.2 Bridge](../part08-md/02-ensembles-integrators.md#bridge) — read the scale-boundary handshake table aloud. Verify `mobility_cu_screw_311K.yaml`, `cu.elastic/`, and `cu.phonon/` sit in the foundation folder beside [`fixtures/cht_export.yaml`](../../fixtures/cht_export.yaml). Then read the [VIII.3 opening hinge](../part08-md/03-ab-initio-and-coarse-graining.md#opening-hinge-viii2-to-viii3) and the [pedigree checklist](../part08-md/03-ab-initio-and-coarse-graining.md#pedigree-checklist-before-the-epilogue) — confirm each upward export row names a Part IX audit gate. Row 37 does not replace row 36 (static surface ↔ phase space) — it reunites **audited trajectories with coarse-grained contracts** when dynamics closed but handoff tables still feel disconnected from OpenDiS and FEM consumers.

**Baby picture:** row 36 asks whether NVT/NPT follows 0 K minimization; row 37 asks whether **yaml handoff tables follow converged trajectories** — same \(T_w\), same potential, but filenames, consumers, and DFT pedigree gates instead of dump files. The [preface row 37 skill checkpoint](../preface.md#skill-navigation-row-37) closes the competence loop; the [memory sheet row 37 baby picture](memory-sheet.md#row-37-baby-picture-ensembles-coarse-graining-reunion) draws the VIII.2 Bridge → VIII.3 opening hinge → pedigree checklist chain for index-card review.
