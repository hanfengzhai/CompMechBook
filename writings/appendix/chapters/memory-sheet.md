...
**Subgraph node ↔ IX.3 pedigree row audit.** Each node in the `act6` and `orch` subgraphs above maps one-to-one to a row in [IX.3's epilogue pedigree table](../part09-dft/03-dft-workflows.md#ix3-epilogue-pedigree-table) — return there when a subgraph label feels abstract without archive artifacts; return here when the pedigree table feels like isolated rows without workflow order:

| Subgraph node | IX.3 pedigree row | Minimal artifact |
|---------------|-------------------|------------------|
| `act6`: DFT | Foundation checklist rows 1–6 | `cu.scf.in`, `cu.elastic/`, `cu.gsf/` |
| `act6`: MD | *(Part VIII EAM fit on DFT exports)* | EAM potential with DFT pedigree |
| `act6`: DDD | *(Part VII mobility from MD)* | `mobility_cu_screw_{T_w}K.yaml` |
| `act6`: FEM | Pedigree row **1 — DFT → continuum** | Voigt \(E\), \(\nu\) in input deck |
| `orch`: H1 | Pedigree row **1 — DFT → continuum** | `foundation_export.yaml` |
| `orch`: H2 | Pedigree row **2 — Joule → CHT** | `cht_export.yaml` |
| `orch`: H3 | Pedigree row **3 — thermal → mechanical** | `alpha_cu_300K.dat` |
| `orch`: H4a | Pedigree row **4a — rate hardening** | `rate_export.yaml` |
| `orch`: H4b | Pedigree row **4b — notch localization** | `fe2_export.yaml` |
| `orch`: OUT | Pedigree row **All — orchestrated chain** | `multiscale_export.yaml` |

The [IX.3 pedigree ↔ parser step mapping](../part09-dft/03-dft-workflows.md#ix3-pedigree-parser-step-mapping) adds script audit trail and [preface row 16](../preface.md#skill-navigation-row-16) step columns to this node audit — return there when subgraph labels need parser names.
