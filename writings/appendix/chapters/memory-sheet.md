...
When row 63 feels disconnected from row 62, read them as **notch vs foundation capstones**: row 62 when **Handshake 4b must finish before Act VI**; row 63 when **that Act VI foundation must inherit dependency-ordered `multiscale_export.yaml`, not verified yamls in separate folders** — same multiscale afternoon, IX.3 pedigree and H1→OUT chain in the rear-view mirror.
