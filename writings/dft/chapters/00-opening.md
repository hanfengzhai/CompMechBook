...
[Part VIII's descent hinge](../part08-md/00-opening.md#descent-hinge-cores-mobility-and-tw-pedigree) closed the **atomistic descent** with vibrating nuclei on EAM potentials, mobility tables at converged \(T_w = 311.48\,\text{K}\) from [`fixtures/cht_export.yaml`](../../fixtures/cht_export.yaml), and phonon-lifetime interpolation for rate sensitivity — not room-temperature defaults from handbook folders. Part IX inherits that **temperature pedigree** at the electronic layer: every quasiharmonic \(\alpha(T)\) export, every phonon DOS used for Handshake 3 thermal strain, and every acoustic-mode lifetime that feeds Handshake 4a drag must be evaluated at the same \(T_w\) archived in `cht_export.yaml` — archive `alpha_export.yaml` beside it with an explicit \(T_w\) column, not a silent 300 K extrapolation. When row 35 restored coarse-grained exports but Schrödinger's equation still feels like a new subject, read the [preface row 36 skill checkpoint](../preface.md#skill-navigation-row-36) — the competence-time mirror of the [opening hinge from VIII.3](#opening-hinge-viii3-to-ix) below.
...
Joule heating raised the wire wall to \(T_w = 311.48\,\text{K}\) in Act II (archived in [`fixtures/cht_export.yaml`](../../fixtures/cht_export.yaml); the Ra/Nu Lab act in [V.4](../part05-fvm/04-navier-stokes-cfd.md#lab-act-extension-two-domain-picard-loop-with-a-1d-fem-solid) teaches a higher engineering estimate near \(379\,\text{K}\) — pick **one** converged export). Most foundation folders archive phonons at 300 K — correct for handbook comparison, **insufficient** for the multiscale afternoon unless you extrapolate:
...
## Opening hinge from VIII.3 {#opening-hinge-viii3-to-ix}

If you have read linearly since the prologue, [VIII.3](../part08-md/03-ab-initio-and-coarse-graining.md) closed with the [pedigree checklist](../part08-md/03-ab-initio-and-coarse-graining.md#pedigree-checklist-before-the-epilogue) — five export rows naming consumers from LAMMPS through OpenDiS — and a [Bridge to Part IX](../part08-md/03-ab-initio-and-coarse-graining.md#bridge-to-part-ix) that requested downward audit without yet showing \(\rho(\mathbf{r})\). Part IX.0 does not re-derive coarse-graining or EAM fitting; it **audits** every row the checklist named:

| Part VIII.3 export (pedigree checklist row) | Part IX.0 audit gate |
|---------------------------------------------|----------------------|
| Lattice parameter \(a_0\) from EAM fit | Murnaghan SCF vs volume; `cu.relax.out` in foundation folder |
| \(E_{\text{coh}}\), \(C_{ij}\) from NPT / handoff table | Strained fcc cells; [`parse_elastic.sh`](../../scripts/parse_elastic.sh) on `cu.elastic/` |
| \(\gamma_{\text{sf}}\) for partial dislocations | GSF slab from [IX.3](03-dft-workflows.md); [`parse_gsf.sh`](../../scripts/parse_gsf.sh) |
...
When **this Bridge names Born–Oppenheimer but SCF logs and \(\alpha(T_w)\) are still missing** after row 35 restored yaml handoff tables, read the [coarse-graining → electronic audit reunion index](../appendix/sources.md#coarse-graining-electronic-audit-reunion-index-row-36) (row 36) at the [preface coarse-graining → electronic audit thread](../preface.md#reading-rhythm) before opening [IX.1](01-born-oppenheimer.md). Row 36 reunites **handoff tables with SCF audit gates**; row 35 remains the audit when yaml exports still feel disconnected from converged trajectories.

When **`cu.relax.out` and \(\alpha(T_w)\) exist but IX.1 still feels like standalone quantum chemistry** — SCF convergence is archived but \(V_{\text{BO}}\) linking to Part VIII's EAM potential is unnamed — read the [electronic audit → Born–Oppenheimer reunion index](../appendix/sources.md#electronic-audit-born-oppenheimer-reunion-index-row-37) (row 37) at the [preface electronic audit → Born–Oppenheimer thread](../preface.md#reading-rhythm) before opening [IX.2](02-kohn-sham.md). Row 38 names the **Born–Oppenheimer → Kohn–Sham** stitch when BO/HK theorems are understood but the SCF fixed-point loop still feels like a new course.
