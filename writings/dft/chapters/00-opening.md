...
## Third descent rung: VIII midpoint reunion {#third-descent-rung-viii-midpoint-reunion}

If you paused at [Part VIII's opening](../part08-md/00-opening.md#second-descent-rung-vii-midpoint-reunion), the **atomistic** export contract is in place — EAM fits, mobility yaml, trajectory evidence for \(\gamma_{\text{sf}}\) and \(M(\tau,T)\). If you paused at [VIII.3's intermission](../part08-md/03-ab-initio-and-coarse-graining.md#intermission-atomistics-ends-electronics-begin), the **plot** turns downward one last time — interatomic potentials on trust need an SCF audit, not another fitted cutoff. Part IX is the chapter block where both pauses land on the same bench: the fcc copper cell still answers to the wire, but the state variable is no longer positions \(\{\mathbf{r}_i\}\) alone.

| Part VIII atomistic export | Part IX electronic consumer | What changes on the wire |
|----------------------------|----------------------------|--------------------------|
| EAM \(V(\{\mathbf{r}_i\})\) on trust | Hohenberg–Kohn \(E[\rho]\) | Tabulated forces → self-consistent density |
| \(E_{\text{coh}}\) from NPT fluctuations | Murnaghan SCF energy vs volume | Statistical average → variational ground state |
| \(C_{ij}\) from stress–strain MD | Strained fcc cells (force theorem) | Trajectory noise → converged stress derivatives |
| [Atomistic ladder](../part08-md/00-opening.md#the-atomistic-ladder-me-412-cross-index) (Schematic 14 parallel) | [Electronic ladder](#the-electronic-ladder-me-412-cross-index) (Schematic 14 parallel) | Trajectory averaging → SCF convergence certificates |
| [VIII.3 pedigree checklist](../part08-md/03-ab-initio-and-coarse-graining.md#pedigree-checklist-before-the-epilogue) | Five-row QE audit in [IX.3](03-dft-workflows.md) | EAM-fit pass criteria → `pw.x` log per export |

[Part VIII's ME 412 cross-index](../part08-md/00-opening.md#representative-schematics-me-412-part-viii) completed Schematic 14's **atomistic parallel**. Part IX begins Schematic 14's **electronic parallel** — Born–Oppenheimer separation, variational ground state, and workflow exports that reunite with EAM tables in [IX.3](03-dft-workflows.md). When atomistic fits and electronic structure feel like separate manuals, read this reunion table beside the [preface descent continuity hinges](../preface.md#descent-continuity-hinges): atomistic → electronic at [VIII.3](../part08-md/03-ab-initio-and-coarse-graining.md#bridge-to-part-ix), electronic → coupling at [IX.3](03-dft-workflows.md#bridge-to-the-epilogue).
...
## Representative schematics (ME 412 cross-index) {#representative-schematics-me-412-part-ix}

[Part II's opening](../part02-functional-analysis/00-opening.md#representative-schematics-me-412) indexed all fourteen schematics from the [Functional Analysis Notes](https://hanfengzhai.github.io/file/teaching/notes/ME412_CourseSummary.pdf). [Part VIII](../part08-md/00-opening.md#representative-schematics-me-412-part-viii) completed Schematic 14's **atomistic parallel**. Part IX does **not** repeat Galerkin projection — electronic structure is variational minimization on density, not energy minimization on a displacement field — but five ME 412 structures still govern convergence and export discipline:

| ME 412 Schematic | Idea | Baby picture (ME 412) | Part IX chapter |
|------------------|------|----------------------|-----------------|
| 1b | Linear algebra → operator problems; well-posedness | Kohn–Sham is an eigenvalue loop on a Hamiltonian operator; ground state is Part I's \(\mathbf{K}\mathbf{u}=\mathbf{f}\) at infinite dimension | [IX.1](01-born-oppenheimer.md)–[IX.2](02-kohn-sham.md) |
| 6 | Orthogonal projection; best approximation in Hilbert space | Plane-wave expansion projects \(\psi(\mathbf{r})\) onto a finite basis; cutoff convergence is best approximation in orbital space | [IX.2](02-kohn-sham.md) |
| 7 | Duality, Riesz representation, weak convergence | k-mesh convergence: total energy and forces stabilize as Brillouin-zone sampling refines | [IX.2](02-kohn-sham.md)–[IX.3](03-dft-workflows.md) |
| 8b | Well-posedness triangle: existence, uniqueness, stability | SCF must converge uniquely; oscillating density is the electronic analogue of an ill-posed BVP | [IX.2](02-kohn-sham.md) |
| 13b | Open Mapping, Bounded Inverse, Banach–Steinhaus | Cutoff and k-mesh must give **uniform** energy-drift bounds as basis size grows | [IX.2](02-kohn-sham.md)–[IX.3](03-dft-workflows.md) |
| 14 | Variational ladder (ascent branch) | Part IX runs the **electronic parallel** — see [below](#the-electronic-ladder-me-412-cross-index) | [IX.1](01-born-oppenheimer.md)–[IX.3](03-dft-workflows.md) |

Schematic **1b** is the explicit **Part I → Part IX hinge** in the ME 412 map: [Part I](../part01-linear-algebra/00-opening.md) diagonalized \(\mathbf{K}\) for normal modes; Part IX diagonalizes the Kohn–Sham Hamiltonian for orbitals — the same eigenvalue instinct at the finest scale. Schematic **6** names plane-wave truncation as **projection** onto a finite orbital subspace — the electronic echo of Galerkin best approximation. Schematic **7** is the k-mesh convergence instinct: doubling Monkhorst–Pack density and checking \(E_{\text{tot}}\) is the electronic mesh-refinement study. Schematic **13b** is the stability audit behind \(E_{\text{cut}}\) choice and SCF mixing parameters — total energy must not drift when the basis grows.

The DFT schematics (1–3) and ME 412 cross-index (1b, 6, 7, 8b, 13b, 14 parallel) are **two labels for one electronic pipeline** — DFT Notes name the implementation stages; ME 412 names the analysis structures those stages inherit from Parts I–II and VIII. When EAM parameters feel like literature constants, match DFT Schematic 3 to ME 412 Schematic 7: *what object is minimized, over what admissible set, with what convergence certificate?*

## The electronic ladder (ME 412 Schematic 14 parallel) {#the-electronic-ladder-me-412-cross-index}

Part VIII completed Schematic 14's **atomistic parallel**. Part IX runs the **electronic parallel** — the finest branch before the epilogue climbs back up:

```mermaid
flowchart TB
  BO[Born-Oppenheimer separation] --> HK[Hohenberg-Kohn density functional]
  HK --> KS[Kohn-Sham SCF loop]
  KS --> QE[Quantum ESPRESSO exports]
  QE --> MD[EAM fit Part VIII]
  MD --> EP[Multiscale epilogue]
```

Read Part IX as the **descent rungs** [VIII.3's Bridge](../part08-md/03-ab-initio-and-coarse-graining.md#bridge-to-part-ix) demanded but could not derive. When MD exports disagree with DFT moduli, walk the electronic ladder backward: if \(C_{11}\) drifts with k-mesh, check strained-cell protocol ([IX.3](03-dft-workflows.md)); if SCF oscillates, check mixing and starting density ([IX.2](02-kohn-sham.md)); if Born–Oppenheimer separation is questioned at high \(T\), check whether phonons or excited states matter ([IX.1](01-born-oppenheimer.md)). The copper wire's Act VI foundation folder is one instance of both ladders speaking at an interface — trajectory evidence from Part VIII, SCF logs from Part IX, handshake loops in the epilogue.

## Scale-boundary discipline

Every quantity DFT exports must carry a pedigree row in the foundation folder (see [Act VI in the sources appendix](../appendix/sources.md#six-acts--parts-laboratory-time)):

| Export | Minimum DFT evidence | Downstream consumer |
|--------|---------------------|---------------------|
| \(a_0\) | `vc-relax`; forces \(< 10^{-4}\) Ry/Bohr | Part VIII EAM box; Part VII Burgers \(b\) |
| \(E_{\text{coh}}\) | Converged SCF at equilibrium volume | EAM embedding function sanity |
| \(\mathbb{C}_{ij}\) | ±0.5% strain cells; symmetric stress response | Part IV elastic step; Part VI \(\mathbb{C}\) |
| \(\gamma_{\text{sf}}\) | Relaxed generalized stacking-fault slab | Part VII partial dislocation separation |
| \(\alpha(T)\) | Quasiharmonic phonons at documented k-mesh | Part VI thermal strain; epilogue Handshake 3 |

If a row lists only "Mishin EAM, 2001" with no QE `pw.x` log, this part is the audit chapter — the same role Part VIII played when Part VII's mobility tables needed trajectory evidence. Workflow readers may have run DFT before LAMMPS — both paths converge when the pedigree checklist is populated before the epilogue's multiscale afternoon.
