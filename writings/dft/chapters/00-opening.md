...
## Closing the arc from Part VIII {#opening-hinge-part-viii-to-ix}
...
The [preface descent continuity hinges](../preface.md#descent-continuity-hinges) name [VIII.3 → IX](../part08-md/03-ab-initio-and-coarse-graining.md#bridge-to-part-ix) as the **atomistic → electronic** turn — the last hinge before the epilogue couples every export. Read this opening when EAM parameters feel like black boxes; read [VIII.3's pedigree checklist](../part08-md/03-ab-initio-and-coarse-graining.md#pedigree-checklist-before-the-epilogue) row-by-row as you work through IX.1–IX.3.
...
| Part VIII export (on trust) | Part IX audit (this part) | Epilogue consumer | Failure mode |
|-----------------------------|---------------------------|-------------------|--------------|
| EAM \(a_0\), \(E_{\text{coh}}\) from fit or experiment | Relaxed fcc cell; SCF total energy per atom | Handshake 1: Voigt \(E\), \(\nu\) in FEM | Wrong functional silently in LAMMPS deck |
| Mobility tables from MD shear | GSF surface from slab DFT | Handshake 4: OpenDiS \(\tau(\gamma)\) | Peierls stress off by 20% without GSF |
| Phonon DOS from VACF | DFPT or finite-difference phonons | Thermal expansion cross-check | Incomplete k-mesh in `ph.x` |
| [VIII.3 pedigree checklist](../part08-md/03-ab-initio-and-coarse-graining.md#pedigree-checklist-before-the-epilogue) five rows | Each row receives QE log + convergence proof | [Epilogue Handshake 1](../epilogue/multiscale.md#handshake-1--dft--continuum-elastic-constants-part-ix--vi) | Act VI foundation folder missing `pw.x` archive |
