...
| IX.3 pedigree row | Act VI subgraph node | Script audit trail parser | Preface row 16 step | Step audit row |
|-------------------|----------------------|---------------------------|---------------------|----------------|
| **1 — DFT → continuum** | `act6`: DFT→MD→DDD→FEM; `orch`: H1 | [`parse_dft_workflow.sh`](../../scripts/parse_dft_workflow.sh), [`parse_elastic.sh`](../../scripts/parse_elastic.sh) | Step 2 — Foundation folder | Row 2 |
| **2 — Joule → CHT** | `orch`: H2 | [`parse_cht.sh`](../../scripts/parse_cht.sh) | Step 1 (understood) + Step 4 (\(\Delta T\) pedigree) | Rows 1, 4 |
| **3 — thermal → mechanical** | `act6`: DFT (phonon); `orch`: H3 | [`parse_alpha.sh`](../../scripts/parse_alpha.sh) | Step 1 + Step 4 | Rows 1, 4 |
| **4a — rate hardening** | `act6`: DDD; `orch`: H4a | [`parse_rate.sh`](../../scripts/parse_rate.sh), [`parse_lifetime.sh`](../../scripts/parse_lifetime.sh) | Step 1 + Step 4 (\(\tau_{\text{ph}}\) at \(T_w\)) | Rows 1, 4 |
| **4b — notch localization** | `act6`: DFT (GSF); `orch`: H4b | [`parse_fe2.sh`](../../scripts/parse_fe2.sh), [`parse_gsf.sh`](../../scripts/parse_gsf.sh) | Step 1 (understood) | Row 1 |
| **All — orchestrated chain** | `orch`: OUT | [`parse_multiscale_workflow.sh`](../../scripts/parse_multiscale_workflow.sh) | Step 3 — Orchestrated chain | Row 3 |
