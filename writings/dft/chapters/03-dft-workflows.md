...
| Epilogue handshake | Subgraph node ([Act VI baby picture](../appendix/memory-sheet.md#act-vi-baby-picture-me-412-coupling-ladder)) | DFT export (this chapter) | Archive artifact | Upstream part that consumes it |
|--------------------|-------------------------------------------------------------------------------------------------------------|---------------------------|------------------|--------------------------------|
| **1 — DFT → continuum** | `orch`: **H1**; `act6`: **DFT** → **FEM** | Voigt \(E\), \(\nu\) from symmetric \(C_{ij}\); relaxed \(a_0\) | `cu.elastic/`, `cu.relax.out` | Part IV elastic step; Part VI \(\mathbb{C}\) |
| **2 — Joule → CHT** | `orch`: **H2** | *(Parts IV–V workflow — not a DFT export)* converged \(T_w\), \(h\), \(\Delta T = T_w - T_\infty\) | `cht_export.yaml` via [`parse_cht.sh`](../../scripts/parse_cht.sh) | Sets \(\Delta T\) for Handshakes 3–4a; [dominates \(T_w\) in sensitivity table](../epilogue/multiscale.md#sensitivity-which-handshake-matters-most); [preface hinge row](../preface.md#epilogue-continuity-hinges) |
| **3 — thermal → mechanical** | `orch`: **H3**; `act6`: **DFT** (phonon) | Quasiharmonic \(\alpha(T)\) from `ph.x` / phonon DOS; fixed-grip \(\sigma_{\text{th}} = E\alpha\Delta T\) with \(\Delta T\) from Handshake 2 — [ranks first for load-cell stress](../epilogue/multiscale.md#sensitivity-which-handshake-matters-most) | `alpha_cu_300K.dat`, `cu.phonon/a_vs_T.dat` | Part VI return-mapping at heated grip; [Handshake 3 worked example](../epilogue/multiscale.md#handshake-3--thermal-strain--mechanical-stiffness-part-vi--iv) |
| **4a — rate hardening** | `orch`: **H4a**; `act6`: **MD** → **DDD** | Phonon lifetime at \(T_w\) bounds drag on \(m\); [`parse_lifetime.sh`](../../scripts/parse_lifetime.sh) | `cu.phonon/phonon_lifetime_vs_T.dat` + MD VACF cross-check | [Epilogue Handshake 4a](../epilogue/multiscale.md#4a--ddd-strain-rate-to-quasi-static-load-cell-act-iv--hardening); [VII.3 rate handshake](../part07-defects/03-polycrystal-and-fem-handoff.md#scale-boundary-handshake-ddd-strain-rate-to-quasi-static-fem); [sensitivity worksheet](../epilogue/multiscale.md#worked-example-sensitivity-ranks) (4a column) |
| **4b — notch localization** | `orch`: **H4b**; `act6`: **DDD** (GSF) | GSF \(\gamma_{\text{sf}}\) from [slab workflow](#worked-example-generalized-stacking-fault-energy-handoff-to-part-vii); vacancy \(E_f^v\) | `cu.gsf/`, `gsf_cu111.dat` via [`parse_gsf.sh`](../../scripts/parse_gsf.sh) | [Epilogue Handshake 4b](../epilogue/multiscale.md#4b--when-continuum-fails-at-the-notch-md-subdomain-act-v--notch); [VII.3 Step 4 FE²](../part07-defects/03-polycrystal-and-fem-handoff.md#step-4--when-offline-calibration-fails-fe-at-the-notch); upstream 4a from [preface row 14](../preface.md#skill-navigation-row-14) |
| **All — orchestrated chain** | `orch`: **OUT** (`multiscale_export.yaml`) | *(orchestrator — not a single DFT export)* | [`parse_multiscale_workflow.sh`](../../scripts/parse_multiscale_workflow.sh) on `cu.foundation/` + `cht_wire.conf` | [Epilogue script audit trail](../epilogue/multiscale.md#script-audit-trail-parse-scripts-handshakes); [preface row 16 step 3–4](../preface.md#skill-navigation-row-16); [memory sheet subgraph node audit](../appendix/memory-sheet.md#act-vi-subgraph-node-audit) |

Each row above maps to a parser in the [epilogue script audit trail](../epilogue/multiscale.md#script-audit-trail-parse-scripts-handshakes) — the **All — orchestrated chain** row is what [`parse_multiscale_workflow.sh`](../../scripts/parse_multiscale_workflow.sh) runs in dependency order to emit `multiscale_export.yaml` from this table's artifacts. Return to this pedigree table when the orchestrated chain row feels like a script name without workflow context — the [preface epilogue hinges Act VI row](../preface.md#epilogue-continuity-hinges) names the same stitch in competence time; the [prologue row 16 preview](../prologue/00-many-scales.md#what-you-should-be-able-to-do-after-the-prologue) and [workflow exam Act VI row](../epilogue/multiscale.md#what-you-should-be-able-to-do-after-the-book) sketch and audit the minimal artifact column; the [memory sheet one-page recap Act VI column](../appendix/memory-sheet.md#one-page-copper-wire-recap) compresses it for index-card review; the [sensitivity worksheet closing](../epilogue/multiscale.md#worked-example-sensitivity-ranks) (row 16 orchestration stitch) proves Handshakes 1–4b ran in dependency order after the partial-derivative audit. When rows 17–42 restored every narrative layer but individual exports still lack orchestrated pedigree, read the [preface row 43 skill checkpoint](../preface.md#skill-navigation-row-43) — the competence-time mirror of the [epilogue opening hinge from rows 17–42 → row 16](../epilogue/multiscale.md#opening-hinge-rows17-42-row16); the [Rows 17–42 → Row 16 orchestration reunion index](../appendix/sources.md#rows17-42-row16-orchestration-reunion-index-row-43) reunites this pedigree table with the full-book closing arc when Handshakes 1–4b feel like separate terminal windows.
...
## Writings canonical hinge (IX.3 → Epilogue) {#writings-canonical-hinge-ix3-to-epilogue}

The unified book ([`SUMMARY.md`](../../SUMMARY.md)) and the vendored [`writings/dft`](../../writings/dft/chapters/SUMMARY.md) subtree close on this Bridge; [`writings/epilogue`](../../writings/epilogue/chapters/SUMMARY.md) opens at [Epilogue Writings canonical landing](../epilogue/multiscale.md#writings-canonical-landing-ix3-to-epilogue) with the same Functional Analysis Notes layout (`chapters/SUMMARY.md`, single closing chapter, **Bridge**-style handoff sections) but upstream **ME 412 multiscale coupling** coursework instead of standalone MSE 5720 homework folders. When the layout feels continuous but **Handshakes 1–4b feel like a new syllabus after `cu.foundation/`**, read [row 51](../appendix/sources.md#ix3-epilogue-writings-canonical-reunion-index-row-51) — the handoff is foundation export discipline → orchestrated workflow on one specimen, not a separate multiscale textbook.

| IX.3 closing beat | Epilogue opening beat |
|-------------------|------------------------|
| [IX.3 epilogue pedigree table](#ix3-epilogue-pedigree-table) + `foundation_export.yaml` | [Opening hinge from Part IX](../epilogue/multiscale.md#opening-hinge-ix3-to-epilogue) maps exports onto the wire |
| [Quasiharmonic \(\alpha\) Lab act](#lab-act-quasiharmonic-alpha-handshake-3-pedigree) (Handshake 3 upstream) | [Handshake 3](../epilogue/multiscale.md#handshake-3--thermal-strain--mechanical-stiffness-part-vi--iv) load-cell reading at \(T_w\) |
| Handshake 4a/4b split on `hardening.yaml` | [Coupling hinge](../epilogue/multiscale.md#opening-hinge-ix3-to-epilogue) — 4a Act IV bulk, 4b Act V notch |
| [Bridge to the epilogue](#bridge-to-the-epilogue) | [Writings canonical landing](../epilogue/multiscale.md#writings-canonical-landing-ix3-to-epilogue) before Handshake sections |

Edit canonical prose under `writings/dft/` and `writings/epilogue/` only; run [`scripts/sync-writings.sh`](../../scripts/sync-writings.sh) before `mdbook build`. Row 51 does not replace [row 10](../appendix/sources.md#continuity-hinges-index-when-the-plot-stutters) (coupling hinge in reading time) or [row 43](../appendix/sources.md#rows17-42-row16-orchestration-reunion-index-row-43) (orchestration reunion) — it reunites **subtree boundaries** when DFT Notes and the epilogue each build as standalone mdBooks.
