...
Turn the page when every rung has trustworthy numbers in isolation but no one can explain how the load cell curve inherits from an SCF log — that is the signal the epilogue's coupling vocabulary is missing. The [IX.2 opening hinge from IX.1](02-kohn-sham.md#opening-hinge-ix1-to-ix2) and [preface row 38 skill checkpoint](../preface.md#skill-navigation-row-38) reunite the SCF implementation with BO/HK theorems when Murnaghan \(a_0\) exists but the fixed-point loop still feels like a new course; the [opening hinge from IX.2](#opening-hinge-ix2-to-ix3) and [preface row 39 skill checkpoint](../preface.md#skill-navigation-row-39) reunite this Bridge with the workflow archive when SCF is understood but `cu.foundation/` still feels like standalone coursework; the [opening hinge to Handshake 3](#opening-hinge-ix3-to-handshake3) and [preface row 40 skill checkpoint](../preface.md#skill-navigation-row-40) reunite the foundation folder with load-cell \(\alpha(T_w)\) when handbook defaults persist beside converged phonon exports.

When **this Bridge names the epilogue but Kohn–Sham SCF still feels like a new course** after row 37 restored BO/HK theorem vocabulary, read the [Born–Oppenheimer → Kohn–Sham reunion index](../appendix/sources.md#born-oppenheimer-kohn-sham-reunion-index-row-38) (row 38) at the [preface Born–Oppenheimer → Kohn–Sham thread](../preface.md#reading-rhythm) before re-reading [IX.2](02-kohn-sham.md#bridge). Row 38 reunites **theorem vocabulary with SCF implementation**; row 39 remains the audit when cutoff sweeps converged but `cu.scf.in` is still missing from `cu.foundation/`.

When **`cu.foundation/` is complete but Handshake 3 still cites handbook \(\alpha(300\,\text{K})\)** — `alpha_export.yaml` lists `target_temperature_K: 300` while [`fixtures/cht_export.yaml`](../../fixtures/cht_export.yaml) archives \(T_w = 311.48\,\text{K}\) — read the [IX.3 → Handshake 3 reunion index](../appendix/sources.md#ix3-handshake3-reunion-index-row-40) (row 40) and the [preface IX.3 → Handshake 3 thread](../preface.md#reading-rhythm) before opening [Handshake 3](../epilogue/multiscale.md#handshake-3--thermal-strain--mechanical-stiffness-part-vi--iv). Row 40 reunites **foundation archive with temperature pedigree at the load cell**; row 39 remains the audit when SCF converged but the calculation ladder never became a reproducible folder.
