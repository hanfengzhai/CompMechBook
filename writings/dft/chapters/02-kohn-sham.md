...
When **this Bridge names IX.3 but Born–Oppenheimer still feels like a new course** after row 36 restored SCF audit gates, read the [electronic audit → Born–Oppenheimer reunion index](../appendix/sources.md#electronic-audit-born-oppenheimer-reunion-index-row-37) (row 37) at the [preface electronic audit → Born–Oppenheimer thread](../preface.md#reading-rhythm) before re-reading [IX.1](01-born-oppenheimer.md#bridge). Row 37 reunites **SCF audit with theorem vocabulary**; row 36 remains the audit when SCF pedigree itself is still missing.

When **cutoff sweeps converged but IX.3 still feels like standalone coursework** — the SCF fixed-point loop is named but `cu.scf.in` and `README.md` are absent from `cu.foundation/` — read the [Kohn–Sham → DFT workflows reunion index](../appendix/sources.md#kohn-sham-dft-workflows-reunion-index-row-39) (row 39) and the [preface Kohn–Sham → DFT workflows thread](../preface.md#reading-rhythm) before opening [IX.3](03-dft-workflows.md). Row 39 reunites **SCF implementation with workflow archives**; row 38 remains the audit when Murnaghan \(a_0\) exists but \(\mathbf{H}[\rho]\mathbf{c}=\epsilon\mathbf{S}\mathbf{c}\) still feels disconnected from Part I eigenvalues.

When **`cu.foundation/` is complete but the epilogue load cell still cites handbook \(\alpha(300\,\text{K})\)** — continue to the [IX.3 → Handshake 3 reunion index](../appendix/sources.md#ix3-handshake3-reunion-index-row-40) (row 40) and the [preface IX.3 → Handshake 3 thread](../preface.md#reading-rhythm) at [IX.3's opening hinge to Handshake 3](03-dft-workflows.md#opening-hinge-ix3-to-handshake3) before opening the epilogue. Row 40 reunites **temperature pedigree at Handshake 3**; row 39 remains the audit when SCF converged but the foundation folder is still empty.
