...
[IX.1's Bridge](01-born-oppenheimer.md#bridge) named the signal to turn the page: **"DFT gave a number" but cutoff, k-sampling, and functional choice were never documented** — that is the signal the foundation run is not yet trustworthy enough to climb the ladder. The [IX.1 → IX.2 reunion index](../appendix/sources.md#ix1-ix2-opening-hinge-reunion-index-row-57) reunites this opening with [row 56](../preface.md#skill-navigation-row-56) when the [Murnaghan export manifest](01-born-oppenheimer.md#murnaghan-export-manifest-handoff-to-ix2) archived \(a_0\) and \(B_0\) but the SCF cycle inside each volume point still feels like a black box disconnected from Part I's generalized eigenvalue problem — same copper cell, same BO surface, now with **the fixed-point loop** that makes `cu.relax.out` reproducible. The [Born–Oppenheimer → Kohn–Sham reunion index](../appendix/sources.md#born-oppenheimer-kohn-sham-reunion-index-row-38) closes the same stitch at meta depth across all prior reunions.
