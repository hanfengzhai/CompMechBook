...
[Part III.4](../part03-pdes/04-energy-methods.md) closed the analytical pipeline with Dirichlet's principle: minimize \(\Pi[u]\) in \(H^1\). Part IV's first move is Rayleigh–Ritz on \(V_h \subset H^1\) — the same minimum, now on nodal coefficients — which is why assembly and energy methods are one story, not two tricks. When the weak form is clear but no global \(\mathbf{K}\) exists yet, you are at the right opening.
...
## Closing the arc from Part III {#opening-hinge-part-iii-to-iv}
...
The [preface ascent continuity hinges](../preface.md#ascent-continuity-hinges) name [III.4 → IV](../part03-pdes/04-energy-methods.md#bridge-to-part-iv) as the **weak PDEs → assembly** turn — where Dirichlet's minimum becomes a meshed solid the load cell can trust. Read this opening when the weak form is clear but no global matrix exists yet.
