...
## Intermission: Galerkin complete, conservation begins {#intermission-galerkin-complete-conservation-begins}

If you have read linearly since the [prologue](../prologue/00-many-scales.md), [Part I's grammar intermission](../part01-linear-algebra/04-toward-infinity.md#intermission-grammar-complete-function-spaces-begin), [Part II's function-space intermission](../part02-functional-analysis/05-spectral-theorem.md#intermission-function-spaces-complete-pdes-begin), and [Part III's assembly intermission](../part03-pdes/04-energy-methods.md#intermission-analysis-complete-assembly-begins), the copper wire has been solved on a mesh four times without changing specimen: springs in \(\mathbb{R}^N\), fields in \(H^1\), weak PDEs, and now Galerkin assembly with a **convergence certificate**. Part IV is the last chapter where one discretization philosophy — trial functions in \(H^1\), energy minimization, assembled \(\mathbf{K}\mathbf{U}=\mathbf{F}\) — carries the entire story.

What Part IV cannot resolve alone is **what happens outside the solid**. Current heats the wire; air carries enthalpy away by convection. Inside the copper, Céa's lemma certifies the thermal and elastic fields on the mesh you refined in the Lab act above. At the wire surface, however, the physics is **transport**: mass and momentum flux through a moving fluid, steep advection fronts, Reynolds-number-dependent boundary layers — problems where Galerkin trial functions oscillate or violate discrete conservation unless carefully stabilized. Part V names that second dialect: integral balance on control volumes, face fluxes, Riemann solvers, Navier–Stokes CFD, and the conjugate heat transfer loop that sets \(T_w \approx 379\,\text{K}\) for every downstream temperature export.

Part VI offers a parallel door for readers whose specimen is solid-dominated: name Cauchy stress and virtual work behind the stiffness matrix Part IV already converged, then reunite with Part V at the continuum midpoint. When patch tests pass and \(h\)-refinement plateaus but the wire still **heats in air you have not discretized**, return to this intermission — the [preface ascent continuity hinge](../preface.md#ascent-continuity-hinges) lists this chapter as the **Galerkin → transport discretization** turn, the fourth hinge in the mathematical climb. [Part V opening — Story so far](../part05-fvm/00-opening.md#story-so-far-parts-i-iv) picks up Door A with the conservation ladder; [V.1 — Closing the arc from Part IV](../part05-fvm/01-conservation-integral.md#conservation-opening-hinge-galerkin-to-flux-balance) is the first chapter where flux balance replaces stiffness assembly; [Part VI opening](../part06-continuum/00-opening.md#midpoint-ascent-complete-descent-ahead) picks up Door B with stress tensors both discretizations approximate.

The specimen on the bench has not moved — only the **organizing principle** forks. Act II's warming still means enthalpy leaving the surface; Act III's pulling still means a converged elastic mesh behind the load cell. What changes on the next page is whether you balance **fluxes across cell faces** in the surrounding air (Door A) or **name the stress tensor** behind the converged displacement field (Door B). Either path must reach Part VI before the descent to dislocations and atoms. Read the [concept map checkpoint](#concept-map-checkpoint-part-iv) above if you need a one-table summary — then continue to the [Bridge: two doors](#bridge-two-doors-from-here) below.
...
**Door A — Part V (conservation on cells).** Fluids at high Reynolds number, shocks, and steep advection fronts favor a different philosophy from Galerkin trial functions: integrate conservation laws over control volumes and balance **fluxes** across faces. The finite volume method is that story — complementary to FEM, not competing with it. When the copper wire heats in air, Part V discretizes the cooling flow; Part IV discretizes conduction in the solid; a fixed-point loop at the wall couples them (conjugate heat transfer). Read Part V next if fluids and CFD are your immediate goal. Continue the parallel narrative at [Part V's conservation ladder section](../part05-fvm/00-opening.md#the-conservation-ladder-fvm-parallel-to-schematic-14) — the transport twin to the Galerkin ladder Part IV just closed — and at [V.1's opening hinge](../part05-fvm/01-conservation-integral.md#conservation-opening-hinge-galerkin-to-flux-balance), where integral balance replaces stiffness assembly for the first time; [Part III weak forms](../part03-pdes/02-weak-form.md) remain the shared analytical foundation.
...
Turn the page when patch tests pass and Céa's rates hold on a bar but the grip displacement still moves when you halve \(h\) at the corner — that is the signal Door A or Door B must name the physics the converged mesh is approximating. If you paused after the Galerkin climb, reread the [Intermission: Galerkin complete, conservation begins](#intermission-galerkin-complete-conservation-begins) above before choosing a door — the solid mesh is certified; the air around it is not.
