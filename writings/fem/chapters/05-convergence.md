...
[IV.4](04-poisson-to-elasticity.md) extended Galerkin assembly from scalar temperature to vector displacement on the same mesh — Act II's Joule heating and Act III's tension now share one connectivity. This chapter connects Part I's discrete norms, Part II's function-space error analysis, and Part IV's implementation choices (\(h\), \(p\), element type) into a coherent refinement strategy, then names the **two exit doors** every reader must choose before Part VI: conservation on control volumes for the air that cools the wire (Part V), or continuum stress and strain for the solid-dominated load cell curve (Part VI).
...
Recall [IV.4's Lab act](04-poisson-to-elasticity.md): one mesh, two fields — thermal load from \(\alpha\Delta T\) and mechanical displacement from grip traction. Convergence in the energy norm must be checked on **both** passes before Act II's thermocouple or Act III's load cell inherit trustworthy numbers. A refined solid mesh that converges in displacement but ignores thermal eigenstrain is the same category error Part V later flags at the fluid–solid interface: each field can look smooth while the coupled story is wrong.
