...
| MD export (this chapter) | Audit gate | Consumer | Failure mode |
|--------------------------|------------|----------|--------------|
...
| Phonon peaks from VACF | Within 5–10% of DFT `dispersion.dat` | Part IX quasi-harmonic \(\alpha(T)\) | Bulk-fit EAM wrong at dislocation core |
...
| Stress–strain at 300 K NPT | Converged \(\Delta t\); equilibrated box | Part VII mobility \(M(\tau,T)\) fit | Transient thermostat exported as yield |
...
Return to the [prologue](../../prologue/00-many-scales.md): **Act II — Warming** needs NVT at 300–600 K before Joule heating couples to FVM; **Act III — Pulling** needs NPT tension with audited \(\Delta t\) before moduli climb to Part IV; **Act V — Notch** needs a converged shear run before stress concentration feeds the multiscale handoff. [VIII.1](01-potentials-phase-space.md) defined the ink; this chapter proved the ink flows at laboratory temperature; [VIII.3](03-ab-initio-and-coarse-graining.md) is the **export chapter** — where atomistics stops being a standalone movie and becomes input for coarser models, while naming what only [Part IX](../part09-dft/03-dft-workflows.md) can re-derive from \(\rho(\mathbf{r})\).

Turn the page when NVE drift is flat and NPT moduli match experiment in bulk but the EAM curve fails at the notch root — that is the signal to audit the potential against electronic structure. Read the [VIII.3 opening hinge](03-ab-initio-and-coarse-graining.md#opening-hinge-viii2-to-viii3) when integrator certificates pass but export folders still lack EAM-fit audits and mobility yaml tables.
