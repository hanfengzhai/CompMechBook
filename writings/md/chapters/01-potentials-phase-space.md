...
When **this Bridge names dynamics but VIII.2 still feels like standalone statistical mechanics** after row 33 restored mesoscale pedigree, read the [potentials → ensembles reunion index](../appendix/sources.md#potentials-ensembles-reunion-index-row-34) (row 34) at the [preface potentials → ensembles thread](../preface.md#reading-rhythm) before opening [VIII.3](03-ab-initio-and-coarse-graining.md). Row 35 names the **ensembles → coarse-graining** stitch when trajectories exist but handoff tables lack DFT pedigree gates.
