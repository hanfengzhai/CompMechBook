...
If you have read linearly since the prologue, [VIII.2](02-ensembles-integrators.md) proved the ink flows — NVE drift audits, NVT/NPT moduli, NEB barriers, and KMC rate tables at laboratory temperature. This chapter is the **export chapter**: where atomistic trajectories compress into EAM fits, mobility laws, and coarse-grained numbers Part VII consumes — while naming what only [Part IX](../part09-dft/00-opening.md) can re-derive from \(\rho(\mathbf{r})\).

The [preface descent continuity hinges](../preface.md#descent-continuity-hinges) name [VIII.3 → IX](03-ab-initio-and-coarse-graining.md#bridge-to-part-ix) as the **atomistic → electronic** turn; read this opening hinge when NPT moduli match experiment in bulk but the EAM curve fails at the notch root — the potential audit below is the prerequisite before Part IX opens Schrödinger's equation.

| Part VIII.2 (ensembles on the wire) | Part VIII.3 (coarse-graining on the wire) |
|-------------------------------------|-------------------------------------------|
| NVE/NVT/NPT integrator certificates | EAM-fit audit before Act V notch MD |
| \(D(T)\) from MSD; NEB \(\Delta E_m\) | Arrhenius rates exported to KMC and DDD climb |
| Phonon peaks from VACF | Cross-check against DFT `dispersion.dat` in IX.3 |
| Shear stress–strain at 300 K NPT | Trial \(M(\tau)\) yaml for OpenDiS |
| [VIII.2 Bridge](02-ensembles-integrators.md#bridge) previews export | Pedigree checklist before Part IX |

Part VIII.2's integrator loop is Part I's \(\mathbf{K}\mathbf{u}=\mathbf{f}\) at atomic scale — state vector, force update, repeated evaluation — now with ensemble averages replacing a single equilibrium solve. This chapter asks which averages are **safe to export upward** and which require Born–Oppenheimer audit before the epilogue's multiscale afternoon.

Turn the page when NVE drift is flat and bulk moduli match handbook values but no one can cite the DFT configurations that trained the EAM — that is the signal coarse-graining and electronic-structure audit are the next moves.
