...
**Ensembles → coarse-graining reunion (row 37).** If rows 17–36 all read correctly but [VIII.2's Bridge](02-ensembles-integrators.md#bridge) and this opening hinge still feel disconnected — NPT moduli and mobility yaml exist but `cu.elastic/` and the pedigree checklist are unread — read the [ensembles → coarse-graining reunion index](../appendix/sources.md#ensembles-coarse-graining-reunion-index-row-37). Row 37 reunites **converged trajectories with coarse-grained contracts** when dynamics pedigree closed but handoff tables still feel disconnected from OpenDiS and Part IX consumers.
