...
## Intermission: discretization complete, continuum vocabulary begins {#intermission-discretization-complete-continuum-begins}

If you have read linearly since the [prologue](../prologue/00-many-scales.md), [Part III's assembly intermission](../part03-pdes/04-energy-methods.md#intermission-analysis-complete-assembly-begins), and [Part IV's Galerkin intermission](../part04-fem/05-convergence.md#intermission-galerkin-complete-conservation-begins), the copper wire has been represented twice on the computer without yet receiving a unified mechanical vocabulary. Part IV meshed the solid and assembled \(\mathbf{K}\mathbf{U}=\mathbf{F}\); Part V balanced face fluxes in the air that cools the heated surface and closed a conjugate heat transfer loop at \(T_w \approx 379\,\text{K}\). Both parts solved PDEs — but neither named Cauchy stress, the deformation gradient, or the virtual work principle that makes a stiffness matrix a force-balance statement rather than a sparse array exercise.

Part VI is the last chapter of the **ascent** where the wire still looks smooth on the engineering scale. It supplies the tensor language both discretizations approximate: \(\mathbf{F}\) and \(\boldsymbol{\varepsilon}\) for the solid, rate of deformation \(\mathbf{D}\) for the fluid, Cauchy traction \(\boldsymbol{\sigma}\mathbf{n}\) on the interface the Picard loop already enforced. When FEM and FVM feel like competing software packages rather than adjacent dialects of one afternoon, return to this intermission — the [preface ascent continuity hinge](../preface.md#ascent-continuity-hinges) lists this chapter as the **discretization fork → continuum** turn; [Part VI opening](../part06-continuum/00-opening.md#midpoint-ascent-complete-descent-ahead) picks up the story at the mathematical midpoint; [VI.1 — Closing the arc from Parts IV and V](../part06-continuum/01-kinematics.md#continuum-opening-hinge-discretization-to-kinematics) is the first chapter where \(\mathbf{F}\) replaces nodal \(\mathbf{U}\) as the primary object.

The specimen on the bench has not moved — only the vocabulary has. Act II's warming still means enthalpy fluxes at the wire surface; Act III's pulling still waits for a stress tensor behind the load cell. What changes on the next page is **naming** the fields both codes already carry implicitly: displacement gradients behind \(\mathbf{K}\), viscous stress behind face fluxes, thermal strain \(\alpha\Delta T\) from the converged \(T_w\) in `cht_export.yaml`. If you need a one-paragraph reminder of the discretization arc before continuum mechanics, read [Part V opening — Story so far](../part05-fvm/00-opening.md#story-so-far-parts-i-iv) and then continue here — or skip to [VI.1's opening hinge](../part06-continuum/01-kinematics.md#continuum-opening-hinge-discretization-to-kinematics) when the CHT loop is converged and kinematics is the missing vocabulary.
...
## Bridge to Part VI {#bridge-to-part-vi}
...
Turn the page when sparse linear systems and face fluxes feel like the whole story — continuum mechanics is what makes \(\mathbf{K}\mathbf{U}=\mathbf{F}\) a force-balance statement rather than an array exercise. If the CHT loop converges but the load cell reading still lacks a stress tensor behind it, Part VI is the missing chapter, not another flux limiter. If you paused after the discretization climb, reread the [Intermission: discretization complete, continuum vocabulary begins](#intermission-discretization-complete-continuum-begins) above before opening [Part VI](../part06-continuum/00-opening.md#midpoint-ascent-complete-descent-ahead) — then continue to [VI.1's opening hinge](../part06-continuum/01-kinematics.md#continuum-opening-hinge-discretization-to-kinematics), where deformation gradient \(\mathbf{F}\) replaces nodal \(\mathbf{U}\) for the first time.
