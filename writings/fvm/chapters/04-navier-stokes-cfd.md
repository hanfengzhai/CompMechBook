...
### Writings canonical hinge (V.4 → VI.0) {#writings-canonical-hinge-v4-to-vi0}

The unified book ([`SUMMARY.md`](../SUMMARY.md)) and the vendored [`writings/fvm`](../../writings/fvm/chapters/SUMMARY.md) subtree both end on this Bridge; [`writings/continuum`](../../writings/continuum/chapters/SUMMARY.md) opens at [VI.0](../part06-continuum/00-opening.md#writings-canonical-landing-v4-to-vi0) with the same Functional Analysis Notes layout (`00-opening.md`, numbered chapters, **Bridge** sections) but upstream [Elasticity & Inelasticity Notes](https://hanfengzhai.github.io/file/elasticity_notes.pdf) instead of FVM Notes. When the layout feels continuous but **fluids and solids still feel like separate courses**, read [row 47](../appendix/sources.md#v4-vi0-writings-canonical-reunion-index-row-47) — the handoff reunites Galerkin and conservation on one specimen, not a new syllabus.
...
Picture the wire one last time at the end of Act II: the solid post-processor still shows nodal temperature from Part IV's Galerkin mesh, while the fluid monitor beside it reports cell-averaged enthalpy flux from Part V's control volumes. Two windows, one afternoon — and the Picard table is the conversation between them. Part VI is not a third solver; it is the **tensor vocabulary** both windows were approximating while you balanced fluxes and assembled stiffness. The load cell in Act III will ask for Cauchy stress, not another face Riemann solver; the twin ladders reunite in [VI.0's opening](../part06-continuum/00-opening.md#the-twin-ladders-reunite-galerkin-and-conservation) with the same Scene → Bridge contract this chapter just closed.
