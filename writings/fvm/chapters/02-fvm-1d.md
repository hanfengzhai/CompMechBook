...
Part I's eigenvalue picture reappears in the CFL limit: explicit stability is governed by the **spectrum of the flux Jacobian** at each face — discrete analogues of the operator eigenvalues Part II.5 will connect to spatial modes. Turn the page when first-order Sod's problem converges but the shock front is three cells thick — that is the signal numerical flux design, not finer \(\Delta x\) alone, must improve the physics at the face.
