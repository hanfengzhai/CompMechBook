...
## Lab act: Sod shock tube sanity check (Act II — Warming side channel) {#lab-act-sod-shock-tube-trust-gate}
...
The [epilogue Handshake 2](../epilogue/multiscale.md#handshake-2--joule-heating--conjugate-heat-transfer-part-iv--v) runs the same Picard loop at workflow scale — FEM solid conduction exchanging \(T_w\) and \(q''\) with FVM air. Sod is the **hyperbolic trust gate** that loop inherits: if the Riemann kernel fails mass conservation on a shock tube, no amount of wall-flux under-relaxation will fix the coupled run. Archive the Sod L¹ regression from [Lab act above](#lab-act-sod-shock-tube-trust-gate) beside the CHT fixed-point log — the same verification habit Part IV's patch test demands before Act II validation.
