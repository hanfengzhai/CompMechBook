...
Read the [mesoscale hinge](part07-defects/03-polycrystal-and-fem-handoff.md#bridge-to-part-viii) when mobility or \(\gamma_{\text{sf}}\) feel like fitted constants — Peierls stress and core width hide in atomic trajectories, not Taylor hardening alone. Read the [VII.3 intermission](part07-defects/03-polycrystal-and-fem-handoff.md#intermission-mesoscale-ends-atomistics-begin) when the OpenDiS → DAMASK pipeline is complete but core structure still feels deferred — the last mesoscale page before atoms enter. Read the [VII midpoint reunion](part08-md/00-opening.md#second-descent-rung-vii-midpoint-reunion) when mesoscale yaml and MD trajectories feel like separate manuals — Schematic 14's atomistic parallel reunites both on the same representative volume. Read the [atomistic hinge](part08-md/03-ab-initio-and-coarse-graining.md#bridge-to-part-ix) when EAM matches bulk moduli but no one cites the DFT input deck. Read the [VIII midpoint reunion](part09-dft/00-opening.md#third-descent-rung-viii-midpoint-reunion) when atomistic fits and SCF logs feel like separate manuals — Schematic 14's electronic parallel reunites both on the same foundation folder; the [prologue row 16 preview](prologue/00-many-scales.md#prologue-row-16-preview-part-ix-reunion) names that reunion before Part I. Read the [IX.3 intermission](part09-dft/03-dft-workflows.md#intermission-electronics-ends-coupling-begins) when the foundation archive is complete but no README connects `pw.x` logs to LAMMPS or OpenDiS inputs — the last electronic page before coupling. Read the [coupling hinge](part09-dft/03-dft-workflows.md#bridge-to-the-epilogue) when exports exist in separate folders but no workflow connects them — the epilogue's [ME 412 coupling ladder](epilogue/multiscale.md#the-coupling-ladder-me-412-cross-index) is where pedigree becomes handshake.
...
| Row 16 — Part IX reunion → coupling | When foundation folders are complete but no README connects exports to the load cell | [Row 16 skill checkpoint](#skill-navigation-row-16) |
...
**When to pause.** Read the [prologue row 13 preview](prologue/00-many-scales.md#prologue-row-13-preview) and the [reading compass row 13](prologue/00-many-scales.md#reading-compass-two-clocks-on-one-wire) when narrative time (Acts II–III) and mathematical order (Parts IV–V → IX → epilogue) diverge on the same afternoon. Return to [V.4's Picard loop](part05-fvm/04-navier-stokes-cfd.md#lab-act-extension-two-domain-picard-loop-with-a-1d-fem-solid) for Handshake 2 before opening the epilogue's [Handshake 3 worked load-cell example](epilogue/multiscale.md#handshake-3--thermal-strain--mechanical-stiffness-part-vi--iv). After archiving quasiharmonic \(\alpha\) in [IX.3's Lab act](part09-dft/03-dft-workflows.md#lab-act-quasiharmonic-alpha-handshake-3-pedigree), read [IX.3's Bridge Act III reunion paragraph](part09-dft/03-dft-workflows.md#bridge-act-iii-reunion-handshake-3) beside the epilogue's [Act III reunion paragraph](epilogue/multiscale.md#act-iii-reunion-handshake-3) — the Lab act is the upstream half in mathematical order; the bridge and reunion paragraphs close the same stitch in workflow time. Do not conflate Handshake 2 (\(\Delta T\)) with Handshake 3 (\(\alpha\Delta T\)) — the [preface epilogue hinge row](#epilogue-continuity-hinges) names the full chain in workflow order; the [Act II reunion paragraph](epilogue/multiscale.md#act-ii-reunion-handshake-2) states the Joule-heating stitch when Act II activates; the [sensitivity derivation worksheet closing](epilogue/multiscale.md#worked-example-sensitivity-ranks) links back to the [prologue row 13 preview](prologue/00-many-scales.md#prologue-row-13-preview) when the Handshake 3 competence loop closes.

### Row 14 skill checkpoint — DDD rate → lab load cell {#skill-navigation-row-14}

This checkpoint closes the **VII.2 forest → VII.3 rate handshake → Handshake 4a** chain — the epilogue-only stitch when OpenDiS exports a plausible hardening curve but the load cell yield arrives early because DDD ran at \(10^3\,\text{s}^{-1}\) and the lab grip runs at \(10^{-3}\,\text{s}^{-1}\). [VII.2's forest Lab act](part07-defects/02-dislocation-dynamics.md#lab-act-read-the-hardening-bend-from-forest-density-act-iv) and [VII.3's rate handshake](part07-defects/03-polycrystal-and-fem-handoff.md#scale-boundary-handshake-ddd-strain-rate-to-quasi-static-fem) are the **upstream halves**; [VII.3's Bridge Act IV reunion paragraph](part07-defects/03-polycrystal-and-fem-handoff.md#bridge-act-iv-reunion-handshake-4a) is the mathematical-order downstream mirror; [Handshake 4a](epilogue/multiscale.md#handshake-4--rate-dependent-hardening-and-notch-localization-part-vii--vi--viii) and the [six-act reunion Act IV paragraph](epilogue/multiscale.md#act-iv-reunion-handshake-4a) are the workflow-time downstream halves. It pairs [memory sheet row 14](appendix/memory-sheet.md#continuity-hinges-master-map) (narrative hinge) with a concrete artifact (competence hinge).
...
**When to pause.** Read the [prologue row 14 preview](prologue/00-many-scales.md#prologue-row-14-preview-handshake-4a) and the [reading compass row for Handshake 4a](prologue/00-many-scales.md#reading-compass-two-clocks-on-one-wire) when narrative time (Act IV) and mathematical order (Part VII → epilogue) diverge on the same afternoon. Return to [VII.2's forest Lab act](part07-defects/02-dislocation-dynamics.md#lab-act-read-the-hardening-bend-from-forest-density-act-iv) for \(\tau(\gamma)\) before opening the epilogue's [Handshake 4a worked example](epilogue/multiscale.md#handshake-4--rate-dependent-hardening-and-notch-localization-part-vii--vi--viii). After archiving rate extrapolation via [`parse_rate.sh`](scripts/parse_rate.sh), read [VII.3's Bridge Act IV reunion paragraph](part07-defects/03-polycrystal-and-fem-handoff.md#bridge-act-iv-reunion-handshake-4a) beside the epilogue's [Act IV reunion paragraph](epilogue/multiscale.md#act-iv-reunion-handshake-4a) — the rate handshake is the upstream half in mathematical order; the bridge and reunion paragraphs close the same stitch in workflow time. Do not conflate DDD timestep strain rate with lab grip speed — the [preface epilogue hinge row](#epilogue-continuity-hinges) names the full chain in workflow order; the [Handshake 4a opening paragraph](epilogue/multiscale.md#handshake-4a-opening) states the hardening stitch when Act IV activates; the [sensitivity derivation worksheet closing](epilogue/multiscale.md#worked-example-sensitivity-ranks) links back to the [prologue row 14 preview](prologue/00-many-scales.md#prologue-row-14-preview-handshake-4a) when the Handshake 4a competence loop closes. When Joule heating is active (Act II), evaluate mobility and \(m\) at the converged \(T_w\) from Handshake 2, not at 300 K by default. When Act V activates the optional notch, complete [row 15](#skill-navigation-row-15) after row 14 — bulk \(\tau_{\text{lab}}\) from 4a is necessary but not sufficient for notch-root localization.

### Row 15 skill checkpoint — FE² notch → Act V localization {#skill-navigation-row-15}

This checkpoint closes the **VII.3 Step 4 FE² → Handshake 4b** chain — the epilogue-only stitch when crystal plasticity with scalar hardening from 4a matches bulk flow stress but under-predicts peak von Mises stress at the notch root by 10–15%. [VII.3 Step 4](part07-defects/03-polycrystal-and-fem-handoff.md#step-4--when-offline-calibration-fails-fe-at-the-notch) is the **upstream half**; [VII.3's Bridge Act V reunion paragraph](part07-defects/03-polycrystal-and-fem-handoff.md#bridge-act-v-reunion-handshake-4b) is the mathematical-order downstream mirror; [Handshake 4b](epilogue/multiscale.md#4b--when-continuum-fails-at-the-notch-md-subdomain-act-v--notch) and the [Act V reunion paragraph](epilogue/multiscale.md#act-v-reunion-handshake-4b) are the workflow-time downstream halves. It pairs [memory sheet row 15](appendix/memory-sheet.md#continuity-hinges-master-map) (narrative hinge) with a concrete artifact (competence hinge). Upstream rate extrapolation must come from [row 14](#skill-navigation-row-14) first — FE² inherits the same `hardening.yaml` that 4a calibrated.
...
**When to pause.** Read the [prologue row 15 preview](prologue/00-many-scales.md#prologue-row-15-preview-handshake-4b) and the [reading compass row for Handshake 4b](prologue/00-many-scales.md#reading-compass-two-clocks-on-one-wire) when narrative time (Act V) and mathematical order (Part VII → epilogue) diverge on the same afternoon. Return to [VII.3 Step 4](part07-defects/03-polycrystal-and-fem-handoff.md#step-4--when-offline-calibration-fails-fe-at-the-notch) for the FE² procedure before opening the epilogue's [FE² worked example](epilogue/multiscale.md#worked-example-fe-at-the-wire-notch-act-v--notch). After archiving FE² comparison via [`parse_fe2.sh`](scripts/parse_fe2.sh), read [VII.3's Bridge Act V reunion paragraph](part07-defects/03-polycrystal-and-fem-handoff.md#bridge-act-v-reunion-handshake-4b) beside the epilogue's [Act V reunion paragraph](epilogue/multiscale.md#act-v-reunion-handshake-4b) — the FE² procedure is the upstream half in mathematical order; the bridge and reunion paragraphs close the same stitch in workflow time. Do not conflate bulk hardening adequacy with notch-root localization — scalar \(H\) from 4a can match the load-cell knee while missing pile-up physics at \(K_t \approx 3\). When FE² and crystal plasticity agree within 5%, offline calibration from [row 14](#skill-navigation-row-14) suffices and Act V does not require concurrent DDD. The [Handshake 4b opening paragraph](epilogue/multiscale.md#handshake-4b-opening) states the notch stitch when Act V activates; the [sensitivity derivation worksheet closing](epilogue/multiscale.md#worked-example-sensitivity-ranks) links back to the [prologue row 15 preview](prologue/00-many-scales.md#prologue-row-15-preview-handshake-4b) when the Handshake 4b competence loop closes.

### Row 16 skill checkpoint — Part IX reunion → coupling {#skill-navigation-row-16}

This checkpoint closes the **VIII midpoint reunion → IX.3 intermission → epilogue coupling ladder** chain — the stitch when foundation folders hold converged SCF logs but no README connects `pw.x` exports to LAMMPS, OpenDiS, and FEM inputs on the same wire. [Part IX's third descent rung](part09-dft/00-opening.md#third-descent-rung-viii-midpoint-reunion) reunites atomistic trajectories with electron density; [IX.3's intermission](part09-dft/03-dft-workflows.md#intermission-electronics-ends-coupling-begins) is the **last electronic page**; [IX.3's Bridge to epilogue](part09-dft/03-dft-workflows.md#bridge-to-the-epilogue) and the [epilogue ME 412 coupling ladder](epilogue/multiscale.md#the-coupling-ladder-me-412-cross-index) are the workflow-time downstream halves. It pairs [memory sheet row 10](appendix/memory-sheet.md#continuity-hinges-master-map) (narrative hinge) with a concrete artifact (competence hinge).

| Step | Skill on the copper wire | Minimal artifact |
|------|--------------------------|------------------|
| 1 — Foundation audit | Verify `cu.foundation/` holds converged SCF, elastic, and phonon exports with README | `cu.scf.in`, `cu.elastic/`, `README.md` per [IX.3 foundation table](part09-dft/03-dft-workflows.md#lab-act-foundation-archive-act-vi) |
| 2 — Reunion read | Name where atomistic and electronic exports meet on the same fcc cell | Completed [VIII midpoint reunion](part09-dft/00-opening.md#third-descent-rung-viii-midpoint-reunion) checklist |
| 3 — Coupling ladder | Wire Handshakes 1–4b in dependency order with units at every arrow | [`parse_multiscale_workflow.sh`](scripts/parse_multiscale_workflow.sh) on `fixtures/cu.foundation/` → `multiscale_export.yaml` |
| 4 — Epilogue Bridge | State the ME 412 one-line summary for Schematic 14's coupling branch | One sentence from [epilogue Bridge](epilogue/multiscale.md#bridge) matching Schematics 6, 7, 8b, 13b, 1a |

**When to pause.** Read the [prologue row 16 preview](prologue/00-many-scales.md#prologue-row-16-preview-part-ix-reunion) and the [reading compass row for Part IX reunion](prologue/00-many-scales.md#reading-compass-two-clocks-on-one-wire) when mathematical order (Part IX complete) and laboratory time (Act VI foundation running in parallel) diverge on the same afternoon. Return to [IX.3's intermission](part09-dft/03-dft-workflows.md#intermission-electronics-ends-coupling-begins) when SCF logs exist in separate folders with no workflow README. After archiving the foundation folder, read [IX.3's Bridge to epilogue](part09-dft/03-dft-workflows.md#bridge-to-the-epilogue) beside the [epilogue coupling ladder](epilogue/multiscale.md#the-coupling-ladder-me-412-cross-index) — the bridge names *what* to archive; the coupling ladder names *when* each act runs. Do not conflate *downward audit complete* with *workflow complete* — the [preface descent continuity hinge row](#descent-continuity-hinges) names the electronic → coupling turn; the [epilogue Bridge ME 412 one-line](epilogue/multiscale.md#bridge) closes the competence loop when pedigree becomes handshake. When Handshakes 2–4b are active, complete [rows 13–15](#skill-navigation-row-13) first — the multiscale orchestrator inherits their exports in dependency order.
