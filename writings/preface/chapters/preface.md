...
| Step | Skill on the copper wire | Minimal artifact |
|------|--------------------------|------------------|
| **1** — Rows 8–9 and 13–15 understood {#row-16-step-1} | Name Handshakes 2, 3, 4a, 4b and the \(T_w\) pedigree individually before orchestrating | Completed skill rows 8–9 and 13–15 or equivalent artifacts |
| **2** — Foundation folder {#row-16-step-2} | Archive DFT, phonon, DDD, and CHT inputs under `cu.foundation/` | `foundation_export.yaml` via [`parse_dft_workflow.sh`](scripts/parse_dft_workflow.sh) |
