...
When **Act II heating and Act III pulling** still feel like separate FEM courses after you open Part IV — even though this chapter already wrote coupled \(\Pi[u,T]\) — read the [Part IV thermoelastic assembly thread](../part04-fem/00-opening.md#acts-ii-and-iii-together-thermoelastic-assembly-thread) and the [Thermoelastic assembly reunion index](../appendix/sources.md#thermoelastic-assembly-reunion-index-row-29) (row 29) before running a mechanical pass without a stored \(\Delta T\) field. Row 29 reunites **laboratory time** with **IV.1–IV.5 chapter order**; row 28 remains the audit when handbook \(\alpha\) or mesh files lack export pedigree.
