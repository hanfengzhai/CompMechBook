...
## Writings canonical hinge (Part III → Part IV) {#writings-canonical-hinge-iii4-to-iv0}

This chapter is the last file in the **PDE Notes** subtree ([`writings/pde/`](../../writings/pde/)); the next page of the continuous book is the **FEM Notes** subtree ([`writings/fem/`](../../writings/fem/)), beginning at [Part IV opening](../part04-fem/00-opening.md). Part III ends at `04-energy-methods.md`; Part IV opens at `00-opening.md` with [**Writings canonical landing**](../part04-fem/00-opening.md#writings-canonical-landing-iii4-to-iv0) and [**Closing the arc from Part III**](../part04-fem/00-opening.md#closing-the-arc-from-part-iii) — the same four ME 412 questions (object, structure, theorem, what breaks), now answered with trial spaces \(V_h\), shape functions, and assembled \(\mathbf{K}\) instead of existence proofs alone.

**One continuous sentence (read aloud before Part IV):** the Rayleigh–Ritz Lab act above already produced \(K_{22} T_2 = F_2\) on two bar elements — Part IV does not introduce new physics; it systematizes **Galerkin assembly** on the mesh the thermocouple and load cell share, completing Schematic 14's convergence half after Lax–Milgram closed the existence half.

The unified mdBook [`src/SUMMARY.md`](../SUMMARY.md) syncs these canonical files — edit under `writings/<part>/chapters/`, run `./scripts/sync-writings.sh`, then `mdbook build`. If Part IV ever feels like a separate FEA software course after this chapter, run `./scripts/sync-writings.sh --check` and read the [III.4 → IV.0 PDE–FEM reunion (row 48)](../appendix/sources.md#rows17-47-iii4-iv0-pde-fem-reunion-index-row-48) — the break is usually skipping this hinge and the [Writings canonical landing](../part04-fem/00-opening.md#writings-canonical-landing-iii4-to-iv0), not missing finite element prerequisites.
...
The [prologue](../prologue/00-many-scales.md) promised **Act II — Warming** and **Act III — Pulling** on one afternoon — Part III wrote the coupled energy \(\Pi[u,T]\) those acts minimize; Part IV assigns node values and scatter loops. The [Writings canonical hinge](#writings-canonical-hinge-iii4-to-iv0) above names the file boundary: PDE Notes end here; [Finite Element Analysis Notes](https://hanfengzhai.github.io/file/FEA_notes.pdf) begin on the **same wire**, not a new syllabus.

Turn the page when the weak form is clear and the energy functional is written, but no global matrix exists yet — that is the signal Galerkin assembly is the next move. In the canonical tree, the next markdown file is [`writings/fem/chapters/00-opening.md`](../../writings/fem/chapters/00-opening.md) — read it as the next paragraph of Part III, not a course change. The same \(\mathbf{K}\mathbf{U}=\mathbf{F}\) from Part I, now built from shape functions, Jacobians, and the bilinear forms defined in Part III, awaits at [IV Writings canonical landing](../part04-fem/00-opening.md#writings-canonical-landing-iii4-to-iv0).
