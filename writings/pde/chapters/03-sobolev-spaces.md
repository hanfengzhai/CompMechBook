...
[III.2](02-weak-form.md#bridge) moved derivatives onto test functions and wrote the copper wire's equilibrium as \(a(u,v)=\ell(v)\) — but it deliberately deferred the contract question: *in which space does \(u\) live so that \(\nabla u\) in the bilinear form is meaningful when the field is only piecewise smooth?* Part II named \(H^1\) and \(L^2\) from the limit of mesh refinement; this chapter makes those names **computable**: weak derivatives, trace theorems for boundary data, Poincaré inequalities for coercivity, and the regularity gap between \(H^1\) membership and \(H^2\) rates.
