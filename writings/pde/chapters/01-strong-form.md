...
[Part III opening](00-opening.md#bridge) promised the weak form's first act: name the PDEs on the blackboard before integration by parts moves derivatives onto test functions. This chapter delivers that opening scene — **strong forms** that hold pointwise when classical smoothness is available, and fail honestly at the grip corner, thermocouple weld, and insulator interface where the copper wire's geometry and boundary data break the \(C^2\) contract.
...
The [preface ascent continuity hinge](../preface.md#ascent-continuity-hinges) names Part III as the **analysis → PDEs** turn — III.1 is the blackboard physics that turn begins with; [III.2](02-weak-form.md) is where the prologue's recurring weak form speaks its first full sentences. **Acts II–III** in the lab both start here: Joule heating and grip displacement as elliptic PDEs before FEM or FVM assign node values or cell fluxes.
