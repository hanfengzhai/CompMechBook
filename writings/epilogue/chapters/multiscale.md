...
|------|----------------------------------------------------------------------------------|---------------------------------------------------------|-----------------------------------------------------------------------------------|
| 1 | One sentence: "CHT sets \(T_w\) once; every finer rung inherits \(M(\tau, T_w)\), not 300 K" | Step 1 — Handshake 2 | `cht_export.yaml` via [`parse_cht.sh`](../../scripts/parse_cht.sh); converged \(T_w \approx 379\,\text{K}\), \(\Delta T = T_w - T_\infty\) |
| 2 | [VIII.0 descent hinge](../part08-md/00-opening.md#descent-hinge-cores-mobility-and-tw-pedigree) states the contract | Step 2 — NVT shear at \(T_w\) | `mobility_cu_screw_{T_w}K.yaml` beside `cht_export.yaml` — [VII.2 Lab act](../part07-defects/02-dislocation-dynamics.md#lab-act-calibrate-screw-mobility-from-md-shear-act-iv--mobility-prelude) |
...
| 4 | [Memory sheet rows 8–9 baby picture](../appendix/memory-sheet.md#rows-8-9-baby-picture-tw-temperature-pedigree) compresses the chain | Step 4 — OpenDiS pedigree | OpenDiS input cites `mobility_cu_screw_{T_w}K.yaml`; cross-slip counts at \(T_w\) archived beside WHAM export |

When MD mobility folders cite 300 K after Act II warmed the wire, start at [V.4's Picard loop](../part05-fvm/04-navier-stokes-cfd.md#lab-act-extension-two-domain-picard-loop-with-a-1d-fem-solid) — not at Part VIII trajectories — and archive `cht_export.yaml` before opening any NVT shear deck. The [memory sheet rows 8–9 baby picture](../appendix/memory-sheet.md#rows-8-9-baby-picture-tw-temperature-pedigree) compresses the chain; the [VIII.0 descent hinge](../part08-md/00-opening.md#descent-hinge-cores-mobility-and-tw-pedigree) is the downstream half this loop expands. Do not conflate handbook 300 K drag with Joule-heated \(T_w\) — phonon softening shifts Act IV hardening before Part IX audits the phonon curve. Proceed to [row 9](../preface.md#skill-navigation-row-9) when \(T_w\) is archived and finer-scale folders inherit the same temperature column.

### Row 9 closing loop (phonon audit reunion) {#row-9-closing-loop}

This subsection is the **downstream half** of [memory sheet row 9](../appendix/memory-sheet.md#continuity-hinges-master-map), the [preface descent continuity hinges row 9](../preface.md#descent-continuity-hinges), and the [preface row 9 skill checkpoint](../preface.md#skill-navigation-row-9). The [prologue row 9 preview](../prologue/00-many-scales.md#prologue-preview-row-9) and [prologue row 9 closing stitch](../prologue/00-many-scales.md#row-9-closing-stitch) are the **upstream halves** — return there when EAM matches bulk moduli but `alpha_export.yaml` lacks \(\alpha(T_w)\) beside SCF logs. Row 9 closes the phonon audit loop that [row 8](../preface.md#skill-navigation-row-8) opened at \(T_w\) pedigree: [row 8 closing stitch](../prologue/00-many-scales.md#row-8-closing-stitch) sets converged wall temperature; row 9 audits \(\alpha(T_w)\) and \(\tau_{\text{ph}}(T_w)\) at the electronic scale before [row 10](../preface.md#skill-navigation-row-10) composes foundation exports into Handshakes 1–4b.
