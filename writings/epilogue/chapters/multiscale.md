...
**Scale-boundary handshake (epilogue → next project).**

| Book rung | Foundation export | Wire-scale consumer | Audit before trusting |
|-----------|-------------------|---------------------|------------------------|
| Part IX DFT | `cu.scf.in`, converged \(C_{ij}\), \(\gamma_{\text{sf}}\) | Part VIII EAM fit; Part VII Peierls | Cutoff doubling \(< 1\) meV/atom |
| Part VIII MD | `Cu.eam.alloy`, mobility yaml, NEB barriers | Part VII OpenDiS; Part VI \(M(T)\) | NVE drift; VACF vs DFT phonons |
| Part VII DDD | \(\tau(\gamma)\), \(\rho(\gamma)\), link statistics | Part VI \(H\), \(\sigma_{y0}\); DAMASK yaml | Taylor fit stable to \(\pm 5\%\) |
| Part VI continuum | \(\mathbb{C}\), return-mapping, Perzyna \(m\) | Part IV nonlinear \(\mathbf{K}_T\); load cell curve | Mesh converged but hardening slope wrong → descend |
| Parts IV–V FEM/FVM | \(\mathbf{K}\), face fluxes, CHT Picard log | Acts II–III load cell + thermocouple | Three-mesh \(h\)-study; flux match at wall |
| Parts I–III analysis | Weak form, \(H^1\) norms, energy minimum | Every Galerkin assembly | Céa's rates; Lax–Milgram coercivity |

Each row is the epilogue's **Handshake discipline** in table form: no macro parameter climbs the ladder without a convergence log or archived export at the rung below. When a new material replaces copper, the specimen changes; the handshake table does not.
...
Turn the page back to the [prologue](../prologue/00-many-scales.md) when the multiscale afternoon is documented but the next specimen still feels like a fresh bag of tricks — that is the signal the four questions, not a new code, are what the project needs first.
