...
| 13b | Open Mapping, Bounded Inverse, Banach–Steinhaus | Unit conversion and frame consistency are **uniform bounds** at every arrow — one bad conversion poisons the ladder | [Handshake mechanics](#handshake-mechanics-what-crosses-interfaces); [Handshake interface checklist](../appendix/memory-sheet.md#handshake-interface-checklist) |
...
The [prologue row 13 preview](../prologue/00-many-scales.md#prologue-row-13-preview) names this stitch before Part I assembles a matrix — return there when the competence loop closes. {#handshake-2-opening}
...
The [prologue row 14 preview](../prologue/00-many-scales.md#prologue-row-14-preview-handshake-4a) names this stitch before Part I assembles a matrix — return there when the competence loop closes. {#handshake-4a-opening}
...
The [prologue row 15 preview](../prologue/00-many-scales.md#prologue-row-15-preview-handshake-4b) names this stitch before Part I assembles a matrix — return there when the competence loop closes. {#handshake-4b-opening}

This section is the **downstream half** of [VII.3 Step 4 — FE² at the notch](../part07-defects/03-polycrystal-and-fem-handoff.md#step-4--when-offline-calibration-fails-fe-at-the-notch): sequential homogenization with scalar \(H\) from 4a may match bulk flow stress while under-predicting notch-root localization by 10–15%. The [prologue reading compass row for Handshake 4b](../prologue/00-many-scales.md#reading-compass-two-clocks-on-one-wire) names this stitch when Act V and mathematical order diverge; the [preface row 15 skill checkpoint](../preface.md#skill-navigation-row-15) lists all four notch competence steps; the [sensitivity derivation worksheet](#worked-example-sensitivity-ranks) (Handshake 4b column) ranks when FE² matters versus when [preface row 14](../preface.md#skill-navigation-row-14) offline calibration suffices. Upstream bulk \(\tau_{\text{lab}}\) must come from Handshake 4a before the notch analysis begins.

If the wire has a micro-notch (Act V), continuum FEM gives stress concentration \(K_t \approx 3\) at the root. Peak stress \(\sim 20\,\text{MPa}\) still looks elastic — but **gradient** of stress over atomic spacing matters for nucleation. A concurrent MD/FEM domain hands atomistic resolution within 2 nm of the notch tip while FEM carries the bulk field (Part VIII, [VIII.3](../part08-md/03-ab-initio-and-coarse-graining.md)). The [FE² worked example](#worked-example-fe-at-the-wire-notch-act-v--notch) quantifies the uplift when scalar \(H\) under-predicts localization; the [sensitivity derivation worksheet](#worked-example-sensitivity-ranks) (Handshake 4b column) ranks when FE² matters versus when offline calibration suffices.
...
The table above is not intuition — it follows from the same formulas Handshakes 1–3 already used. For the **Joule heat → fixed-grip stress** chain (Handshake 2 → [IX.3 \(\alpha\)](../part09-dft/03-dft-workflows.md#lab-act-quasiharmonic-alpha-handshake-3-pedigree) → Handshake 3), [memory sheet row 13](../appendix/memory-sheet.md#continuity-hinges-master-map) is the one-page navigation aid — this worksheet is the quantitative proof behind that row. The [prologue row 13 preview](../prologue/00-many-scales.md#prologue-row-13-preview) named Handshakes 2 and 3 before Part I — read the Handshake 2 paragraph below when Act II activates, then the Handshake 3 paragraph when Act III ramps load on fixed grips. The [prologue row 14 preview](../prologue/00-many-scales.md#prologue-row-14-preview-handshake-4a) named rate extrapolation before Part I — read the Handshake 4a paragraph below when Act IV bends the curve. The [prologue row 15 preview](../prologue/00-many-scales.md#prologue-row-15-preview-handshake-4b) named notch localization before Part I — read the Handshake 4b paragraph below when Act V activates. Take the converged mid-span wall temperature \(T_w \approx 390\,\text{K}\) with \(T_\infty = 300\,\text{K}\), so \(\Delta T = T_w - T_\infty \approx 90\,\text{K}\).

**Handshake 2 — convection coefficient \(h\) (from [`parse_cht.sh`](../scripts/parse_cht.sh)).** This paragraph is the quantitative audit behind the [Act II warming skill row](#what-you-should-be-able-to-do-after-the-book), [preface row 13 step 1](../preface.md#skill-navigation-row-13), and [V.4's Picard loop](../part05-fvm/04-navier-stokes-cfd.md#lab-act-extension-two-domain-picard-loop-with-a-1d-fem-solid) — downstream Handshake 3 inherits \(\Delta T\) only after this loop converges. At the converged fixed point, integrated Joule source balances surface convection: \(P \approx h A \Delta T\). Differentiate:
...
This worksheet is the multiscale analogue of Part IV's mesh refinement log: before trusting the load-cell answer, show which partial derivative controlled it. Return to the [Handshake 3 worked load-cell example](#handshake-3--thermal-strain--mechanical-stiffness-part-vi--iv) above to verify \(\sigma_{\text{th}}\) and the \(\alpha\)-sensitivity table with the same \(\Delta T = 90\,\text{K}\) baseline — upstream numbers come from [IX.3's quasiharmonic \(\alpha\) Lab act](../part09-dft/03-dft-workflows.md#lab-act-quasiharmonic-alpha-handshake-3-pedigree) and [`parse_alpha.sh`](../scripts/parse_alpha.sh); the [preface row 13 skill checkpoint](../preface.md#skill-navigation-row-13) lists all four thermal steps in competence order; the [prologue row 13 preview](../prologue/00-many-scales.md#prologue-row-13-preview) named that stitch before Part I — return there when this worksheet closes the Handshake 3 competence loop; the [Act II reunion paragraph](#act-ii-reunion-handshake-2) (Handshake 2) and [Act III reunion paragraph](#act-iii-reunion-handshake-3) (Handshake 3), [IX.3's Bridge Act III reunion paragraph](../part09-dft/03-dft-workflows.md#bridge-act-iii-reunion-handshake-3), and [memory sheet row 13 baby picture](../appendix/memory-sheet.md#continuity-hinges-master-map) state the same stitch in workflow time. Return to the [Handshake 4a worked example](#handshake-4--rate-dependent-hardening-and-notch-localization-part-vii--vi--viii) to verify \(\tau_{\text{lab}}\) — upstream OpenDiS sweeps come from [VII.2's forest Lab act](../part07-defects/02-dislocation-dynamics.md#lab-act-read-the-hardening-bend-from-forest-density-act-iv), [VII.3's rate handshake](../part07-defects/03-polycrystal-and-fem-handoff.md#scale-boundary-handshake-ddd-strain-rate-to-quasi-static-fem), and [`parse_rate.sh`](../scripts/parse_rate.sh); the [preface row 14 skill checkpoint](../preface.md#skill-navigation-row-14) lists all four hardening steps in competence order; the [prologue row 14 preview](../prologue/00-many-scales.md#prologue-row-14-preview-handshake-4a) named rate extrapolation before Part I — return there when this worksheet closes the Handshake 4a competence loop; the [Act IV reunion paragraph](#act-iv-reunion-handshake-4a), [VII.3's Bridge Act IV reunion paragraph](../part07-defects/03-polycrystal-and-fem-handoff.md#bridge-act-iv-reunion-handshake-4a), and [memory sheet row 14 baby picture](../appendix/memory-sheet.md#continuity-hinges-master-map) state the same hardening stitch in workflow time. Return to the [FE² worked example](#worked-example-fe-at-the-wire-notch-act-v--notch) to verify notch-root uplift — upstream rate extrapolation comes from [preface row 14](../preface.md#skill-navigation-row-14) and Handshake 4a; the [preface row 15 skill checkpoint](../preface.md#skill-navigation-row-15) lists all four notch steps in competence order. The [prologue row 15 preview](../prologue/00-many-scales.md#prologue-row-15-preview-handshake-4b) named Act V localization before Part I — return there when this worksheet closes the Handshake 4b competence loop; the [Handshake 4b opening paragraph](#handshake-4b-opening), [Act V reunion paragraph](#act-v-reunion-handshake-4b), [VII.3's Bridge Act V reunion paragraph](../part07-defects/03-polycrystal-and-fem-handoff.md#bridge-act-v-reunion-handshake-4b), and [memory sheet row 15 baby picture](../appendix/memory-sheet.md#continuity-hinges-master-map) state the same stitch in one breath.
...
Act II in the table is the **Joule-heating stitch** the [prologue row 13 preview](../prologue/00-many-scales.md#prologue-row-13-preview) names before Part I assembles a matrix — current may warm the wire to \(T_w \approx 380\,\text{K}\) while the FEM deck still assumes room-temperature conductivity until the Picard loop converges. [V.4's Picard loop](../part05-fvm/04-navier-stokes-cfd.md#lab-act-extension-two-domain-picard-loop-with-a-1d-fem-solid), [Handshake 2](#handshake-2--joule-heating--conjugate-heat-transfer-part-iv--v), and the [Handshake 2 opening paragraph](#handshake-2-opening) are the upstream workflows; [preface row 13 step 1](../preface.md#skill-navigation-row-13) closes the competence loop when Act II activates; the [sensitivity derivation worksheet](#worked-example-sensitivity-ranks) (Handshake 2 paragraph) proves why \(h\) and \(\Delta T\) dominate mid-span temperature before \(\alpha\) enters in Act III — do not conflate Handshake 2 (\(\Delta T\)) with Handshake 3 (\(\alpha\Delta T\)). {#act-ii-reunion-handshake-2}

Act III in the table is the **thermal pre-stress stitch** the [prologue row 13 preview](../prologue/00-many-scales.md#prologue-row-13-preview) names before Part I assembles a matrix — CHT may converge at \(T_w \approx 380\,\text{K}\) while the load cell still cites handbook \(\alpha\) and reads thermal compression on fixed grips. [V.4's Picard loop](../part05-fvm/04-navier-stokes-cfd.md#lab-act-extension-two-domain-picard-loop-with-a-1d-fem-solid) and [IX.3's quasiharmonic \(\alpha\) Lab act](../part09-dft/03-dft-workflows.md#lab-act-quasiharmonic-alpha-handshake-3-pedigree) are the upstream workflows; [IX.3's Bridge Act III reunion paragraph](../part09-dft/03-dft-workflows.md#bridge-act-iii-reunion-handshake-3) is the downstream pedigree mirror of this reunion in mathematical order; [memory sheet row 13](../appendix/memory-sheet.md#continuity-hinges-master-map) and [preface row 13](../preface.md#skill-navigation-row-13) close the competence loop when Act III activates; the [sensitivity derivation worksheet](#worked-example-sensitivity-ranks) (Handshake 2–3 chain) proves why \(\alpha\) dominates fixed-grip stress once \(\Delta T\) is set — do not conflate Handshake 2 (\(\Delta T\)) with Handshake 3 (\(\alpha\Delta T\)). {#act-iii-reunion-handshake-3}

Act IV in the table is the **hardening stitch** the [prologue Handshake 4a preview row](../prologue/00-many-scales.md#prologue-row-14-preview-handshake-4a) names before Part I assembles a matrix — OpenDiS at \(10^3\,\text{s}^{-1}\) cannot feed a \(10^{-3}\,\text{s}^{-1}\) load cell without power-law extrapolation. [VII.2's forest Lab act](../part07-defects/02-dislocation-dynamics.md#lab-act-read-the-hardening-bend-from-forest-density-act-iv) and [VII.3's rate handshake](../part07-defects/03-polycrystal-and-fem-handoff.md#scale-boundary-handshake-ddd-strain-rate-to-quasi-static-fem) are the upstream workflows; [VII.3's Bridge Act IV reunion paragraph](../part07-defects/03-polycrystal-and-fem-handoff.md#bridge-act-iv-reunion-handshake-4a) is the downstream pedigree mirror of this reunion in mathematical order; [memory sheet row 14](../appendix/memory-sheet.md#continuity-hinges-master-map) (baby picture: **VII.3 sets \(\tau_{\text{flow}}(\dot\varepsilon_{\text{DDD}})\); Handshake 4a sets \(\tau_{\text{lab}}\)** — conflating DDD timestep strain rate with lab grip speed overpredicts yield by the same order as a handbook \(\alpha\) error shifts thermal stress) and [preface row 14](../preface.md#skill-navigation-row-14) close the competence loop when Act IV activates; the [sensitivity derivation worksheet](#worked-example-sensitivity-ranks) (Handshake 4a column) proves why direct import overpredicts yield. {#act-iv-reunion-handshake-4a}

Act V in the table is the **notch stitch** the [prologue row 15 preview](../prologue/00-many-scales.md#prologue-row-15-preview-handshake-4b) names before Part I assembles a matrix — scalar hardening from Handshake 4a may match bulk flow stress while under-predicting root localization by 10–15%. [VII.3 Step 4](../part07-defects/03-polycrystal-and-fem-handoff.md#step-4--when-offline-calibration-fails-fe-at-the-notch) is the upstream workflow; [VII.3's Bridge Act V reunion paragraph](../part07-defects/03-polycrystal-and-fem-handoff.md#bridge-act-v-reunion-handshake-4b) is the downstream pedigree mirror of this reunion in mathematical order; [Handshake 4b opening paragraph](#handshake-4b-opening) states the notch stitch when Act V activates; [memory sheet row 15](../appendix/memory-sheet.md#continuity-hinges-master-map) (baby picture: **4a sets bulk \(\tau_{\text{lab}}\); 4b asks whether scalar \(H\) suffices at the notch root** — sequential homogenization can match bulk flow stress while under-predicting localization by 10–15%) and [preface row 15](../preface.md#skill-navigation-row-15) close the competence loop when Act V activates; the [sensitivity derivation worksheet](#worked-example-sensitivity-ranks) (Handshake 4b column) proves why FE² matters at \(K_t \approx 3\). {#act-v-reunion-handshake-4b}
...
| Act II warming (Handshake 2) | Run FEM solid ↔ FVM fluid until wall flux matches Joule source | [`parse_cht.sh`](../scripts/parse_cht.sh) → `cht_export.yaml`; verify against [V.4 Picard loop](../part05-fvm/04-navier-stokes-cfd.md#lab-act-extension-two-domain-picard-loop-with-a-1d-fem-solid), [preface row 13 step 1](../preface.md#skill-navigation-row-13), [sensitivity derivation](#worked-example-sensitivity-ranks) (Handshake 2 paragraph), and [Act II reunion paragraph](#act-ii-reunion-handshake-2) |
| Act III thermal (Handshake 3) | Export \(\alpha\) after CHT; verify fixed-grip stress dominates load cell | [`parse_alpha.sh`](../scripts/parse_alpha.sh) + [sensitivity derivation](#worked-example-sensitivity-ranks) (Handshake 3 paragraph); [preface row 13](../preface.md#skill-navigation-row-13); [Act III reunion paragraph](#act-iii-reunion-handshake-3); [memory sheet row 13](../appendix/memory-sheet.md#continuity-hinges-master-map) |
...
| Act V notch (Handshake 4b) | Compare FE² root stress to crystal plasticity; decide if offline calibration suffices | [`parse_fe2.sh`](../scripts/parse_fe2.sh) on `fe2_notch_comparison.dat` → `fe2_export.yaml`; verify against [FE² worked example](#worked-example-fe-at-the-wire-notch-act-v--notch), [preface row 15](../preface.md#skill-navigation-row-15), [Act V reunion paragraph](#act-v-reunion-handshake-4b), and [sensitivity derivation](#worked-example-sensitivity-ranks) (Handshake 4b column); upstream rate from [preface row 14](../preface.md#skill-navigation-row-14) |
...
None of these require a national allocation — but each one separates a reproducible multiscale study from a pile of incompatible input decks. If you can trace one modulus to an archived SCF log, run a conjugate heat loop until fluxes agree ([Act II warming skill row](#what-you-should-be-able-to-do-after-the-book), [`parse_cht.sh`](../scripts/parse_cht.sh)), complete the [row 13 skill checkpoint](../preface.md#skill-navigation-row-13) when handbook \(\alpha\) persists after CHT converges, complete the [row 14 skill checkpoint](../preface.md#skill-navigation-row-14) when DDD exports feed the plasticity deck without rate extrapolation, complete the [row 15 skill checkpoint](../preface.md#skill-navigation-row-15) when the optional notch under-predicts root stress — the same competence gap the [prologue row 15 preview](../prologue/00-many-scales.md#prologue-row-15-preview-handshake-4b) named before Part I: scalar \(H\) from 4a matches bulk flow stress but FE² at the root may add 10–15% uplift that crystal plasticity alone misses — complete the [row 16 skill checkpoint](../preface.md#skill-navigation-row-16) when foundation folders are complete but no `multiscale_export.yaml` links exports to the load cell — the same competence gap the [prologue row 16 preview](../prologue/00-many-scales.md#prologue-row-16-preview-part-ix-reunion) named before Part I: SCF archive complete but workflow incomplete — and say which act of the lab session a new simulation belongs to, you have closed the loop the prologue opened. Return to the [prologue four questions](../prologue/00-many-scales.md#the-same-questions-at-every-scale) on the next material; the specimen changes, the habit does not.
