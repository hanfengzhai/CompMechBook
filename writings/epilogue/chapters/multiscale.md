...
## Conjugate heat transfer recap (Parts IV–V)

Before the electron-scale handshakes below, recall the **engineering-scale coupling** the book already practiced. [Part IV](../part04-fem/00-opening.md) assembled steady conduction in the copper wire — \(-k\nabla^2 T = q\) with Joule source \(q\) — on an \(H^1\)-conforming solid mesh whose nodal temperatures are the Act II thermocouple readings in embryo. [Part V](../part05-fvm/00-opening.md) balanced fluxes on control volumes around the same specimen: convection at the surface, conservation of enthalpy in the cooling air, CFL limits when the current switches on.

**Conjugate heat transfer (CHT)** is the fixed-point loop that stitches those discretizations at the wall:

| Part | Discretization | Wall quantity exported | Wall quantity imported |
|------|----------------|------------------------|------------------------|
| IV (FEM solid) | Nodal \(T_h \in H^1\) on wire mesh | Surface temperature \(T_w\) from boundary nodes | Heat flux \(q_w = -k\,\partial T/\partial n\) from Neumann/Robin face |
| V (FVM fluid) | Cell-average fields on air mesh | Convection coefficient \(h(T_w)\) or resolved \(T_\infty, \mathbf{v}\) | Wall flux \(q_w'' = h(T_w - T_\infty)\) or coupled Robin data |

The iteration is always the same habit, whether the fluid mesh is a boundary-layer strip or a full Navier–Stokes domain:

1. Guess interface state (typically \(T_w\) or \(q_w\)).
2. Solve the solid conduction problem with the current fluid boundary data.
3. Read updated \(T_w\) and flux from surface nodes or face quadrature.
4. Solve (or update) the fluid problem with the new wall temperature.
5. Repeat until \(|T_w^{(k+1)} - T_w^{(k)}|\) and integrated flux balance fall below tolerance.

The [V.4 Lab act](../part05-fvm/04-navier-stokes-cfd.md) and Handshake 2 below instantiate this loop on the prologue wire. **`parse_cht.sh`** archives the converged \(T_w\), flux residual, and iteration count into `cht_export.yaml` — the same **pedigree discipline** Handshakes 1 and 3–4 require for moduli and mobility. DFT does not replace CHT; it supplies \(\alpha(T)\) and moduli that change what the solid block exports once \(T_w\) converges. When the multiscale afternoon scene lists four terminals, the CHT outer loop is often still running in the background from Act II — archived flux balance before trusting any downstream number.
