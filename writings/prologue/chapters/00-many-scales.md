...
**Baby picture:** 4a sets bulk \(\tau_{\text{lab}}\); 4b asks whether scalar \(H\) suffices at the notch root — sequential homogenization can match bulk flow stress while under-predicting localization by 10–15%. The [epilogue Act V reunion paragraph](../epilogue/multiscale.md#act-v-reunion-handshake-4b), [FE² worked example](../epilogue/multiscale.md#worked-example-fe-at-the-wire-notch-act-v--notch), [sensitivity derivation worksheet closing](../epilogue/multiscale.md#worked-example-sensitivity-ranks) (Handshake 4b paragraph when Act V activates), [preface row 15 skill checkpoint](../preface.md#skill-navigation-row-15), and [memory sheet row 15](../appendix/memory-sheet.md#continuity-hinges-master-map) close this loop in workflow time when the optional notch activates — return here when the [sensitivity derivation worksheet closing](../epilogue/multiscale.md#worked-example-sensitivity-ranks) links back to this preview and the Handshake 4b competence loop closes. Upstream rate extrapolation must come from [row 14 preview](#prologue-row-14-preview-handshake-4a) first — FE² inherits the same `hardening.yaml` that Handshake 4a calibrated.
