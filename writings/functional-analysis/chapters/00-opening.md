...
Part I's [ME 300A → ME 412 master roadmap preview](../part01-linear-algebra/00-opening.md#me-300a--me-412-master-roadmap-preview) drew **Schematics 1–4** on the left of the diagram — vectors, maps, eigenmodes, the \(N \to \infty\) limit — and pointed to **Schematics 1a–14** on the right. You are now at that junction: the spring chain from [I.4](../part01-linear-algebra/04-toward-infinity.md) becomes \(u \in H^1\), the stiffness matrix becomes an operator, and the concept map table above is ME 412's replay of the same four questions Part I asked in finite dimensions. When Schematic 14 (variational ladder) or the full-book coupling ladder feel distant, return to Part I's roadmap: the grammar is already in hand — only the room grows.
