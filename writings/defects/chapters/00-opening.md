...
Part VII inherits that thermal history. The \(T_w \approx 379\,\text{K}\) from conjugate heat transfer softens dislocation mobility before the first plastic increment — DDD segment rules must read \(M(\tau, T_w)\), not \(M(\tau, 300\,\text{K})\). When Peach–Köhler forces feel disconnected from the FEM stress field, return to [continuity hinge row 4](../appendix/sources.md#continuity-hinges-index-when-the-plot-stutters) (twin ladders reunite at Part VI) and [row 5–7](../appendix/sources.md#continuity-hinges-index-when-the-plot-stutters) (midpoint → descent → defect taxonomy). The [VI.4→VII.1 opening hinge](01-defect-taxonomy.md#defect-taxonomy-opening-hinge-plasticity-to-forest) is the first chapter-level turn where J₂ placeholders become Burgers circuits — read it when Part VII opening feels like a scale change without a new primary object. The [epilogue Handshake 3](../epilogue/multiscale.md#handshake-3--thermal-strain--mechanical-stiffness-part-vi--iv) is the workflow-order export of the same thermal strain Part VI wrote into balance laws — Act III's fixed-grip load cell reads that pre-stress before Part VII's forest bends the curve in Act IV. Part VIII's [descent hinge](../part08-md/00-opening.md#descent-hinge-cores-mobility-and-tw-pedigree) is where \(M(\tau, T_w)\) is **measured** on atomistic cores before the [VII.3 rate handshake](03-polycrystal-and-fem-handoff.md#scale-boundary-handshake-ddd-strain-rate-to-quasi-static-fem) and [epilogue Handshake 4a](../epilogue/multiscale.md#4a--ddd-strain-rate-to-quasi-static-load-cell-act-iv--hardening) extrapolate to lab grip speed.
...
| [VI.4 Bridge](../part06-continuum/04-nonlinear-plasticity-preview.md#bridge-to-part-vii) names the hinge | [VII.1 opening hinge](01-defect-taxonomy.md#defect-taxonomy-opening-hinge-plasticity-to-forest) replaces scalar \(\alpha\) with Burgers geometry |
...
Turn the page when phenomenological hardening feels like curve-fitting — defect taxonomy is where the wire's strength acquires a geometry. If the return-mapping Lab act in [VI.4](../part06-continuum/04-nonlinear-plasticity-preview.md#lab-act-return-mapping-on-the-load-cell-knee-act-iv-hardening) reproduced the knee but \(\alpha\) still feels like a fitted scalar, continue to [VII.1's opening hinge](01-defect-taxonomy.md#defect-taxonomy-opening-hinge-plasticity-to-forest), where Burgers vector \(\mathbf{b}\) replaces internal variable \(\alpha\) as the primary object for the first time since Part VI began.
