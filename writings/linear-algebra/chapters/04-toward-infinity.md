...
## Intermission: grammar complete, function spaces begin {#intermission-grammar-complete-function-spaces-begin}

If you have read linearly since the [prologue intermission](../prologue/00-many-scales.md#intermission-panorama-complete-grammar-begins), the copper wire has already appeared at nine scales — but Part I is the first time those scales spoke in **syntax**: vectors and matrices, symmetry and sparsity, eigenmodes that decouple vibration, and the refinement limit that sends nodal values toward a function \(u(x)\) instead of a longer array. Part I closes with the grammar every simulation shares, written explicitly in \(\mathbb{R}^N\) before infinity makes the notation honest.

Part II is where the story stops counting components and starts **naming spaces**. The specimen on the bench has not moved — only the state variable widens: from \(\mathbf{u} \in \mathbb{R}^N\) to \(u \in H^1\), from \(\mathbf{K}\mathbf{u}=\mathbf{f}\) to a bilinear form \(a(u,v)=\ell(v)\), from discrete eigenvectors to the spectrum of a self-adjoint operator. When mesh refinement feels like chasing a longer vector rather than approximating a field, return to this intermission: \(\mathbf{K}_N\) was always a shadow; Part II supplies the room the shadow falls on.

The [preface ascent continuity hinge](../preface.md#ascent-continuity-hinges) names this turn as **finite → infinite dimensions** — the first hinge in the mathematical climb after the prologue's panorama. [Part II opening — Closing the arc from Part I](../part02-functional-analysis/00-opening.md#closing-the-arc-from-part-i) replays the four questions in \(H^1\) and \(L^2\). Read the [concept map checkpoint](#concept-map-checkpoint-part-i) above if you need a one-table summary before turning the page — then continue to the [Bridge](#bridge-to-part-ii) below.
...
Turn the page when springs and eigenmodes feel complete but \(\mathbf{K}\mathbf{u}=\mathbf{f}\) still feels unrelated to PDEs — that is the signal function spaces are the next move, not another mesh refinement. If you paused after the grammar climb, reread the [Intermission: grammar complete, function spaces begin](#intermission-grammar-complete-function-spaces-begin) above before opening [Part II](../part02-functional-analysis/00-opening.md#closing-the-arc-from-part-i). We leave the comfort of \(\mathbb{R}^N\) and enter the space of admissible fields. The copper wire’s temperature and displacement live there; our meshes are finite-dimensional shadows of those fields, and the shadow improves as \(h \to 0\).
