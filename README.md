...
Chapters synthesize the author's existing notes on [hanfengzhai.github.io](https://hanfengzhai.github.io/note.html) and related repositories. Canonical source markdown lives under [`writings/`](./writings/) in Functional Analysis Notes style (one mdBook per part, numbered chapters, **Bridge** sections). When the external `Writings` git repo is available, run `./scripts/clone-writings.sh` to fetch upstream; then `./scripts/sync-writings.sh` to refresh `src/` from those sources.

## Build

```bash
# Install mdBook: https://github.com/rust-lang/mdBook/releases
chmod +x scripts/clone-writings.sh scripts/sync-writings.sh scripts/build-all-writings.sh
