...
**Potentials → ensembles reunion (row 36).** If rows 17–35 all read correctly but [VIII.1's Bridge](01-potentials-phase-space.md#bridge) and this opening hinge still feel disconnected — the EAM archive exists but NPT equilibration has not read \(T_w = 311.48\,\text{K}\) from [`fixtures/cht_export.yaml`](../../fixtures/cht_export.yaml) — read the [potentials → ensembles reunion index](../appendix/sources.md#potentials-ensembles-reunion-index-row-36). Row 36 reunites **0 K minimization with NVT/NPT trajectories** when the potential surface is calibrated but the wire's laboratory temperature has not yet entered the simulation.

**Ensembles → coarse-graining reunion (row 37).** If rows 17–36 all read correctly but this chapter's Bridge and [VIII.3's opening hinge](03-ab-initio-and-coarse-graining.md#opening-hinge-viii2-to-viii3) still feel disconnected — `mobility_cu_screw_311K.yaml` exists but no [pedigree checklist](03-ab-initio-and-coarse-graining.md#pedigree-checklist-before-the-epilogue) names DFT audit gates — read the [ensembles → coarse-graining reunion index](../appendix/sources.md#ensembles-coarse-graining-reunion-index-row-37). Row 37 reunites **audited trajectories with yaml handoff tables** when dynamics closed but coarse-grained exports still feel like multiscale folklore.
