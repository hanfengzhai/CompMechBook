...
**VII.3 → VIII descent handoff reunion (row 35).** If rows 17–34 all read correctly but [VII.3's Bridge](../part07-defects/03-polycrystal-and-fem-handoff.md#bridge-to-part-viii) and this opening hinge still feel disconnected — the export table above maps Part VII columns to Part VIII.1 vocabulary but the handoff bundle has not yet archived `cu_eam_a0.txt` — read the [VII.3 → VIII descent handoff reunion index](../appendix/sources.md#vii3-viii-descent-handoff-reunion-index-row-35). Row 35 reunites **mesoscale parameter citations with atomistic foundation** when the Bridge named the ink but coordinates and EAM forces have not yet replaced line cutoffs.

**Potentials → ensembles reunion (row 36).** If rows 17–35 all read correctly but this chapter's EAM Lab act and [VIII.2's opening hinge](../part08-md/02-ensembles-integrators.md#opening-hinge-viii1-to-viii2) still feel disconnected — `cu_eam_a0.txt` exists but `MD_NVT_shear_PartVIII` has not run at \(T_w = 311.48\,\text{K}\) from [`fixtures/cht_export.yaml`](../../fixtures/cht_export.yaml) — read the [potentials → ensembles reunion index](../appendix/sources.md#potentials-ensembles-reunion-index-row-36). Row 36 reunites **static potential surface with dynamic sampling** when the foundation archive is complete but phase space has not yet been sampled at laboratory temperature.
