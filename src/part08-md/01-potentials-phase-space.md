...
The [Functional Analysis Notes](https://hanfengzhai.github.io/file/teaching/notes/ME412_CourseSummary.pdf) ask four questions at every scale change. At the atomistic level they read:

| Question | Answer for this chapter |
|----------|---------------------------|
| What **object**? | Positions \(\{\mathbf{r}_i\}\), momenta \(\{\mathbf{p}_i\}\), potential energy \(V(\{\mathbf{r}_i\})\) |
| What **structure**? | Hamiltonian phase space; periodic boundaries; neighbor lists within cutoff |
| What **theorem**? | Born–Oppenheimer separation (Part IX); Newton's equations equivalent to Hamilton's |
| What **breaks**? | Wrong EAM fit; chemistry beyond classical MD; strain-rate and time-scale gaps vs. wire tests |

Keep this table visible as we descend from DDD's closed cores to explicit atomic trajectories. The copper wire's notch — already a stress concentrator in Part VI, a dislocation source in Part VII — becomes here a region where bonds stretch and rearrange. MD is the first rung that can watch that rearrangement happen.
