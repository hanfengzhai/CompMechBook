...
Part VII closed with dislocation lines, Taylor hardening, and the OpenDiS→DAMASK→FEM handoff. The drawn copper wire's strength is no longer a mystery \(\sigma_y\) on a stress–strain curve — it is a **forest** of line defects whose density and link statistics evolve with strain. Yet DDD treats each dislocation core as a closed singularity with empirical short-range rules. When cores meet grain boundaries, when a notch concentrates stress until bonds break, or when oxygen pins a dislocation at a surface, the physics lives in **atomic positions**, not in Peach–Köhler alone.

Part VIII puts the atoms back. Mobility \(M(\tau,T)\), stacking-fault energy \(\gamma_{\mathrm{sf}}\), and crack-tip nucleation barriers are not adjustable forever — they are extracted from trajectories on a potential energy surface. That surface, in turn, is an average over electrons that Part IX will compute from first principles. The ladder does not restart here; it **refines the ink** in which Part VII wrote its lines.
