...
Part VII ended with dislocation lines gliding through a polycrystal, exporting hardening laws and link statistics to crystal plasticity FEM. That picture is still **coarse-grained**: the dislocation core is a line singularity regularized by a cutoff radius; mobility tables are fit from experiments or atomistic snapshots, not derived from first principles. If **Act II — Warming** ran the conjugate heat transfer loop from [V.4](../part05-fvm/04-navier-stokes-cfd.md#lab-act-extension-two-domain-picard-loop-with-a-1d-fem-solid), the wall temperature \(T_w = 311.48\,\text{K}\) from [`fixtures/cht_export.yaml`](../../fixtures/cht_export.yaml) softens phonon drag before the first MD timestep — NVT shear tests and mobility fits in this part must read \(M(\tau, T_w)\), not \(M(\tau, 300\,\text{K})\). [Row 33](../appendix/sources.md#part-vii-viii-descent-hinge-reunion-index-row-33) reunites this opening with [Part VII's descent hinge](../part07-defects/00-opening.md#descent-hinge-from-vi4-energy-break-forest-begins) when DDD and MD still feel like separate courses.
...
[Part VII's ascent hinge](../part07-defects/00-opening.md#ascent-hinge-midpoint-and-twin-ladders) closed the **ascent** with the [twin ladders reunion](../part06-continuum/00-opening.md#the-twin-ladders-reunite-galerkin-and-conservation) — Galerkin energy (Part IV) and conservation flux (Part V) on one specimen before descent began. Part VII inherited \(T_w = 311.48\,\text{K}\) from [`fixtures/cht_export.yaml`](../../fixtures/cht_export.yaml) and demanded DDD mobility \(M(\tau, T_w)\), not a room-temperature default. Part VIII inherits that **temperature pedigree** at the atomistic layer: every NVT shear test that calibrates drag, every parallel-tempering ladder in [VIII.3](03-ab-initio-and-coarse-graining.md#wham-part-vii-mobility-hinge-act-ii-temperature-pedigree), and every phonon-lifetime interpolation for rate sensitivity must match the converged \(T_w\) from [V.4's Picard loop](../part05-fvm/04-navier-stokes-cfd.md#lab-act-extension-two-domain-picard-loop-with-a-1d-fem-solid) — archive `cht_export.yaml` beside `mobility_cu_screw_311K.yaml` so OpenDiS, MD, and the epilogue share one wall temperature.
...
When **VIII.2 NVT/NPT trajectories are audited but VIII.3 handoff tables still feel like a separate course** — `mobility_cu_screw_{T_w}K.yaml` without `cu.elastic/` in the foundation folder, or the [pedigree checklist](03-ab-initio-and-coarse-graining.md#pedigree-checklist-before-the-epilogue) empty before Part IX — read the [ensembles → coarse-graining reunion index](../appendix/sources.md#ensembles-coarse-graining-reunion-index-row-35) (row 35) and the [preface ensembles → coarse-graining thread](../preface.md#reading-rhythm). Row 35 reunites **trajectories with yaml exports**; row 34 remains the audit when 0 K minimization and finite-\(T\) sampling still feel disconnected.
...
The first chapter below opens **phase space** — positions, momenta, Hamiltonian mechanics — and the interatomic potentials every MD run of copper assumes on trust until the audit in Part IX. When the mesh is fine enough but the core is still wrong — that is the hinge between line defects and atoms. **Acts V and VI** in the lab reunion — notch resolution and foundation pedigree — depend on this descent: the [epilogue preview reunion](../epilogue/multiscale.md#lab-act-reunion-preview) names Act V before the handshake sections; the [expanded reunion](../epilogue/multiscale.md#lab-act-reunion-six-acts-one-afternoon) wires Acts V–VI to Handshakes 4b and foundation orchestration after Part VIII exports are archived — preview rows [#reunion-preview-act-v](../epilogue/multiscale.md#reunion-preview-act-v) and [#reunion-preview-act-vi](../epilogue/multiscale.md#reunion-preview-act-vi) link forward to [#reunion-expanded-act-v](../epilogue/multiscale.md#reunion-expanded-act-v) and [#reunion-expanded-act-vi](../epilogue/multiscale.md#reunion-expanded-act-vi). Turn the page.
