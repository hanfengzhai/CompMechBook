...
When **the pedigree checklist has consumer rows filled but Part IX.0 still feels like standalone DFT coursework** — `cu.foundation/` lacks `cu.relax.out`, or phonon folders cite 300 K only — read the [coarse-graining → electronic audit reunion index](../appendix/sources.md#coarse-graining-electronic-audit-reunion-index-row-36) (row 36) and the [preface reading rhythm](../preface.md#reading-rhythm) before opening [Part IX](../part09-dft/00-opening.md). Row 36 reunites **handoff tables with SCF audit gates**; row 35 remains the audit when yaml exports still feel disconnected from converged trajectories.
