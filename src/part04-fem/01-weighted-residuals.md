...
[Part III.4](../part03-pdes/04-energy-methods.md#bridge-to-part-iv) closed the ascent with a pipeline the copper wire already obeyed in the lab: strong PDE → weak form → energy minimum in \(H^1\) → search on a finite-dimensional subspace \(V_h\). The Dirichlet principle made equilibrium a bowl-shaped functional; Lax–Milgram proved the minimizer exists; the Rayleigh–Ritz preview promised that nodal coefficients would reproduce the same minimum once shape functions were chosen. Part IV opens on the **operational** sentence that preview deferred: *how do we enforce the weak form on a mesh and recover \(\mathbf{K}\mathbf{U}=\mathbf{F}\)?*

Return to the copper wire from the prologue. At the engineering scale we want its temperature under Joule heating, or its displacement under tension — fields governed by elliptic PDEs. Part III showed how to weaken those PDEs: multiply by a test function, integrate by parts, and obtain a bilinear form \(a(u,v)=\ell(v)\). Weighted residuals are the discretization of that statement: instead of asking the PDE to hold at every point, ask the **residual** to vanish when averaged against a finite set of test directions.
