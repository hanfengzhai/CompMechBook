...
| [IV.5](05-convergence.md#lab-act-extension-thermoelastic-h-refinement-on-one-mesh-acts-iiiii) | \(h\)-refinement on both blocks on same \(\mathcal{G}\); export converged \(T_w\) before [V.0 CHT](../part05-fvm/00-opening.md#conjugate-heat-transfer-the-wire-meets-the-wind) |

When the grip reaction in Act III exceeds what elastic strain alone predicts, the discrepancy is usually **thermal stress** from Act II — not a mystery in the plasticity model downstream. Part III named that channel in energy language ([III.4 monolithic vs staggered](../part03-pdes/04-energy-methods.md#monolithic-vs-staggered-thermoelastic-energy)); Part IV assigns node values to the blocks. Handbook \(\alpha\) at 300 K violates the export contract when Act II archived \(T_w = 311.48\,\text{K}\) in `cht_export.yaml` — the [epilogue Handshake 3](../epilogue/multiscale.md#handshake-3--thermal-strain--mechanical-stiffness-part-vi--iv) is the workflow-time mirror; the Scale-boundary handshake at the end of this opening is the reading-time mirror.
...
| Thermal eigenstrain \(\varepsilon_{\text{th}} = \alpha(T_h - T_{\text{ref}})\) | Equivalent load vector \(\mathbf{F}_{\text{th}}\) in mechanical block | [VI.2 \(\alpha\) handshake](../part06-continuum/02-stress-balance.md#scale-boundary-handshake-thermal-expansion-coefficient-alpha) | Handbook \(\alpha\) at 300 K when `cht_export.yaml` documents \(T_w = 311.48\,\text{K}\) |
