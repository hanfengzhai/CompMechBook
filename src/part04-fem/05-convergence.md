...
When **Door A opened Part V but solid conduction and fluid convection still feel like separate solvers** — even though this chapter certified the thermoelastic \(h\)-study — read the [CHT outer-loop reunion index](../appendix/sources.md#cht-outer-loop-reunion-index-row-30) (row 30) and run the [V.4 Picard + parser checkpoint](../part05-fvm/04-navier-stokes-cfd.md#parser-checkpoint-archive-cht-export-yaml) before opening Part VI. Row 30 reunites **Galerkin solid and FVM fluid at \(\Gamma_w\)**; row 29 remains the audit when heat and mechanics never shared one mesh.

When **Door B opens Part VI but \(\mathbf{K}\mathbf{U}=\mathbf{F}\) still feels like sparse linear algebra without virtual work** — Céa's certificate is clear but the load cell has no energy story — read the [twin-ladder → virtual work reunion index](../appendix/sources.md#twin-ladder-virtual-work-reunion-index-row-31) (row 31) at [VI.0](../part06-continuum/00-opening.md#the-twin-ladders-reunite-galerkin-and-conservation) before blaming Part VI tensor notation. Row 31 reunites **Galerkin assembly with \(\delta\Pi = 0\)**; row 30 remains the audit when Door A skipped `cht_export.yaml`.

When **virtual work closed the elastic climb but [VI.4](../part06-continuum/04-nonlinear-plasticity-preview.md) return-mapping still feels like curve-fitting** — Céa's rates hold and \(\delta\Pi = 0\) matched the load cell, but the yield knee has fitted \(H\) with no descent story — read the [virtual work → plasticity preview reunion index](../appendix/sources.md#virtual-work-plasticity-preview-reunion-index-row-32) (row 32) at [VI.4's return-mapping Lab act](../part06-continuum/04-nonlinear-plasticity-preview.md#lab-act-return-mapping-on-the-load-cell-knee-act-iv-hardening) before opening Part VII. Row 32 reunites **elastic energy with plastic history**; row 31 remains the audit when assembly never entered \(\Pi[\mathbf{u}]\).
