...
[IV.2](02-galerkin-assembly.md) closed with a diagnostic: when the scatter loop is correct but stress contours still jump between meshes, the fault lives **inside** the element loop — not in connectivity or BC rows. Shape functions, reference-to-physical maps, and quadrature determine whether the local integrands in \(\mathbf{k}^e\) are evaluated faithfully. This chapter opens that loop.

A finite element is three things bundled together: a **reference domain** with fixed geometry, a set of **shape functions** that interpolate fields on that domain, and a **quadrature rule** that integrates weak-form kernels accurately and efficiently. Mesh generators supply node coordinates and connectivity; the element library supplies everything else.

Chapter 2 showed assembly as a scatter of local matrices. Here we explain what happens before the scatter — how geometry enters through the Jacobian, how polynomial order controls accuracy, and why bad elements (slivers, nearly incompressible materials on Q1 meshes) produce bad answers even when the assembly code is correct.

## Scene: the mesh becomes tiny shapes

Return to the copper wire model and zoom until individual elements fill the screen: small triangles or bricks, each with the same reference template, stretched and rotated to fit the local geometry. During **Act II — Warming**, these patches carried the temperature field whose energy Part III minimized; during **Act III — Pulling**, the same patches carry displacement. Shape functions interpolate both fields inside each patch; quadrature integrates the weak form as a weighted sum of point values. A coarse mesh captures bulk stretch; a fine mesh resolves the hot spot where current density peaks — same element library, different resolution, one afternoon on the bench.
...
| IV.3 output (this chapter) | IV.4 consumer ([next chapter](04-poisson-to-elasticity.md)) | Wire act | Failure mode |
|----------------------------|-------------------------------------------------------------|----------|--------------|
| P1 triangles; centroid quadrature on \(\int k\|\nabla T\|^2\) | Block \(\mathbf{K}\) from \(\mathbf{B}^T\mathbb{C}\mathbf{B}\) | Act II heated cross-section + Act III tension on one mesh | Thermal eigenstrain omitted in mechanical pass |
| Conforming \(H^1\) continuity; patch test (Lab act) | Vector patch test on linear elasticity | Linear elastic climb before yield | Non-symmetric \(\mathbf{K}\) from transposed B-matrix |
| Isoparametric \(\mathbf{J}\), \(\det J\) in volume integrals | Traction face quadrature on grip Neumann BC | End load split from body weight | Wrong \(\det J\) sign in distorted grip cluster |
| \(p\)- vs. \(h\)-refinement on smooth temperature | Anisotropic \(\mathbb{C}\) from cold-drawn texture | Part VI variational elasticity handoff | Q1 hex hourglassing when \(\nu \to 1/2\) |
...
Return to the [prologue](../../prologue/00-many-scales.md): **Act III — Pulling** is where the load cell measures force on a mesh built from the element families named here. Part III minimized thermal energy on the same P1 triangles during **Act II — Warming**; [IV.4](04-poisson-to-elasticity.md) closes the scalar-to-vector jump — the chapter where the copper wire stops being a temperature field alone and becomes the tensile bar the prologue tracks through yield. The [preface ascent preview chain](../preface.md#ascent-preview-chain) placed Part IV after Part III so both acts could share one element library before Part V adds a second discretization dialect for the air cooling the wire.
