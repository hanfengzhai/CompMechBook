...
| Row 8 — \(T_w\) pedigree (Act II → MD/DDD) | [Preface: descent hinge row 8](../preface.md#descent-continuity-hinges) — [V.4 CHT](../part05-fvm/04-navier-stokes-cfd.md#lab-act-extension-two-domain-picard-loop-with-a-1d-fem-solid) → [`cht_export.yaml`](../scripts/parse_cht.sh) → [VIII.0 descent hinge](../part08-md/00-opening.md#descent-hinge-cores-mobility-and-tw-pedigree) → [VIII.3 WHAM at \(T_w\)](../part08-md/03-ab-initio-and-coarse-graining.md#wham--part-vii-mobility-hinge-act-ii-temperature-pedigree); [memory sheet rows 8–9 baby picture](../appendix/memory-sheet.md#rows-8-9-baby-picture-tw-temperature-pedigree) |
| Row 9 — phonon audit at \(T_w\) (MD → DFT) | [Preface: descent hinge row 9](../preface.md#descent-continuity-hinges) — [IX.0 electronic audit hinge](../part09-dft/00-opening.md#electronic-audit-hinge-descent-pedigree-and-tw-phonon) · [thermal phonon audit at \(T_w\)](../part09-dft/00-opening.md#thermal-phonon-audit-at-tw); [`parse_alpha.sh`](../scripts/parse_alpha.sh) with `--temperature` from `cht_export.yaml`; [memory sheet rows 8–9 baby picture](../appendix/memory-sheet.md#rows-8-9-baby-picture-tw-temperature-pedigree); [epilogue Closing the arc from Part IX](../epilogue/multiscale.md#closing-the-arc-from-part-ix) |
...
| Act VI orchestration → multiscale export (row 16) | [Preface: row 16 skill checkpoint](../preface.md#skill-navigation-row-16) — [IX.3 Bridge to epilogue](../part09-dft/03-dft-workflows.md#bridge-to-the-epilogue) → [`parse_multiscale_workflow.sh`](../scripts/parse_multiscale_workflow.sh) → `multiscale_export.yaml`; [Part IX coupling ladder](../part09-dft/00-opening.md#the-coupling-ladder-me-412-reunion); epilogue [Act VI reunion paragraph](../epilogue/multiscale.md#lab-act-reunion-six-acts-one-afternoon); [memory sheet row 16](../appendix/memory-sheet.md#continuity-hinges-master-map) ME 412 coupling ladder baby picture; upstream handshakes from [rows 13–15](../preface.md#skill-navigation-row-13) |
...
| Row 8 preview (Act II → descent) | Explain why MD mobility and WHAM must inherit \(T_w\) from CHT, not 300 K defaults | Sketch: V.4 Picard loop → `cht_export.yaml` → NVT shear at \(T_w\) — [preface descent hinge row 8](../preface.md#descent-continuity-hinges); [VIII.0 descent hinge](../part08-md/00-opening.md#descent-hinge-cores-mobility-and-tw-pedigree); [memory sheet rows 8–9 baby picture](../appendix/memory-sheet.md#rows-8-9-baby-picture-tw-temperature-pedigree) |
| Row 9 preview (MD → DFT audit) | Explain why EAM bulk moduli on trust still need \(\alpha(T_w)\) and \(\tau_{\text{ph}}(T_w)\) beside SCF logs | One sentence: "SCF audits moduli; phonon audit audits thermal exports at the same \(T_w\)" — [preface descent hinge row 9](../preface.md#descent-continuity-hinges); [IX.0 thermal phonon audit](../part09-dft/00-opening.md#thermal-phonon-audit-at-tw); [memory sheet rows 8–9 baby picture](../appendix/memory-sheet.md#rows-8-9-baby-picture-tw-temperature-pedigree) |
...
| Row 16 preview (Act VI foundation) | Explain why individual handshake exports need an orchestrated pedigree file before the epilogue reunites them | Sketch: [`parse_multiscale_workflow.sh`](../scripts/parse_multiscale_workflow.sh) runs Handshakes 1–4b in dependency order → `multiscale_export.yaml` — [preface row 16](../preface.md#skill-navigation-row-16); [Part IX coupling ladder](../part09-dft/00-opening.md#the-coupling-ladder-me-412-reunion); [IX.3 Bridge to epilogue](../part09-dft/03-dft-workflows.md#bridge-to-the-epilogue); epilogue [Act VI reunion paragraph](../epilogue/multiscale.md#lab-act-reunion-six-acts-one-afternoon); [memory sheet Act VI baby picture](../appendix/memory-sheet.md#act-vi-baby-picture-me-412-coupling-ladder) |

None of these require running a code — but each one is the navigation discipline the Functional Analysis Notes layout assumes at every part opening. Rows 8–9 are **forward-looking** previews of the descent temperature contract — Act II's Joule heating sets \(T_w\) once; MD, DDD, and DFT phonon audits inherit that column before Handshakes 3 and 4a run in the epilogue. The [memory sheet rows 8–9 baby picture](../appendix/memory-sheet.md#rows-8-9-baby-picture-tw-temperature-pedigree) draws the chain; the [preface descent continuity hinges](../preface.md#descent-continuity-hinges) name rows 8 and 9 when Parts VIII–IX feel disconnected from Act II warming. The last six rows (8–9, 13–16) are **forward-looking** mirrors of the [epilogue workflow exam](../epilogue/multiscale.md#what-you-should-be-able-to-do-after-the-book): you will not produce [`parse_alpha.sh`](../scripts/parse_alpha.sh), [`parse_rate.sh`](../scripts/parse_rate.sh), [`parse_fe2.sh`](../scripts/parse_fe2.sh), or [`parse_multiscale_workflow.sh`](../scripts/parse_multiscale_workflow.sh) artifacts until Parts IV–IX, but naming Handshakes 2–3, 4a, 4b, and the orchestrated coupling ladder here keeps Acts II–VI from feeling like separate courses when mathematical order (I→IX) and laboratory time diverge on the same afternoon. If you can locate a chapter on the ladder, name the four questions it answers, and say which lab act the operator is watching, Part I's \(\mathbf{K}\mathbf{u}=\mathbf{f}\) will not feel like a new subject. It will feel like the bottom rung of a story you have already started.
