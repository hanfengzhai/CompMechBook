...
| Act | What happens in the lab | What the book computes | Parts |
|-----|-------------------------|------------------------|-------|
| **I — Mounting** | A cold-drawn copper cylinder is gripped in wedge jaws; the operator zeros the load cell and thermocouple. | Boundary conditions, a chain of bar elements, the first \(\mathbf{K}\mathbf{u}=\mathbf{f}\). | Prologue, I |
| **II — Warming** | Current is switched on. The narrowest cross-section heats; air cools the surface. | Steady heat conduction in the solid (FEM), enthalpy flux in the surrounding air (FVM), Robin coupling at the wall. | III, IV, V |
| **III — Pulling** | Grip displacement ramps. The force–displacement curve climbs almost linearly. | Weak-form elasticity, Galerkin assembly, convergence in \(H^1\); Cauchy stress and virtual work name what the curve measures. | II, III, IV, VI |
| **IV — Hardening** | The curve bends upward: more force for each increment of stretch. | Dislocation forest from cold drawing plus fresh multiplication; DDD and Taylor hardening explain the bend without a magic constant. | VII |
| **V — Notch (optional)** | A scratched wire or a grip corner concentrates stress. | Continuum fields predict *where*; MD at the tip resolves bond breaking and dislocation nucleation. | VI, VIII |
| **VI — Foundation (always, in parallel)** | Before any simulation ran, someone chose \(E\), \(\nu\), and a yield stress. | DFT on a small fcc cell supplies cohesive energy and elastic constants; MD fits an EAM potential; those numbers climb the ladder into FEM input decks. | IX → VIII → VII → IV |
