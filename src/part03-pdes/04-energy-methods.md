...
## Intermission: analysis complete, assembly begins {#intermission-analysis-complete-assembly-begins}

If you have read linearly since the [prologue](../prologue/00-many-scales.md) and [Part I's grammar intermission](../part01-linear-algebra/04-toward-infinity.md#intermission-grammar-complete-function-spaces-begin), the copper wire has changed representation three times without changing material: springs in \(\mathbb{R}^N\), fields in \(H^1\), and now **equations** on those fields — strong forms that fail at corners, weak forms that survive, Sobolev regularity that names what "smooth enough" means, and energy methods that prove equilibrium is minimization. Part III is the last purely analytical chapter before the book writes **code**.

Part IV will replace the infinite-dimensional search over admissible fields with a finite-dimensional search over nodal values — Galerkin assembly, shape functions, quadrature — and call the result \(\mathbf{K}\mathbf{U}=\mathbf{F}\). The matrix from Part I returns, now built from bilinear forms defined here. When Rayleigh–Ritz and weak-form assembly feel like separate subjects, return to this intermission: they are the same minimization in two languages. The [preface ascent continuity hinge](../preface.md#ascent-continuity-hinges) lists this chapter as the **well-posedness → assembly** turn; [Part IV opening](../part04-fem/00-opening.md#story-so-far-parts-i-iii) picks up the story with the meshed solid.

The specimen on the bench has not moved — only the vocabulary has. Act I's mounting still means boundary conditions; Act II's warming still means a thermal functional; Act III's pulling still means elastic energy minus grip work. What changes on the next page is **how** those functionals become numbers: element loops instead of proofs, Gauss points instead of Gâteaux derivatives. If you need a one-paragraph reminder of the analytical arc before assembly, read [Part III opening — Story so far](../part03-pdes/00-opening.md#story-so-far-parts-i-ii) and then continue here.
...
Turn the page when the weak form is clear and the energy functional is written, but no global matrix exists yet — that is the signal Galerkin assembly is the next move. If you paused after the analytical climb, reread the [Intermission: analysis complete, assembly begins](#intermission-analysis-complete-assembly-begins) above before opening Part IV. The same \(\mathbf{K}\mathbf{U}=\mathbf{F}\) from Part I, now built from shape functions, Jacobians, and the bilinear forms defined in Part III, awaits in [IV opening](../part04-fem/00-opening.md#closing-the-arc-from-part-iii).
