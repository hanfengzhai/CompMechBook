...
|---------------------------------|---------------------------------------------------|---------------|--------------|
| \(a(u,v)=\ell(v)\) with one derivative on tests | \(H^1\): \(\nabla u \in L^2\) for piecewise-smooth fields | Midspan heating (Act II); elastic zone (Act III) | Demanding \(C^2\) at grip corner |
| Galerkin preview \(\mathbf{K}\mathbf{U}=\mathbf{F}\) | \(H^1_0\) encodes Dirichlet BC; trace on boundaries | Fixed-grip ends at 300 K | Mixing essential and natural BC rows |
| Point loads as functionals | \(H^{-1}\) dual loads; midspan force on bar | Act III concentrated grip load | Storing Dirac delta in assembly vector |
| Lax–Milgram hypotheses stated | Poincaré inequality; \(H^2\) for optimal rates | Mesh refinement toward smooth interior | Expecting \(O(h^2)\) at reentrant corner |
...
[III.3](03-sobolev-spaces.md) opens on the same picture: ten linear bar elements for Joule heating, continuous temperature with kinks at nodes — conforming \(H^1\) approximations the weak form here already justified. Energy methods ([III.4](04-energy-methods.md)) then recast \(a(u,v)=\ell(v)\) as minimization or saddle-point principles — the variational backbone of FEM and, in nonlinear settings, of hyperelastic and phase-field solvers.
