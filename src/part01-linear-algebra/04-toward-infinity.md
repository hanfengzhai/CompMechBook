...
Picture the operator one last time at Act III’s prelude: the load cell still reads zero, but the post-processor already shows three displacement profiles from the bar-refinement Lab act. The coarse mesh is jagged; the fine mesh is almost a straight line. Nothing in the laboratory changed between plots — only the **state description** did, from a column vector to a field the eye interprets as \(u(x)\). Part II is that same afternoon written in analysis language: the space where those profiles converge, the norm that makes “closer” mean lower strain energy, and the completeness that guarantees the limit stays inside the admissible class. The story does not jump to a new course; the wire simply acquires infinitely many degrees of freedom that finite meshes approximate.
