...
## Writings canonical hinge (Part I → Part II) {#writings-canonical-hinge-i4-to-ii0}

This chapter is the last file in the **Linear Algebra Notes** subtree ([`writings/linear-algebra/`](../../writings/linear-algebra/)); the next page of the continuous book is the **Functional Analysis Notes** subtree ([`writings/functional-analysis/`](../../writings/functional-analysis/)), beginning at [Part II opening](../part02-functional-analysis/00-opening.md). The numbering is intentional: Part I ends at `04-toward-infinity.md`; Part II opens at `00-opening.md` with [**Closing the arc from Part I**](../part02-functional-analysis/00-opening.md#closing-the-arc-from-part-i) — the same four ME 412 questions (object, structure, theorem, what breaks), now answered with fields \(u(x)\), \(T(x)\) instead of vectors \(\mathbf{u}_N\).

**One continuous sentence (read aloud before Part II):** the thermocouple mesh from the Scene above is still the copper wire; only the state variable promoted from \(\mathbb{R}^N\) to a function space; Part II does not change the experiment, it names the room where the limiting profile lives.

The unified mdBook [`src/SUMMARY.md`](../SUMMARY.md) is a sync of these canonical files — edit under `writings/<part>/chapters/`, run `./scripts/sync-writings.sh`, then `mdbook build`. If Part II ever feels like a separate course after this chapter, run `./scripts/sync-writings.sh --check` and read the [Writings canonical reunion (row 46)](../appendix/sources.md#rows17-45-writings-canonical-reunion-index-row-46) — the break is usually skipping this hinge, not missing functional analysis prerequisites.
