...
**Hyperbolic prerequisite (Part V.3).** Before trusting the fluid-side advection that carries enthalpy away from the wire, run the Sod shock-tube regression from [V.3](../part05-fvm/03-fluxes-riemann.md#lab-act-sod-shock-tube-trust-gate) — mass conserved to machine precision, shock positions within 5% at 100 cells. Handshake 2 couples two meshes; if the FVM kernel fails conservative differencing in isolation, the Picard loop on \(T_w\) converges to a **wrong** fixed point. Archive Sod L¹ errors beside the CHT iteration log — the same discipline Part IV's patch test demands before Act III.
...
| **II — Warming** | Current on; thermocouple climbs | III–V | FEM conduction ↔ FVM convection at wall; [V.3 Sod gate](../part05-fvm/03-fluxes-riemann.md) before CHT Picard |
