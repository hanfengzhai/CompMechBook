...
None of these require a national allocation — but each one separates a reproducible multiscale study from a pile of incompatible input decks. If you can trace one modulus to an archived SCF log, run a conjugate heat loop until fluxes agree, complete the [row 13 skill checkpoint](../preface.md#skill-navigation-row-13) when handbook \(\alpha\) persists after CHT converges, complete the [row 14 skill checkpoint](../preface.md#skill-navigation-row-14) when DDD exports feed the plasticity deck without rate extrapolation, complete the [row 15 skill checkpoint](../preface.md#skill-navigation-row-15) when the optional notch under-predicts root stress, and say which act of the lab session a new simulation belongs to, you have closed the loop the prologue opened. Return to the [prologue four questions](../prologue/00-many-scales.md#the-same-questions-at-every-scale) on the next material; the specimen changes, the habit does not.
