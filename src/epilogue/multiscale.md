...
| III — Pulling | Force–displacement ramp | II, III, IV, VI | Handshake 2 → 3: \(\Delta T\) from CHT ([Handshake 2](#handshake-2--joule-heating--conjugate-heat-transfer-part-iv--v)); \(\alpha\Delta T\) from [IX.3 quasiharmonic Lab act](../part09-dft/03-dft-workflows.md#lab-act-quasiharmonic-alpha-handshake-3-pedigree) → fixed-grip stress ([Handshake 3](#handshake-3--thermal-strain--mechanical-stiffness-part-vi--iv)); [memory sheet row 13](../appendix/memory-sheet.md#continuity-hinges-master-map); [preface row 13](../preface.md#skill-navigation-row-13); [`parse_alpha.sh`](../../scripts/parse_alpha.sh); weak form → assembly → stress interpretation |
| IV — Hardening | Curve bends upward | VII | Handshake 4a: DDD \(\tau(\dot\varepsilon)\) → power-law \(m\) → lab-rate \(\tau_{\text{lab}}\) ([memory sheet row 14](../appendix/memory-sheet.md#continuity-hinges-master-map), [preface row 14](../preface.md#skill-navigation-row-14), [VII.3 rate handshake](../part07-defects/03-polycrystal-and-fem-handoff.md#scale-boundary-handshake-ddd-strain-rate-to-quasi-static-fem), [`parse_rate.sh`](../../scripts/parse_rate.sh)); Taylor hardening → crystal plasticity FEM |
| V — Notch | Stress concentrator | VI, VII, VIII | Handshake 4b: scalar \(H\) vs FE² at root ([memory sheet row 15](../appendix/memory-sheet.md#continuity-hinges-master-map), [preface row 15](../preface.md#skill-navigation-row-15)); MD resolves nucleation where continuum regularizes |
...
**Upstream contract ([VII.3 rate handshake](../part07-defects/03-polycrystal-and-fem-handoff.md#scale-boundary-handshake-ddd-strain-rate-to-quasi-static-fem)).** This subsection is the epilogue-only **downstream half** of the rate extrapolation Part VII documented — OpenDiS RVE sweeps at \(\dot\varepsilon \sim 10^2\)–\(10^4\,\text{s}^{-1}\), power-law fit for \(m\), and `rate_export.yaml` provenance beside `hardening.yaml`. If you arrived here from Act IV without reading Part VII, read the [VII.3 rate handshake](../part07-defects/03-polycrystal-and-fem-handoff.md#scale-boundary-handshake-ddd-strain-rate-to-quasi-static-fem) first; if you arrived from [Part VIII's descent hinge](../part08-md/00-opening.md#descent-hinge-cores-mobility-and-tw-pedigree), confirm mobility tables and \(m\) were evaluated at the same \(T_w\) Handshake 2 converged — not at 300 K by default. The upstream handshake names the OpenDiS workflow; Handshake 4a names the load-cell consequence when that workflow is skipped.

This section is the **downstream half** of [VII.2's forest-density Lab act](../part07-defects/02-dislocation-dynamics.md#lab-act-read-the-hardening-bend-from-forest-density-act-iv) and [VII.3's rate handshake](../part07-defects/03-polycrystal-and-fem-handoff.md#scale-boundary-handshake-ddd-strain-rate-to-quasi-static-fem): OpenDiS exports \(\tau(\gamma)\) and \(\rho(\gamma)\); here the power-law exponent \(m\) bridges DDD timestep to lab grip speed before crystal plasticity FEM inherits the curve. The [prologue reading compass row for Handshake 4a](../prologue/00-many-scales.md#reading-compass-two-clocks-on-one-wire) names this stitch when Acts III–IV diverge from mathematical order; the [preface row 14 skill checkpoint](../preface.md#skill-navigation-row-14) lists all four competence steps; the [sensitivity derivation worksheet](#worked-example-sensitivity-ranks) below proves why a 10% error in \(m\) shifts macro flow stress by 5–15%.
...
|-----------|--------|----------------|--------|
| **All** — orchestrated chain {#script-audit-trail-orchestrated-chain} | [`parse_multiscale_workflow.sh`](../../scripts/parse_multiscale_workflow.sh) | `cu.foundation/` + `cht_wire.conf` | `multiscale_export.yaml` linking Handshakes 1–4b; phonon lifetime at converged \(T_w\) |
| 1 — DFT → FEM {#script-audit-trail-h1-dft-fem} | [`parse_dft_workflow.sh`](../../scripts/parse_dft_workflow.sh) | `cu.foundation/` folder | `foundation_export.yaml` with \(C_{ij}\), Voigt \(E\), \(\nu\), optional `md_phonon_dos:`, `phonon_lifetime:`, `ddd_rate_extrapolation:` |
| 1 — elastic only {#script-audit-trail-h1-elastic} | [`parse_elastic.sh`](../../scripts/parse_elastic.sh) | six `pw.x` strain logs in `cu.elastic/` | `C11`, `C12`, `C44`, \(B\), \(G\) |
| 2 — Joule ↔ CHT {#script-audit-trail-h2-cht} | [`parse_cht.sh`](../../scripts/parse_cht.sh) | wire geometry + load config (`cht_wire.conf`) | `cht_export.yaml` with \(T_w\), flux balance, iteration count |
| 3 — Thermal → FEM {#script-audit-trail-h3-alpha} | [`parse_alpha.sh`](../../scripts/parse_alpha.sh) | `cu.phonon/a_vs_T.dat` from quasiharmonic scan | `alpha_export.yaml` with \(\alpha\), \(\varepsilon_{\text{th}}\), fixed-grip \(\sigma_{\text{th}}\) |
| 4 — GSF → DDD {#script-audit-trail-gsf} | [`parse_gsf.sh`](../../scripts/parse_gsf.sh) | `gsf_cu111.dat` from DFT sweep or metadynamics | `gsf_export.yaml` with \(\gamma_{\text{sf}}\), partial separation |
| 4a — DDD → FEM rate {#script-audit-trail-h4a-rate} | [`parse_rate.sh`](../../scripts/parse_rate.sh) | `ddd_tau_vs_rate.dat` from OpenDiS sweep | `rate_export.yaml` with \(\tau_{\text{flow}}\) extrapolated to lab rate |
| 4b — FE² at notch {#script-audit-trail-h4b-fe2} | [`parse_fe2.sh`](../../scripts/parse_fe2.sh) | `fe2_notch_comparison.dat` from macro/DDD run | `fe2_export.yaml` with uplift vs crystal plasticity |
| MD — phonon DOS {#script-audit-trail-md-vacf} | [`parse_vacf.sh`](../../scripts/parse_vacf.sh) | `phonon_dos_md.dat` from NVT VACF | `vacf_export.yaml` with acoustic peak vs DFT LA |
| MD — phonon lifetime {#script-audit-trail-md-lifetime} | [`parse_lifetime.sh`](../../scripts/parse_lifetime.sh) | `phonon_lifetime.dat` or `phonon_lifetime_vs_T.dat` | `lifetime_export.yaml` with LA \(\tau_n\) (optionally vs \(T\)) for mobility drag |
| 4 — replica MD {#script-audit-trail-md-wham} | [`parse_wham.sh`](../../scripts/parse_wham.sh) | replica-exchange histogram | `wham_export.yaml` at target \(T\) |

**Script audit trail ↔ subgraph node ↔ minimal artifact (row-level).** {#script-audit-trail-row-level-audit} Each parser row above maps one-to-one to an [IX.3 epilogue pedigree row](../part09-dft/03-dft-workflows.md#ix3-epilogue-pedigree-table) and an [`act6` / `orch` subgraph node](../appendix/memory-sheet.md#act-vi-baby-picture-me-412-coupling-ladder) — return to the pedigree table when a parser row feels isolated; return here when the pedigree table's **Act VI subgraph node** column needs the script round-trip:

| Script audit trail row | Subgraph node | IX.3 pedigree row | Minimal artifact ↔ [memory sheet subgraph node audit](../appendix/memory-sheet.md#subgraph-node-audit-table) |
|------------------------|---------------|-------------------|-------------------------------------------------------------------------------------------------------------|
| [All — orchestrated chain](#script-audit-trail-orchestrated-chain) | [`orch`: OUT](../appendix/memory-sheet.md#subgraph-node-orch-out) | **All — orchestrated chain** | `multiscale_export.yaml` ↔ [prologue preview step 3](../prologue/00-many-scales.md#row-16-preview-step-3) · [epilogue minimal artifact step 3](#act-vi-foundation-minimal-artifact-step-3) · [memory sheet `orch`: OUT `multiscale_export.yaml`](../appendix/memory-sheet.md#subgraph-node-orch-out) |
| [1 — DFT → FEM](#script-audit-trail-h1-dft-fem) | [`orch`: H1](../appendix/memory-sheet.md#subgraph-node-orch-h1); [`act6`: DFT](../appendix/memory-sheet.md#subgraph-node-act6-dft) | **1 — DFT → continuum** | `foundation_export.yaml` ↔ [prologue preview step 2](../prologue/00-many-scales.md#row-16-preview-step-2) · [epilogue minimal artifact step 2](#act-vi-foundation-minimal-artifact-step-2) · [memory sheet `orch`: H1](../appendix/memory-sheet.md#subgraph-node-orch-h1) · [`act6`: DFT](../appendix/memory-sheet.md#subgraph-node-act6-dft) |
| [1 — elastic only](#script-audit-trail-h1-elastic) | [`act6`: DFT](../appendix/memory-sheet.md#subgraph-node-act6-dft) | **1 — DFT → continuum** (partial) | `cu.elastic/` strain logs ↔ [prologue preview step 2](../prologue/00-many-scales.md#row-16-preview-step-2) · [epilogue minimal artifact step 2](#act-vi-foundation-minimal-artifact-step-2) · [memory sheet `act6`: DFT `cu.elastic/`](../appendix/memory-sheet.md#subgraph-node-act6-dft) |
| [2 — Joule ↔ CHT](#script-audit-trail-h2-cht) | [`orch`: H2](../appendix/memory-sheet.md#subgraph-node-orch-h2) | **2 — Joule → CHT** | `cht_export.yaml` ↔ [prologue preview step 1](../prologue/00-many-scales.md#row-16-preview-step-1) · [step 4](../prologue/00-many-scales.md#row-16-preview-step-4) (\(\Delta T\) pedigree) · [memory sheet `orch`: H2 `cht_export.yaml`](../appendix/memory-sheet.md#subgraph-node-orch-h2) |
| [3 — Thermal → FEM](#script-audit-trail-h3-alpha) | [`orch`: H3](../appendix/memory-sheet.md#subgraph-node-orch-h3); [`act6`: DFT (phonon)](../appendix/memory-sheet.md#subgraph-node-act6-dft) | **3 — thermal → mechanical** | `alpha_cu_300K.dat`, `alpha_export.yaml` ↔ [prologue preview step 1](../prologue/00-many-scales.md#row-16-preview-step-1) · [step 4](../prologue/00-many-scales.md#row-16-preview-step-4) · [memory sheet `orch`: H3](../appendix/memory-sheet.md#subgraph-node-orch-h3) |
| [4 — GSF → DDD](#script-audit-trail-gsf) | [`act6`: DFT (GSF)](../appendix/memory-sheet.md#subgraph-node-act6-dft) | **4b — notch localization** (GSF input) | `gsf_export.yaml` ↔ [prologue preview step 2](../prologue/00-many-scales.md#row-16-preview-step-2) (foundation folder) · [memory sheet `act6`: DFT `cu.gsf/`](../appendix/memory-sheet.md#subgraph-node-act6-dft) |
| [4a — DDD → FEM rate](#script-audit-trail-h4a-rate) | [`orch`: H4a](../appendix/memory-sheet.md#subgraph-node-orch-h4a); [`act6`: DDD](../appendix/memory-sheet.md#subgraph-node-act6-ddd) | **4a — rate hardening** | `rate_export.yaml` ↔ [prologue preview step 1](../prologue/00-many-scales.md#row-16-preview-step-1) · [step 4](../prologue/00-many-scales.md#row-16-preview-step-4) · [memory sheet `orch`: H4a](../appendix/memory-sheet.md#subgraph-node-orch-h4a) · [`act6`: DDD](../appendix/memory-sheet.md#subgraph-node-act6-ddd) |
| [4b — FE² at notch](#script-audit-trail-h4b-fe2) | [`orch`: H4b](../appendix/memory-sheet.md#subgraph-node-orch-h4b) | **4b — notch localization** | `fe2_export.yaml` ↔ [prologue preview step 1](../prologue/00-many-scales.md#row-16-preview-step-1) · [memory sheet `orch`: H4b `fe2_export.yaml`](../appendix/memory-sheet.md#subgraph-node-orch-h4b) |
| [MD — phonon lifetime](#script-audit-trail-md-lifetime) | [`orch`: H4a support](../appendix/memory-sheet.md#subgraph-node-orch-h4a); [`act6`: MD](../appendix/memory-sheet.md#subgraph-node-act6-md) | **4a — rate hardening** (drag at \(T_w\)) | `lifetime_export.yaml` ↔ [prologue preview step 4](../prologue/00-many-scales.md#row-16-preview-step-4) · [epilogue minimal artifact step 4](#act-vi-foundation-minimal-artifact-step-4) · [memory sheet `orch`: H4a lifetime](../appendix/memory-sheet.md#subgraph-node-orch-h4a) · [`act6`: MD](../appendix/memory-sheet.md#subgraph-node-act6-md) |
| [MD — phonon DOS](#script-audit-trail-md-vacf) | [`act6`: MD](../appendix/memory-sheet.md#subgraph-node-act6-md) | Foundation checklist (phonon cross-check) | `vacf_export.yaml` ↔ [prologue preview step 2](../prologue/00-many-scales.md#row-16-preview-step-2) (foundation folder) · [memory sheet `act6`: MD `vacf_export.yaml`](../appendix/memory-sheet.md#subgraph-node-act6-md) |
| [4 — replica MD](#script-audit-trail-md-wham) | [`act6`: MD](../appendix/memory-sheet.md#subgraph-node-act6-md) | [Rows 8–9](../preface.md#skill-navigation-row-8) \(T_w\) pedigree | `wham_export.yaml` ↔ [prologue preview step 4](../prologue/00-many-scales.md#row-16-preview-step-4) (\(T_w\) at converged wall temperature) · [memory sheet `act6`: MD `wham_export.yaml`](../appendix/memory-sheet.md#subgraph-node-act6-md) |

The [memory sheet subgraph node audit](../appendix/memory-sheet.md#subgraph-node-audit-table) **Minimal artifact** column is the per-handshake reverse mirror of this table's fourth column — return there when a parser row's export feels abstract without the subgraph-node archive context; return here when the subgraph node audit's **Minimal artifact ↔ row-level audit** entries need the prologue preview or epilogue minimal-artifact step round-trip. The [IX.3 pedigree ↔ parser step mapping](../part09-dft/03-dft-workflows.md#ix3-pedigree-parser-step-mapping) adds the upstream pedigree and [preface row 16](../preface.md#skill-navigation-row-16) step columns.

Illustrative inputs live under [`fixtures/`](../../fixtures/); verify the chain with `./scripts/test-fixtures.sh` before trusting a new parser version — the same check [preface row 16 step 4](../preface.md#skill-navigation-row-16) names when auditing `delta_T_from_handshake_2` and `target_T_K` in the orchestrated export. For a single command that runs Handshakes 1–4b in dependency order — including Handshake 3 with \(\Delta T\) from the converged CHT loop and phonon lifetime at \(T_w\) rather than 300 K — use [`parse_multiscale_workflow.sh`](../../scripts/parse_multiscale_workflow.sh) and archive the emitted `multiscale_export.yaml` beside the Act VI folder. The [IX.3 pedigree ↔ parser step mapping](../part09-dft/03-dft-workflows.md#ix3-pedigree-parser-step-mapping) maps each row in this table to the epilogue pedigree table and [preface row 16](../preface.md#skill-navigation-row-16) competence steps — return there when the **All — orchestrated chain** row above needs the upstream handshake context. Handshake 3 exports \(\alpha(300\,\text{K})\) from `cu.phonon/a_vs_T.dat` via [`parse_alpha.sh`](../../scripts/parse_alpha.sh) — the same script runs automatically when [`parse_dft_workflow.sh`](../../scripts/parse_dft_workflow.sh) finds phonon data in the foundation folder. When `cu.phonon/phonon_dos_md.dat` is archived beside the DFT phonon folder, the same workflow runs [`parse_vacf.sh`](../../scripts/parse_vacf.sh) and merges acoustic-peak pass/fail into `foundation_export.yaml` under `md_phonon_dos:`; when `cu.phonon/phonon_lifetime.dat` or `phonon_lifetime_vs_T.dat` is present, [`parse_lifetime.sh`](../../scripts/parse_lifetime.sh) merges LA linewidth and lifetime under `phonon_lifetime:` (temperature sweep adds `ln_tau_vs_T_slope` for Handshake 4a drag at elevated \(T\)) — one Act VI audit for elastic constants, quasiharmonic \(\alpha\), stacking-fault energy, MD phonon validation, and acoustic drag pedigree. When `ddd_tau_vs_rate.dat` is archived in the same foundation folder, [`parse_rate.sh`](../../scripts/parse_rate.sh) merges Handshake 4a lab-rate extrapolation under `ddd_rate_extrapolation:` so a single `foundation_export.yaml` carries both electronic-structure and DDD-rate pedigree. Archive `alpha_cu_300K.dat` beside `cu.phonon/` as in [IX.3](../part09-dft/03-dft-workflows.md#thermal-expansion-from-quasiharmonic-phonons-handshake-3-pedigree). Handshake 4a also exports standalone via [`parse_rate.sh`](../../scripts/parse_rate.sh); Handshake 4b audits FE² notch uplift via [`parse_fe2.sh`](../../scripts/parse_fe2.sh). Handshakes 1, 2, 3, 4a, and 4b exports should always cite a script name in the yaml header, the same way SCF logs cite `pw.x` version strings. Return to the [prologue reading compass row 16](../prologue/00-many-scales.md#reading-compass-two-clocks-on-one-wire), [row 16 preview](../prologue/00-many-scales.md#what-you-should-be-able-to-do-after-the-prologue), and [row 16 closing stitch](../prologue/00-many-scales.md#row-16-closing-stitch) when narrative time (Act VI foundation) and mathematical order (Part IX → epilogue) diverge — this table names the per-handshake parsers; [`parse_multiscale_workflow.sh`](../../scripts/parse_multiscale_workflow.sh) runs them in dependency order. The [preface epilogue hinges Act VI row](../preface.md#epilogue-continuity-hinges) names the same **All — orchestrated chain** stitch in competence time; the [workflow exam Act VI row](#what-you-should-be-able-to-do-after-the-book) lists the minimal artifact column this table assumes. The [Act VI reunion paragraph](#lab-act-reunion-six-acts-one-afternoon) and [sensitivity worksheet closing](#worked-example-sensitivity-ranks) (row 16 orchestration stitch) reunite the same chain in workflow time after Handshakes 1–4b are understood individually; the [memory sheet Act VI baby picture](../appendix/memory-sheet.md#act-vi-baby-picture-me-412-coupling-ladder), [IX.3 foundation checklist](../part09-dft/03-dft-workflows.md#ix3-foundation-checklist), and [IX.3 epilogue pedigree table](../part09-dft/03-dft-workflows.md#ix3-epilogue-pedigree-table) (orchestrated chain row) draw the ME 412 coupling ladder in one diagram — return to the [All — orchestrated chain row](#script-audit-trail-orchestrated-chain) above when that pedigree table's closing paragraph sends you here for the parser round-trip.
...
This worksheet is the multiscale analogue of Part IV's mesh refinement log: before trusting the load-cell answer, show which partial derivative controlled it. Return to the [Handshake 3 worked load-cell example](#handshake-3--thermal-strain--mechanical-stiffness-part-vi--iv) above to verify \(\sigma_{\text{th}}\) and the \(\alpha\)-sensitivity table with the same \(\Delta T = 90\,\text{K}\) baseline — upstream numbers come from [IX.3's quasiharmonic \(\alpha\) Lab act](../part09-dft/03-dft-workflows.md#lab-act-quasiharmonic-alpha-handshake-3-pedigree) and [`parse_alpha.sh`](../../scripts/parse_alpha.sh); the [preface row 13 skill checkpoint](../preface.md#skill-navigation-row-13) lists all four thermal steps in competence order; the [prologue row 13 preview](../prologue/00-many-scales.md#what-you-should-be-able-to-do-after-the-prologue) named that stitch before Part I — return there when this worksheet closes the Handshake 3 competence loop; the [Act II reunion paragraph](#lab-act-reunion-six-acts-one-afternoon) (Handshake 2) and [Act III reunion paragraph](#lab-act-reunion-six-acts-one-afternoon) (Handshake 3) and [memory sheet row 13 baby picture](../appendix/memory-sheet.md#continuity-hinges-master-map) state the same stitch in workflow time. Return to the [FE² worked example](#worked-example-fe-at-the-wire-notch-act-v--notch) to verify notch-root uplift — upstream rate extrapolation comes from [preface row 14](../preface.md#skill-navigation-row-14) and Handshake 4a; the [preface row 15 skill checkpoint](../preface.md#skill-navigation-row-15) lists all four notch steps in competence order. The [prologue Handshake 4b preview row](../prologue/00-many-scales.md#what-you-should-be-able-to-do-after-the-prologue) named Act V localization before Part I — return there when this worksheet closes the Handshake 4b competence loop; the [Act V reunion paragraph](#lab-act-reunion-six-acts-one-afternoon) and [memory sheet row 15 baby picture](../appendix/memory-sheet.md#continuity-hinges-master-map) state the same stitch in one breath. When Handshakes 1–4b are understood individually, archive them through the [script audit trail](#script-audit-trail-parse-scripts-handshakes) and run [`parse_multiscale_workflow.sh`](../../scripts/parse_multiscale_workflow.sh) per [preface row 16](../preface.md#skill-navigation-row-16) — the worksheet proves which partial derivative controlled each handshake; the orchestrator proves they ran in dependency order. The [preface Step audit ↔ workflow exam paragraph](../preface.md#skill-navigation-row-16) and [Act VI workflow exam step audit](#act-vi-workflow-exam-step-audit) close the row 16 competence loop bidirectionally — partial-derivative audit first (this worksheet), orchestration second (step audit row 3), pedigree fields third (step audit row 4); return to the [preface Step audit paragraph](../preface.md#skill-navigation-row-16) when this closing stitch sends you there for the preface ↔ workflow exam round-trip. Return to the [Act VI reunion paragraph](#lab-act-reunion-six-acts-one-afternoon) when this closing stitch completes row 16 — the reunion paragraph names the same orchestrated chain in workflow time; the [prologue row 16 preview](../prologue/00-many-scales.md#what-you-should-be-able-to-do-after-the-prologue) named it before Part I — return there when this closing stitch completes the competence loop the preview opened (minimal artifact column ↔ [workflow exam Act VI row](#what-you-should-be-able-to-do-after-the-book)); the [prologue row 16 closing stitch](../prologue/00-many-scales.md#row-16-closing-stitch) reunites the same loop in narrative time when the step audit table passes all four rows; the [IX.3 epilogue pedigree table](../part09-dft/03-dft-workflows.md#ix3-epilogue-pedigree-table) and [pedigree ↔ parser step mapping](../part09-dft/03-dft-workflows.md#ix3-pedigree-parser-step-mapping) map each handshake to foundation archive artifacts before the orchestrator runs; the [script audit trail All — orchestrated chain row](#script-audit-trail-orchestrated-chain) is the parser round-trip mirror of that pedigree table; the [memory sheet one-page recap Act VI column](../appendix/memory-sheet.md#one-page-copper-wire-recap) compresses the same foundation → orchestration chain for index-card review — return there when this closing stitch completes the row 16 orchestration audit.
...
| **II — Warming** | Current on; thermocouple climbs | III–V | Handshake 2: FEM solid ↔ FVM fluid until wall flux matches Joule source ([Handshake 2](#handshake-2--joule-heating--conjugate-heat-transfer-part-iv--v)); [V.4 Picard loop](../part05-fvm/04-navier-stokes-cfd.md#lab-act-extension-two-domain-picard-loop-with-a-1d-fem-solid); [`parse_cht.sh`](../../scripts/parse_cht.sh) → `cht_export.yaml`; converged \(\Delta T = T_w - T_\infty\) feeds Handshake 3 in Act III ([preface row 13 step 1](../preface.md#skill-navigation-row-13)) |
| **III — Pulling** | Displacement ramps; curve linear | II–IV, VI | Handshake 2 → 3: \(\Delta T\) from [V.4 Picard loop](../part05-fvm/04-navier-stokes-cfd.md#lab-act-extension-two-domain-picard-loop-with-a-1d-fem-solid); \(\alpha\Delta T\) from [IX.3 quasiharmonic Lab act](../part09-dft/03-dft-workflows.md#lab-act-quasiharmonic-alpha-handshake-3-pedigree) → fixed-grip \(\sigma_{\text{th}}\) ([Handshake 3](#handshake-3--thermal-strain--mechanical-stiffness-part-vi--iv)); [memory sheet row 13](../appendix/memory-sheet.md#continuity-hinges-master-map); [preface row 13](../preface.md#skill-navigation-row-13); [`parse_alpha.sh`](../../scripts/parse_alpha.sh); \(\mathbf{K}\) from Galerkin; Cauchy stress from virtual work |
| **IV — Hardening** | Curve bends; slip lines appear | VII | Handshake 4a: \(\tau_{\text{lab}}\) from rate extrapolation ([VII.3 rate handshake](../part07-defects/03-polycrystal-and-fem-handoff.md#scale-boundary-handshake-ddd-strain-rate-to-quasi-static-fem)); [memory sheet row 14](../appendix/memory-sheet.md#continuity-hinges-master-map); [preface row 14](../preface.md#skill-navigation-row-14); [`parse_rate.sh`](../../scripts/parse_rate.sh); Taylor \(\tau(\gamma)\) → crystal plasticity → FEM |
| **V — Notch** | Optional stress concentration | VI, VII, VIII | Handshake 4b: FE² vs crystal plasticity at notch root ([VII.3 Step 4](../part07-defects/03-polycrystal-and-fem-handoff.md#step-4--when-offline-calibration-fails-fe-at-the-notch)); [memory sheet row 15](../appendix/memory-sheet.md#continuity-hinges-master-map); [preface row 15](../preface.md#skill-navigation-row-15); [`parse_fe2.sh`](../../scripts/parse_fe2.sh) |
| **VI — Foundation** | Parameters in every input deck | IX → VIII → VII | DFT → EAM → mobility → [`multiscale_export.yaml`](../../scripts/parse_multiscale_workflow.sh); [memory sheet Act VI baby picture](../appendix/memory-sheet.md#act-vi-baby-picture-me-412-coupling-ladder); [preface row 16](../preface.md#skill-navigation-row-16); [one-page recap Act VI column](../appendix/memory-sheet.md#one-page-copper-wire-recap) |
...
Act II in the table is the **Joule-heating stitch** the [prologue row 13 preview row](../prologue/00-many-scales.md#what-you-should-be-able-to-do-after-the-prologue) names before Part I assembles a matrix — current may warm the wire to \(T_w \approx 380\,\text{K}\) while the FEM deck still assumes room-temperature conductivity until the Picard loop converges. [V.4's Picard loop](../part05-fvm/04-navier-stokes-cfd.md#lab-act-extension-two-domain-picard-loop-with-a-1d-fem-solid) and [Handshake 2](#handshake-2--joule-heating--conjugate-heat-transfer-part-iv--v) are the upstream workflows; [preface row 13 step 1](../preface.md#skill-navigation-row-13) closes the competence loop when Act II activates; the [sensitivity derivation worksheet](#worked-example-sensitivity-ranks) (Handshake 2 paragraph) proves why \(h\) and \(\Delta T\) dominate mid-span temperature before \(\alpha\) enters in Act III — do not conflate Handshake 2 (\(\Delta T\)) with Handshake 3 (\(\alpha\Delta T\)).

Act III in the table is the **thermal pre-stress stitch** the [prologue row 13 preview row](../prologue/00-many-scales.md#what-you-should-be-able-to-do-after-the-prologue) names before Part I assembles a matrix — CHT may converge at \(T_w \approx 380\,\text{K}\) while the load cell still cites handbook \(\alpha\) and reads thermal compression on fixed grips. [V.4's Picard loop](../part05-fvm/04-navier-stokes-cfd.md#lab-act-extension-two-domain-picard-loop-with-a-1d-fem-solid) and [IX.3's quasiharmonic \(\alpha\) Lab act](../part09-dft/03-dft-workflows.md#lab-act-quasiharmonic-alpha-handshake-3-pedigree) are the upstream workflows; [memory sheet row 13](../appendix/memory-sheet.md#continuity-hinges-master-map) and [preface row 13](../preface.md#skill-navigation-row-13) close the competence loop when Act III activates; the [sensitivity derivation worksheet](#worked-example-sensitivity-ranks) (Handshake 2–3 chain) proves why \(\alpha\) dominates fixed-grip stress once \(\Delta T\) is set — do not conflate Handshake 2 (\(\Delta T\)) with Handshake 3 (\(\alpha\Delta T\)).

Act IV in the table is the **hardening stitch** the [prologue Handshake 4a preview row](../prologue/00-many-scales.md#what-you-should-be-able-to-do-after-the-prologue) names before Part I assembles a matrix — OpenDiS at \(10^3\,\text{s}^{-1}\) cannot feed a \(10^{-3}\,\text{s}^{-1}\) load cell without power-law extrapolation. [VII.3's rate handshake](../part07-defects/03-polycrystal-and-fem-handoff.md#scale-boundary-handshake-ddd-strain-rate-to-quasi-static-fem) is the upstream workflow; [memory sheet row 14](../appendix/memory-sheet.md#continuity-hinges-master-map) and [preface row 14](../preface.md#skill-navigation-row-14) close the competence loop when Act IV activates; the [sensitivity derivation worksheet](#worked-example-sensitivity-ranks) (Handshake 4a column) proves why direct import overpredicts yield.

Act V in the table is the **notch stitch** the [prologue Handshake 4b preview row](../prologue/00-many-scales.md#what-you-should-be-able-to-do-after-the-prologue) names before Part I assembles a matrix — scalar hardening from Handshake 4a may match bulk flow stress while under-predicting root localization by 10–15%. [VII.3 Step 4](../part07-defects/03-polycrystal-and-fem-handoff.md#step-4--when-offline-calibration-fails-fe-at-the-notch) is the upstream workflow; [memory sheet row 15](../appendix/memory-sheet.md#continuity-hinges-master-map) (baby picture) and [preface row 15](../preface.md#skill-navigation-row-15) close the competence loop when Act V activates — the worksheet closing paragraph above links back to the prologue preview when Handshake 4b is complete.

Act VI in the table is the **foundation prequel** archived in [IX.3's Bridge to the epilogue](../part09-dft/03-dft-workflows.md#bridge-to-the-epilogue) and its [Bridge closing paragraph](../part09-dft/03-dft-workflows.md#bridge-to-the-epilogue-closing) — the pedigree checklist and four-handshake export table that map DFT folders onto the multiscale afternoon. When mathematical order (I→IX) and laboratory time diverge, read that bridge first for *what* to archive; return here for *when* each act runs; read the [Bridge closing round-trip](../part09-dft/03-dft-workflows.md#bridge-to-the-epilogue-closing) when this reunion paragraph and Part IX.3 feel disconnected. The [quasiharmonic \(\alpha\) Lab act](../part09-dft/03-dft-workflows.md#lab-act-quasiharmonic-alpha-handshake-3-pedigree) is Act VI's thermal export; [memory sheet row 13](../appendix/memory-sheet.md#continuity-hinges-master-map) names the Handshake 2 → \(\alpha\) → Handshake 3 chain when the load cell reads thermal compression after CHT converges. Before orchestrating Handshakes 1–4b, audit the foundation folder for a **`T_w` column**: illustrative fixtures such as [`fixtures/cu.foundation/`](../../fixtures/cu.foundation/README.md) archive SCF metadata and phonon tables but omit converged wall temperature until [`parse_cht.sh`](../../scripts/parse_cht.sh) emits `cht_export.yaml` beside the DFT deck — when `alpha_export.yaml` or `foundation_export.yaml` lists phonons at 300 K alone, return to the [memory sheet rows 8–9 baby picture](../appendix/memory-sheet.md#rows-8-9-baby-picture-tw-temperature-pedigree), [preface row 8 and row 9 skill checkpoints](../preface.md#skill-navigation-row-8), and [descent continuity hinges rows 8–9](../preface.md#descent-continuity-hinges) before trusting Handshake 3 thermal strain or Handshake 4a drag; the [prologue row 8–9 preview rows](../prologue/00-many-scales.md#what-you-should-be-able-to-do-after-the-prologue) named that contract before Part I. Act VI is also the **[ME 412 coupling ladder reunion](../part09-dft/00-opening.md#the-coupling-ladder-me-412-reunion)** — the Part IX opening draws the full ascent-to-descent diagram; this paragraph is its **workflow-time mirror**: when Handshakes 1–4b exist as separate exports but no orchestrated pedigree links them, run [`parse_multiscale_workflow.sh`](../../scripts/parse_multiscale_workflow.sh) on `cu.foundation/` + `cht_wire.conf` to emit `multiscale_export.yaml` with Handshake 2's \(\Delta T\) feeding Handshake 3 and phonon lifetime at converged \(T_w\) feeding Handshake 4a drag; per-handshake parsers are named in the [script audit trail](#script-audit-trail-parse-scripts-handshakes); the [memory sheet Act VI baby picture](../appendix/memory-sheet.md#act-vi-baby-picture-me-412-coupling-ladder) (foundation → handshake diagram) and [one-page recap Act VI column](../appendix/memory-sheet.md#one-page-copper-wire-recap) compress the same chain for index-card review; [preface row 16](../preface.md#skill-navigation-row-16) and the [prologue reading compass row 16 closing stitch](../prologue/00-many-scales.md#reading-compass-two-clocks-on-one-wire) close the competence loop when Act VI activates — return to those diagrams when this paragraph and the [preface epilogue continuity hinges](../preface.md#epilogue-continuity-hinges) Act VI orchestration row diverge; the [sensitivity worksheet closing](#worked-example-sensitivity-ranks) (row 16 orchestration stitch) proves Handshakes 1–4b ran in dependency order after the partial-derivative audit; upstream handshakes from [rows 8–9](../appendix/memory-sheet.md#rows-8-9-baby-picture-tw-temperature-pedigree) and [rows 13–15](../preface.md#skill-navigation-row-13) must be understood individually first.
...
| Row 13 — Handshake 2 → 3 | Export \(\alpha\) after CHT; verify fixed-grip stress dominates load cell | [`parse_alpha.sh`](../../scripts/parse_alpha.sh) + [sensitivity derivation](#worked-example-sensitivity-ranks); [preface row 13](../preface.md#skill-navigation-row-13) |
| Act IV hardening (Handshake 4a) | Export \(\tau(\gamma)\) from DDD with documented strain rate; extrapolate to lab grip speed | [`parse_rate.sh`](../../scripts/parse_rate.sh) on `ddd_tau_vs_rate.dat` → `rate_export.yaml`; verify against [VII.3 rate handshake](../part07-defects/03-polycrystal-and-fem-handoff.md#scale-boundary-handshake-ddd-strain-rate-to-quasi-static-fem), [preface row 14](../preface.md#skill-navigation-row-14), and [sensitivity derivation](#worked-example-sensitivity-ranks) (Handshake 4a column) |
| Act V notch (Handshake 4b) | Compare FE² root stress to crystal plasticity; decide if offline calibration suffices | [`parse_fe2.sh`](../../scripts/parse_fe2.sh) on `fe2_notch_comparison.dat` → `fe2_export.yaml`; verify against [FE² worked example](#worked-example-fe-at-the-wire-notch-act-v--notch), [preface row 15](../preface.md#skill-navigation-row-15), and [sensitivity derivation](#worked-example-sensitivity-ranks) (Handshake 4b column); upstream rate from [preface row 14](../preface.md#skill-navigation-row-14) |
| Act VI foundation {#act-vi-foundation-workflow-exam-row} | Wire IX → VIII → VII → IV in workflow order; complete [rows 8–9](../preface.md#skill-navigation-row-8) (\(T_w\) pedigree) and [rows 13–15](../preface.md#skill-navigation-row-13) before orchestrating | Pedigree diagram with units at every arrow {#act-vi-foundation-minimal-artifact}: [step 1 {#act-vi-foundation-minimal-artifact-step-1} ↔ preface step 1](../preface.md#row-16-step-1) · [prologue preview step 1](../prologue/00-many-scales.md#row-16-preview-step-1) (Handshakes 2–4b understood; per-handshake: [H2](#script-audit-trail-h2-cht) · [H3](#script-audit-trail-h3-alpha) · [4a](#script-audit-trail-h4a-rate) · [4b](#script-audit-trail-h4b-fe2)) → [step 2 {#act-vi-foundation-minimal-artifact-step-2} ↔ preface step 2](../preface.md#row-16-step-2) · [prologue preview step 2](../prologue/00-many-scales.md#row-16-preview-step-2) (`cu.foundation/`; [H1](#script-audit-trail-h1-dft-fem) · [H1 elastic](#script-audit-trail-h1-elastic)) → [step 3 {#act-vi-foundation-minimal-artifact-step-3} ↔ preface step 3](../preface.md#row-16-step-3) · [prologue preview step 3](../prologue/00-many-scales.md#row-16-preview-step-3) ([`parse_multiscale_workflow.sh`](../../scripts/parse_multiscale_workflow.sh) → `multiscale_export.yaml`; [All — orchestrated chain](#script-audit-trail-orchestrated-chain)) → [step 4 {#act-vi-foundation-minimal-artifact-step-4} ↔ preface step 4](../preface.md#row-16-step-4) · [prologue preview step 4](../prologue/00-many-scales.md#row-16-preview-step-4) (pedigree audit; [H2](#script-audit-trail-h2-cht) \(\Delta T\) → [H3](#script-audit-trail-h3-alpha); [MD lifetime](#script-audit-trail-md-lifetime) at \(T_w\) → [4a](#script-audit-trail-h4a-rate)); [script audit trail row-level audit](#script-audit-trail-row-level-audit) (↔ each per-handshake anchor above); [preface row 16](../preface.md#skill-navigation-row-16) (minimal artifact column ↔ per-step anchors above); [Act VI step audit](#act-vi-workflow-exam-step-audit) (step-by-step ↔ preface row 16); [preface epilogue hinges Act VI row](../preface.md#epilogue-continuity-hinges); [prologue row 16 preview](../prologue/00-many-scales.md#what-you-should-be-able-to-do-after-the-prologue) (competence sketch before Part I — minimal artifact column ↔ this row); [prologue row 16 closing stitch](../prologue/00-many-scales.md#row-16-closing-stitch); [memory sheet Act VI baby picture closing paragraph](../appendix/memory-sheet.md#act-vi-baby-picture-closing); [sources parameter pedigree path](../appendix/sources.md#parameter-pedigree-path-act-vi-reading-order); [IX.3 foundation checklist](../part09-dft/03-dft-workflows.md#ix3-foundation-checklist); [IX.3 Bridge closing](../part09-dft/03-dft-workflows.md#bridge-to-the-epilogue-closing); [IX.3 epilogue pedigree table](../part09-dft/03-dft-workflows.md#ix3-epilogue-pedigree-table); [IX.3 pedigree ↔ parser step mapping](../part09-dft/03-dft-workflows.md#ix3-pedigree-parser-step-mapping); [one-page recap Act VI column](../appendix/memory-sheet.md#one-page-copper-wire-recap); [sensitivity worksheet closing](#worked-example-sensitivity-ranks) (row 16 orchestration stitch); [Act VI reunion paragraph](#lab-act-reunion-six-acts-one-afternoon) |
...
#### Act VI workflow exam step audit {#act-vi-workflow-exam-step-audit}

The [preface row 16 skill checkpoint](../preface.md#skill-navigation-row-16) lists four competence steps before orchestration; the [preface Step audit ↔ workflow exam paragraph](../preface.md#skill-navigation-row-16) maps those steps one-to-one to the rows below; the [prologue row 16 preview Skill row](../prologue/00-many-scales.md#row-16-preview-skill) and [prologue row 16 Skill three-way audit](../prologue/00-many-scales.md#row-16-skill-three-way-audit) are the narrative-time mirrors of this table's **Skill** and **Pass criterion** columns — return there when all four rows pass and the round-trip from preview to step audit is complete. The [prologue row 16 closing stitch](../prologue/00-many-scales.md#row-16-closing-stitch) names the same divergence in narrative time. This table is the **workflow exam mirror** — audit each step against the Act VI foundation row above before archiving `multiscale_export.yaml`:

| Preface row 16 step ↔ [Skill column](../preface.md#skill-navigation-row-16) | Workflow exam check on the copper wire | Pass criterion ↔ [preface Skill](../preface.md#skill-navigation-row-16) |
|----------------------------------------------------------------------------|----------------------------------------|-------------------------------------------------------------------------|
| **1** — Rows 8–9 and 13–15 understood {#act-vi-workflow-exam-step-1} ↔ [preface step 1](../preface.md#row-16-step-1) ↔ [prologue preview step 1](../prologue/00-many-scales.md#row-16-preview-step-1) | Name Handshakes 2, 3, 4a, 4b and the \(T_w\) pedigree without opening new folders ↔ [preface Skill: name handshakes individually before orchestrating](../preface.md#row-16-step-1) | Can explain why 300 K defaults fail after [V.4 CHT](../part05-fvm/04-navier-stokes-cfd.md#lab-act-extension-two-domain-picard-loop-with-a-1d-fem-solid) converged at \(T_w\) ↔ [preface Skill row 1](../preface.md#row-16-step-1) |
| **2** — Foundation folder {#act-vi-workflow-exam-step-2} ↔ [preface step 2](../preface.md#row-16-step-2) ↔ [prologue preview step 2](../prologue/00-many-scales.md#row-16-preview-step-2) | `cu.foundation/` archives DFT, phonon, DDD, and CHT inputs with README pedigree ↔ [preface Skill: archive under `cu.foundation/`](../preface.md#row-16-step-2) | [`parse_dft_workflow.sh`](../../scripts/parse_dft_workflow.sh) emits `foundation_export.yaml`; [IX.3 foundation checklist](../part09-dft/03-dft-workflows.md#ix3-foundation-checklist) six rows pass ↔ [preface Skill row 2](../preface.md#row-16-step-2) · [sources step 1–2](../appendix/sources.md#parameter-pedigree-path-act-vi-reading-order) |
| **3** — Orchestrated chain {#act-vi-workflow-exam-step-3} ↔ [preface step 3](../preface.md#row-16-step-3) ↔ [prologue preview step 3](../prologue/00-many-scales.md#row-16-preview-step-3) | [`parse_multiscale_workflow.sh`](../../scripts/parse_multiscale_workflow.sh) runs Handshakes 1–4b in dependency order ↔ [preface Skill: run full multiscale workflow](../preface.md#row-16-step-3) | `multiscale_export.yaml` exists; [script audit trail All — orchestrated chain row](#script-audit-trail-orchestrated-chain) parsers match artifacts; [subgraph node `orch`: OUT](../appendix/memory-sheet.md#subgraph-node-orch-out) ↔ [preface Skill row 3](../preface.md#row-16-step-3) · [sources step 6](../appendix/sources.md#sources-pedigree-step-6) |
| **4** — Pedigree audit {#act-vi-workflow-exam-step-4} ↔ [preface step 4](../preface.md#row-16-step-4) ↔ [prologue preview step 4](../prologue/00-many-scales.md#row-16-preview-step-4) | Handshake 3 inherits \(\Delta T\) from Handshake 2; Handshake 4a inherits phonon lifetime at \(T_w\) ↔ [preface Skill: verify \(\Delta T\) and lifetime at \(T_w\)](../preface.md#row-16-step-4) | `delta_T_from_handshake_2` and `target_T_K` match converged CHT; `./scripts/test-fixtures.sh` passes ↔ [preface Skill row 4](../preface.md#row-16-step-4) · [IX.3 parser mapping rows 2, 4a](../part09-dft/03-dft-workflows.md#ix3-pedigree-parser-step-mapping) |

Return to the [preface row 16 When-to-pause paragraph](../preface.md#skill-navigation-row-16) when any step fails — or jump directly to the failing [preface step anchor](../preface.md#row-16-step-1) or [prologue preview step anchor](../prologue/00-many-scales.md#row-16-preview-step-1) from the ↔ link in the first column — the **Pass criterion ↔ preface Skill** column above is the bidirectional round-trip when competence-time skill statements and workflow-exam pass/fail checks feel disconnected. The [IX.3 Bridge closing](../part09-dft/03-dft-workflows.md#bridge-to-the-epilogue-closing) and [Act VI reunion paragraph](#lab-act-reunion-six-acts-one-afternoon) are the narrative-time mirrors of this audit table. Step 3 should follow the [sensitivity worksheet closing](#worked-example-sensitivity-ranks) (row 16 orchestration stitch) — partial-derivative audit first, orchestrator second; step 4 audits `delta_T_from_handshake_2` and `target_T_K` against the same fields the [IX.3 pedigree ↔ parser step mapping](../part09-dft/03-dft-workflows.md#ix3-pedigree-parser-step-mapping) and [script audit trail row-level audit](#script-audit-trail-row-level-audit) name for Handshakes 2–4a.
