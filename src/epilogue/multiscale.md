...
| Row 0 — Act I mounting (prologue → Part I) | Write \(\mathbf{K}\mathbf{u}=\mathbf{f}\) with fixed grip DOFs and zero load; explain why BC tags export to every later mesh | \(3 \times 3\) (or larger) sparse \(\mathbf{K}\); \(\mathbf{f}=\mathbf{0}\); annotated BC sketch — [preface row 0](../preface.md#skill-navigation-row-0); [prologue Act I preview](../prologue/00-many-scales.md#prologue-preview-act-i); [prologue row 0 closing stitch](../prologue/00-many-scales.md#row-0-closing-stitch); [Part I mounting Lab act](../part01-linear-algebra/00-opening.md#lab-act-i--mounting); [row 0 closing loop](#row-0-closing-loop) |
...
### Row 0 closing loop (opening hinge reunion) {#row-0-closing-loop}

This subsection is the **downstream half** of [memory sheet row 0](../appendix/memory-sheet.md#continuity-hinges-master-map), the [preface opening continuity hinge](../preface.md#opening-continuity-hinge), and the [preface row 0 skill checkpoint](../preface.md#skill-navigation-row-0). The [prologue Act I preview](../prologue/00-many-scales.md#prologue-preview-act-i) and [prologue row 0 closing stitch](../prologue/00-many-scales.md#row-0-closing-stitch) are the **upstream halves** — return there when the nine-scale ladder feels like a menu before a single matrix is assembled, or when zero-load assembly is conflated with Act III's ramp or Handshake 1's DFT moduli. Row 0 closes the grammar loop that the [prologue reading compass](../prologue/00-many-scales.md#reading-compass-two-clocks-on-one-wire) opened: panoramic scale change is not the first numeric model — Part I.1 is where the four questions get their first finite-dimensional answers.
...
|------|----------------------------------------------------------------------------------|---------------------------------------------------------|-----------------------------------------------------------------------------------|
| 1 | Name nine rungs in order; say what each exports upward | Step 1 — Ladder sketch | One-row ladder: algebra → … → DFT |
| 2 | Fill state / equations / discretization / export for FEM and MD | Step 2 — Four questions | Completed two-row table from the prologue |
| 3 | Sketch \(\mathbf{K}\mathbf{u}=\mathbf{f}\) with fixed DOFs, zero load | Step 3 — Mounting model | Sparse \(\mathbf{K}\); \(\mathbf{f}=\mathbf{0}\); \(\mathbf{u}=\mathbf{0}\) |
| 4 | Explain why boundary tags export to every later handshake | Step 4 — BC discipline | Annotated sketch: fixed nodes, removed translation mode |

When the ladder feels overwhelming, start at the [prologue Bridge](../prologue/00-many-scales.md#bridge-to-part-i) — not at Part IX — and walk forward through [Part I's opening hinge](../part01-linear-algebra/00-opening.md#opening-hinge-prologue-to-part-i) before Act II warming or Handshake 1 moduli enter the story. The [memory sheet row 0](../appendix/memory-sheet.md#continuity-hinges-master-map) (opening hinge stitch) draws the prologue → Part I map; the [Act I expanded reunion paragraph](#reunion-expanded-act-i) states the same stitch in workflow time. Do not conflate panoramic scale change with the first numeric model — mounting exports boundary tags every handshake inherits. Proceed to [Part I's skill table](../part01-linear-algebra/00-opening.md#what-you-should-be-able-to-do-after-part-i) when fields replace vectors in Part II, or to [row 8](../preface.md#skill-navigation-row-8) when Act II warming and Part VIII descent diverge on temperature defaults.

### Row 8 closing loop (\(T_w\) pedigree reunion) {#row-8-closing-loop}

This subsection is the **downstream half** of [memory sheet row 8](../appendix/memory-sheet.md#continuity-hinges-master-map), the [preface Act II temperature pedigree row](../preface.md#epilogue-continuity-hinges), and the [preface row 8 skill checkpoint](../preface.md#skill-navigation-row-8). The [prologue row 8 preview](../prologue/00-many-scales.md#prologue-preview-row-8) and [prologue row 8 closing stitch](../prologue/00-many-scales.md#row-8-closing-stitch) are the **upstream halves** — return there when MD mobility, WHAM, or OpenDiS drag folders cite 300 K after [V.4 CHT](../part05-fvm/04-navier-stokes-cfd.md#lab-act-extension-two-domain-picard-loop-with-a-1d-fem-solid) converged at \(T_w \approx 379\,\text{K}\). Row 8 closes the temperature pedigree loop that Act II opened: Joule heating sets wall temperature once; every finer rung must inherit that \(T_w\) before exporting upward.
...
|------|----------------------------------------------------------------------------------|---------------------------------------------------------|-----------------------------------------------------------------------------------|
| 1 | V.4 Picard loop → `cht_export.yaml` → NVT shear at \(T_w\) | Step 1 — Handshake 2 | [`parse_cht.sh`](../../scripts/parse_cht.sh) → `cht_export.yaml`; converged \(T_w\), \(\Delta T\) |
| 2 | MD mobility at converged \(T_w\), not 300 K | Step 2 — NVT shear at \(T_w\) | `mobility_cu_screw_{T_w}K.yaml` beside `cht_export.yaml` |
| 3 | WHAM reweight at \(T_w \pm 5\,\text{K}\) | Step 3 — WHAM reweight | [`parse_wham.sh`](../../scripts/parse_wham.sh) at `--target-t` from `cht_export.yaml` |
| 4 | OpenDiS mobility cites \(M(\tau, T_w)\) | Step 4 — OpenDiS pedigree | OpenDiS input cites `mobility_cu_screw_{T_w}K.yaml` |

When MD mobility or OpenDiS drag folders cite 300 K after Act II warmed the wire, start at [V.4's Picard loop](../part05-fvm/04-navier-stokes-cfd.md#lab-act-extension-two-domain-picard-loop-with-a-1d-fem-solid) — not at Part VIII — and walk forward through the [VIII.0 descent hinge](../part08-md/00-opening.md#descent-hinge-cores-mobility-and-tw-pedigree) before opening any NVT shear folder. The [memory sheet rows 8–9 baby picture](../appendix/memory-sheet.md#rows-8-9-baby-picture-tw-temperature-pedigree) draws the chain; the [Act II expanded reunion paragraph](#reunion-expanded-act-ii) states the same stitch in workflow time. Do not conflate handbook 300 K drag with Joule-heated \(T_w\) — phonon softening shifts Act IV hardening before Part IX audits the phonon curve. Proceed to [row 9](../preface.md#skill-navigation-row-9) before Handshakes 3 and 4a inherit thermal exports, or to [row 10](../preface.md#skill-navigation-row-10) when foundation folders need composition.

### Row 9 closing loop (phonon audit reunion) {#row-9-closing-loop}

This subsection is the **downstream half** of [memory sheet row 9](../appendix/memory-sheet.md#continuity-hinges-master-map), the [preface electronic phonon audit row](../preface.md#epilogue-continuity-hinges), and the [preface row 9 skill checkpoint](../preface.md#skill-navigation-row-9). The [prologue row 9 preview](../prologue/00-many-scales.md#prologue-preview-row-9) and [prologue row 9 closing stitch](../prologue/00-many-scales.md#row-9-closing-stitch) are the **upstream halves** — return there when EAM matches bulk moduli but `alpha_export.yaml` lacks \(\alpha(T_w)\) beside SCF logs, or when phonon lifetimes still default to 300 K folklore. Row 9 closes the electronic audit loop that row 8 opened at \(T_w\): SCF audits moduli; the phonon audit audits \(\alpha(T_w)\) and \(\tau_{\text{ph}}(T_w)\) at the same wall temperature Act II converged.
...
|------|----------------------------------------------------------------------------------|---------------------------------------------------------|-----------------------------------------------------------------------------------|
| 1 | Upstream [row 8](../prologue/00-many-scales.md#prologue-preview-row-8) complete — `cht_export.yaml` with \(T_w\) | Step 1 — Row 8 complete | `T_wall_K` field from [`parse_cht.sh`](../../scripts/parse_cht.sh) |
| 2 | SCF audits moduli; phonon audit audits thermal exports | Step 2 — SCF audit | `foundation_export.yaml` via [`parse_dft_workflow.sh`](../../scripts/parse_dft_workflow.sh) |
| 3 | Evaluate \(\alpha(T_w)\), not \(\alpha(300\,\text{K})\) alone | Step 3 — \(\alpha(T_w)\) | [`parse_alpha.sh`](../../scripts/parse_alpha.sh) with `--target-t` from `cht_export.yaml` |
| 4 | Interpolate \(\tau_{\text{ph}}(T_w)\) for Handshake 4a drag | Step 4 — \(\tau_{\text{ph}}(T_w)\) | [`parse_lifetime.sh`](../../scripts/parse_lifetime.sh) at converged \(T_w\) |

When EAM fits look credible but the foundation folder lacks an explicit \(T_w\) column, start at the [IX.0 thermal phonon audit table](../part09-dft/00-opening.md#thermal-phonon-audit-at-tw) — not at handbook comparison values — and walk forward through [`parse_alpha.sh`](../../scripts/parse_alpha.sh) before trusting Handshake 3 thermal strain or Handshake 4a drag. The [memory sheet row 9 baby picture](../appendix/memory-sheet.md#rows-8-9-baby-picture-tw-temperature-pedigree) draws the MD → DFT chain; the [epilogue opening hinge from Part IX](#opening-hinge-ix3-to-epilogue) is where those audits compose with Handshakes 2–4a in workflow time. Do not conflate \(\alpha(300\,\text{K})\) handbook comparison with \(\alpha(T_w)\) production export — skip row 9 and row 13 inherits handbook \(\alpha\) beside a converged CHT loop. Proceed to [row 10](../preface.md#skill-navigation-row-10) when foundation exports need composition, or to [row 13](../preface.md#skill-navigation-row-13) for the Handshake 2 → 3 chain once phonon audits are complete.

### Row 10 closing loop (coupling hinge reunion) {#row-10-closing-loop}

This subsection is the **downstream half** of [memory sheet row 10](../appendix/memory-sheet.md#continuity-hinges-master-map), the [preface coupling hinge row](../preface.md#epilogue-continuity-hinges), and the [preface row 10 skill checkpoint](../preface.md#skill-navigation-row-10). The [prologue row 10 preview](../prologue/00-many-scales.md#prologue-preview-row-10) and [prologue row 10 closing stitch](../prologue/00-many-scales.md#row-10-closing-stitch) are the **upstream halves** — return there when Part IX feels complete but DFT, MD, and FEM outputs live in separate directories with no README at the arrows, or when Handshakes 4a and 4b feel undifferentiated on the same `hardening.yaml`. Row 10 closes the coupling loop that rows 8–9 opened at the temperature layer: row 8 sets \(T_w\); row 9 audits \(\alpha(T_w)\) and \(\tau_{\text{ph}}(T_w)\) beside SCF logs; row 10 maps foundation exports onto Handshakes 1–4b before rows 13–16 run individual parsers.
...
|------|----------------------------------------------------------------------------------|---------------------------------------------------------|-----------------------------------------------------------------------------------|
| 1 | Upstream [rows 8–9](../prologue/00-many-scales.md#prologue-preview-row-8) named before Part I | Step 1 — Rows 8–9 complete | `cht_export.yaml`, `alpha_export.yaml`, SCF logs share \(T_w\) column |
...
| 3 | "4a is bulk hardening; 4b is notch localization" — [preview Act IV row](#reunion-preview-act-iv) and [preview Act V row](#reunion-preview-act-v) | Step 3 — 4a/4b split | One sentence distinguishing Act IV knee from Act V notch on `hardening.yaml` |
| 4 | [Epilogue coupling hinge](../epilogue/multiscale.md#opening-hinge-ix3-to-epilogue) before rows 13–16 | Step 4 — Coupling hinge | Annotated workflow sketch linking `cu.foundation/` → Handshakes 1–3 sections |

When foundation exports exist in separate folders but no workflow README sits at the arrows, start at [IX.3's Bridge to the epilogue](../part09-dft/03-dft-workflows.md#bridge-to-the-epilogue) — not at individual handshake parsers — and walk forward through the [IX.3 epilogue pedigree table](../part09-dft/03-dft-workflows.md#ix3-epilogue-pedigree-table) before opening Handshakes 4a and 4b. The [memory sheet row 10 baby picture](../appendix/memory-sheet.md#continuity-hinges-master-map) (coupling hinge stitch paragraph) draws the IX.3 → epilogue map; the [opening hinge from Part IX](#opening-hinge-ix3-to-epilogue) states the same stitch in workflow time. Do not conflate foundation archiving (row 10) with orchestrated export ([row 16](../preface.md#skill-navigation-row-16)) — row 10 names **what composes**; row 16 proves **dependency order** in `multiscale_export.yaml`. Proceed to [row 11](../preface.md#skill-navigation-row-11) when workflow order is unclear, or to [row 13](../preface.md#skill-navigation-row-13) for individual handshake chains once the pedigree table is complete.
