...
|---------------------|----------------------------------------|----------------|
| **1** — Rows 8–9 and 13–15 understood | Name Handshakes 2, 3, 4a, 4b and the \(T_w\) pedigree without opening new folders | Can explain why 300 K defaults fail after [V.4 CHT](../part05-fvm/04-navier-stokes-cfd.md#lab-act-extension-two-domain-picard-loop-with-a-1d-fem-solid) converged at \(T_w\) |
| **2** — Foundation folder | `cu.foundation/` archives DFT, phonon, DDD, and CHT inputs with README pedigree | [`parse_dft_workflow.sh`](../../scripts/parse_dft_workflow.sh) emits `foundation_export.yaml`; [IX.3 foundation checklist](../part09-dft/03-dft-workflows.md#ix3-foundation-checklist) six rows pass |
| **3** — Orchestrated chain | [`parse_multiscale_workflow.sh`](../../scripts/parse_multiscale_workflow.sh) runs Handshakes 1–4b in dependency order | `multiscale_export.yaml` exists; [script audit trail All — orchestrated chain row](#script-audit-trail-orchestrated-chain) parsers match artifacts |
| **4** — Pedigree audit | Handshake 3 inherits \(\Delta T\) from Handshake 2; Handshake 4a inherits phonon lifetime at \(T_w\) | `delta_T_from_handshake_2` and `target_T_K` match converged CHT; `./scripts/test-fixtures.sh` passes |

Return to the [preface row 16 When-to-pause paragraph](../preface.md#skill-navigation-row-16) when any step fails — the [IX.3 Bridge closing](../part09-dft/03-dft-workflows.md#bridge-to-the-epilogue-closing) and [Act VI reunion paragraph](#lab-act-reunion-six-acts-one-afternoon) are the narrative-time mirrors of this audit table. Step 3 should follow the [sensitivity worksheet closing](#worked-example-sensitivity-ranks) (row 16 orchestration stitch) — partial-derivative audit first, orchestrator second; step 4 audits `delta_T_from_handshake_2` and `target_T_K` against the same fields the [IX.3 pedigree ↔ parser step mapping](../part09-dft/03-dft-workflows.md#ix3-pedigree-parser-step-mapping) names for Handshakes 2–4a.
