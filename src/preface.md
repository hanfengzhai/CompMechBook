...
| Step | Skill on the copper wire ↔ [workflow exam Pass criterion](epilogue/multiscale.md#act-vi-workflow-exam-step-audit) | Minimal artifact |
|------|---------------------------------------------------------------------------------------------------------------------|------------------|
| **1** — Rows 8–9 and 13–15 understood {#row-16-step-1} | Name Handshakes 2, 3, 4a, 4b and the \(T_w\) pedigree individually before orchestrating ↔ [Pass: explain why 300 K defaults fail after CHT converged at \(T_w\)](epilogue/multiscale.md#act-vi-workflow-exam-step-1) | Completed skill rows 8–9 and 13–15 or equivalent artifacts ↔ [epilogue Act VI foundation minimal artifact step 1](epilogue/multiscale.md#act-vi-foundation-minimal-artifact-step-1); per-handshake: [H2](epilogue/multiscale.md#script-audit-trail-h2-cht) · [H3](epilogue/multiscale.md#script-audit-trail-h3-alpha) · [4a](epilogue/multiscale.md#script-audit-trail-h4a-rate) · [4b](epilogue/multiscale.md#script-audit-trail-h4b-fe2) |
| **2** — Foundation folder {#row-16-step-2} | Archive DFT, phonon, DDD, and CHT inputs under `cu.foundation/` ↔ [Pass: `parse_dft_workflow.sh` emits `foundation_export.yaml`; IX.3 foundation checklist six rows pass](epilogue/multiscale.md#act-vi-workflow-exam-step-2) | `foundation_export.yaml` via [`parse_dft_workflow.sh`](scripts/parse_dft_workflow.sh) ↔ [epilogue Act VI foundation minimal artifact step 2](epilogue/multiscale.md#act-vi-foundation-minimal-artifact-step-2); [H1](epilogue/multiscale.md#script-audit-trail-h1-dft-fem) · [H1 elastic](epilogue/multiscale.md#script-audit-trail-h1-elastic) |
| **3** — Orchestrated chain {#row-16-step-3} | Run full multiscale workflow in dependency order ↔ [Pass: `multiscale_export.yaml` exists; orchestrated chain parsers match artifacts](epilogue/multiscale.md#act-vi-workflow-exam-step-3) | [`parse_multiscale_workflow.sh`](scripts/parse_multiscale_workflow.sh) on `cu.foundation/` + `cht_wire.conf` → `multiscale_export.yaml`; per-handshake parsers in [epilogue script audit trail](epilogue/multiscale.md#script-audit-trail-parse-scripts-handshakes) ↔ [epilogue Act VI foundation minimal artifact step 3](epilogue/multiscale.md#act-vi-foundation-minimal-artifact-step-3); [All — orchestrated chain](epilogue/multiscale.md#script-audit-trail-orchestrated-chain) |
| **4** — Pedigree audit {#row-16-step-4} | Verify Handshake 3 uses \(\Delta T\) from Handshake 2 (not handbook default) and Handshake 4a uses lifetime at \(T_w\) ↔ [Pass: `delta_T_from_handshake_2` and `target_T_K` match converged CHT; `test-fixtures.sh` passes](epilogue/multiscale.md#act-vi-workflow-exam-step-4) | `multiscale_export.yaml` fields `delta_T_from_handshake_2` and `target_T_K` match converged CHT ↔ [epilogue Act VI foundation minimal artifact step 4](epilogue/multiscale.md#act-vi-foundation-minimal-artifact-step-4); [H2](epilogue/multiscale.md#script-audit-trail-h2-cht) \(\Delta T\) → [H3](epilogue/multiscale.md#script-audit-trail-h3-alpha); [MD lifetime](epilogue/multiscale.md#script-audit-trail-md-lifetime) at \(T_w\) → [4a](epilogue/multiscale.md#script-audit-trail-h4a-rate) |

**Step audit ↔ workflow exam ↔ minimal artifact.** Each step above maps one-to-one to the [epilogue Act VI workflow exam step audit](epilogue/multiscale.md#act-vi-workflow-exam-step-audit) and the [epilogue Act VI foundation minimal artifact](epilogue/multiscale.md#act-vi-foundation-minimal-artifact-step-1) per-step inline anchors — the **Minimal artifact** column ↔ links above are the bidirectional round-trip to those epilogue anchors. [Preface step 1](#row-16-step-1) ↔ [workflow exam row 1](epilogue/multiscale.md#act-vi-workflow-exam-step-1) ↔ [prologue preview step 1](prologue/00-many-scales.md#row-16-preview-step-1) (upstream handshakes understood); [step 2](#row-16-step-2) ↔ [row 2](epilogue/multiscale.md#act-vi-workflow-exam-step-2) ↔ [prologue preview step 2](prologue/00-many-scales.md#row-16-preview-step-2) (foundation folder); [step 3](#row-16-step-3) ↔ [row 3](epilogue/multiscale.md#act-vi-workflow-exam-step-3) ↔ [prologue preview step 3](prologue/00-many-scales.md#row-16-preview-step-3) (orchestrated chain); [step 4](#row-16-step-4) ↔ [row 4](epilogue/multiscale.md#act-vi-workflow-exam-step-4) ↔ [prologue preview step 4](prologue/00-many-scales.md#row-16-preview-step-4) (pedigree audit). The [prologue row 16 preview Skill row](prologue/00-many-scales.md#row-16-preview-skill) ↔ this Skill column ↔ [workflow exam Skill column](epilogue/multiscale.md#act-vi-workflow-exam-step-audit) form the three-way Skill audit — see the [prologue row 16 Skill three-way audit paragraph](prologue/00-many-scales.md#row-16-skill-three-way-audit). Return to [step 1](#row-16-step-1) from [epilogue step 1](epilogue/multiscale.md#act-vi-foundation-minimal-artifact-step-1) when the competence loop closes. The [prologue row 16 preview step sketch](prologue/00-many-scales.md#row-16-preview-steps) is the narrative-time mirror of this four-step audit — read it first when Act VI foundation runs in parallel with Acts I–V. The [epilogue workflow exam Act VI row](epilogue/multiscale.md#what-you-should-be-able-to-do-after-the-book) collects the minimal artifacts; the step audit table is the pass/fail checklist against that row. The [epilogue sensitivity worksheet closing](epilogue/multiscale.md#worked-example-sensitivity-ranks) (row 16 orchestration stitch) is the quantitative audit that should precede step 3 — prove which partial derivative controlled each handshake before running the orchestrator; return to this Step audit paragraph when that worksheet closing sends you here for the preface ↔ workflow exam ↔ prologue preview bidirectional audit. The [prologue row 16 closing stitch](prologue/00-many-scales.md#row-16-closing-stitch) names the same divergence in narrative time — read it first when Act VI foundation runs in parallel with Acts I–V.

**When to pause.** The [prologue row 16 closing stitch](prologue/00-many-scales.md#row-16-closing-stitch) names this divergence in narrative time — read it first when Act VI foundation runs in parallel with Acts I–V but mathematical order says Part IX → epilogue. Then read the [prologue row 16 preview](prologue/00-many-scales.md#what-you-should-be-able-to-do-after-the-prologue) and the [reading compass row 16](prologue/00-many-scales.md#reading-compass-two-clocks-on-one-wire) when narrative time (Act VI foundation) and mathematical order (Part IX → epilogue) diverge on the same afternoon — this skill checkpoint is the competence-time mirror; the compass row is the narrative-time mirror; the [skill navigation row 16 intro](#skill-navigation) above named the ME 412 coupling ladder stitch before this checkpoint's four steps. Read the [Part IX coupling ladder](part09-dft/00-opening.md#the-coupling-ladder-me-412-reunion) (ME 412 reunion) when the ascent grammar (Parts I–III) and the descent pedigree (Parts VII–IX) feel like separate books — row 16 is where the ME 412 coupling ladder reunites them in workflow time. Return to [IX.3's Bridge](part09-dft/03-dft-workflows.md#bridge-to-the-epilogue), the [IX.3 Bridge closing](part09-dft/03-dft-workflows.md#bridge-to-the-epilogue-closing), and the [IX.3 epilogue pedigree table](part09-dft/03-dft-workflows.md#ix3-epilogue-pedigree-table) for the foundation checklist before running the orchestrator. Do not archive individual exports without the orchestrated `multiscale_export.yaml` — the [epilogue Act VI foundation table row](epilogue/multiscale.md#lab-act-reunion-six-acts-one-afternoon), [workflow exam Act VI row](epilogue/multiscale.md#what-you-should-be-able-to-do-after-the-book), [Act VI step audit](epilogue/multiscale.md#act-vi-workflow-exam-step-audit), [Act VI reunion paragraph](epilogue/multiscale.md#lab-act-reunion-six-acts-one-afternoon), [script audit trail](epilogue/multiscale.md#script-audit-trail-parse-scripts-handshakes) closing paragraph (orchestrated chain row), [memory sheet Act VI baby picture closing paragraph](appendix/memory-sheet.md#act-vi-baby-picture-closing), [sources parameter pedigree path](appendix/sources.md#parameter-pedigree-path-act-vi-reading-order), and [one-page recap Act VI column](appendix/memory-sheet.md#one-page-copper-wire-recap) name the same stitch when Act VI activates in workflow time. Verify the chain with `./scripts/test-fixtures.sh` before trusting a new parser version — the same check the [script audit trail](epilogue/multiscale.md#script-audit-trail-parse-scripts-handshakes) closing paragraph cites when auditing `delta_T_from_handshake_2` and `target_T_K`.
