...
When **this Bridge names Born–Oppenheimer but SCF logs and \(\alpha(T_w)\) are still missing** after row 35 restored yaml handoff tables, read the [coarse-graining → electronic audit reunion index](../appendix/sources.md#coarse-graining-electronic-audit-reunion-index-row-36) (row 36) at the [preface coarse-graining → electronic audit thread](../preface.md#reading-rhythm) before opening [IX.1](01-born-oppenheimer.md). Row 37 names the **electronic audit → Born–Oppenheimer** stitch when `cu.relax.out` exists but Hohenberg–Kohn still feels disconnected from Part VIII's EAM archive.
