...
## Worked example: generalized stacking-fault energy (handoff to Part VII) {#worked-example-generalized-stacking-fault-energy-handoff-to-part-vii}
...
| Epilogue handshake | DFT export (this chapter) | Archive artifact | Upstream part that consumes it | Act VI subgraph node ([baby picture](../appendix/memory-sheet.md#act-vi-baby-picture-me-412-coupling-ladder)) | Script audit trail row |
|--------------------|---------------------------|------------------|--------------------------------|-------------------------------------------------------------------------------------------------------------|-------------------------|
| **1 — DFT → continuum** | Voigt \(E\), \(\nu\) from symmetric \(C_{ij}\); relaxed \(a_0\) | `cu.elastic/`, `cu.relax.out` | Part IV elastic step; Part VI \(\mathbb{C}\) | [`act6`: DFT → FEM](../appendix/memory-sheet.md#subgraph-node-act6-dft); [`orch`: **H1**](../appendix/memory-sheet.md#subgraph-node-orch-h1) | [1 — DFT → FEM](../epilogue/multiscale.md#script-audit-trail-h1-dft-fem) |
| **2 — Joule → CHT** | *(Parts IV–V workflow — not a DFT export)* converged \(T_w\), \(h\), \(\Delta T = T_w - T_\infty\) | `cht_export.yaml` via [`parse_cht.sh`](../../scripts/parse_cht.sh) | Sets \(\Delta T\) for Handshakes 3–4a; [dominates \(T_w\) in sensitivity table](../epilogue/multiscale.md#sensitivity-which-handshake-matters-most); [preface hinge row](../preface.md#epilogue-continuity-hinges) | [`orch`: **H2**](../appendix/memory-sheet.md#subgraph-node-orch-h2) | [2 — Joule ↔ CHT](../epilogue/multiscale.md#script-audit-trail-h2-cht) |
| **3 — thermal → mechanical** | Quasiharmonic \(\alpha(T)\) from `ph.x` / phonon DOS; fixed-grip \(\sigma_{\text{th}} = E\alpha\Delta T\) with \(\Delta T\) from Handshake 2 — [ranks first for load-cell stress](../epilogue/multiscale.md#sensitivity-which-handshake-matters-most) | `alpha_cu_300K.dat`, `cu.phonon/a_vs_T.dat` | Part VI return-mapping at heated grip; [Handshake 3 worked example](../epilogue/multiscale.md#handshake-3--thermal-strain--mechanical-stiffness-part-vi--iv) | [`act6`: DFT (phonon)](../appendix/memory-sheet.md#subgraph-node-act6-dft); [`orch`: **H3**](../appendix/memory-sheet.md#subgraph-node-orch-h3) | [3 — Thermal → FEM](../epilogue/multiscale.md#script-audit-trail-h3-alpha) |
| **4a — rate hardening** | Phonon lifetime at \(T_w\) bounds drag on \(m\); [`parse_lifetime.sh`](../../scripts/parse_lifetime.sh) | `cu.phonon/phonon_lifetime_vs_T.dat` + MD VACF cross-check | [Epilogue Handshake 4a](../epilogue/multiscale.md#4a--ddd-strain-rate-to-quasi-static-load-cell-act-iv--hardening); [VII.3 rate handshake](../part07-defects/03-polycrystal-and-fem-handoff.md#scale-boundary-handshake-ddd-strain-rate-to-quasi-static-fem); [sensitivity worksheet](../epilogue/multiscale.md#worked-example-sensitivity-ranks) (4a column) | [`act6`: DDD](../appendix/memory-sheet.md#subgraph-node-act6-ddd); [`orch`: **H4a**](../appendix/memory-sheet.md#subgraph-node-orch-h4a) | [4a — DDD → FEM rate](../epilogue/multiscale.md#script-audit-trail-h4a-rate); [MD — phonon lifetime](../epilogue/multiscale.md#script-audit-trail-md-lifetime) |
| **4b — notch localization** | GSF \(\gamma_{\text{sf}}\) from [slab workflow](#worked-example-generalized-stacking-fault-energy-handoff-to-part-vii); vacancy \(E_f^v\) | `cu.gsf/`, `gsf_cu111.dat` via [`parse_gsf.sh`](../../scripts/parse_gsf.sh) | [Epilogue Handshake 4b](../epilogue/multiscale.md#4b--when-continuum-fails-at-the-notch-md-subdomain-act-v--notch); [VII.3 Step 4 FE²](../part07-defects/03-polycrystal-and-fem-handoff.md#step-4--when-offline-calibration-fails-fe-at-the-notch); upstream 4a from [preface row 14](../preface.md#skill-navigation-row-14) | [`act6`: DFT (GSF)](../appendix/memory-sheet.md#subgraph-node-act6-dft); [`orch`: **H4b**](../appendix/memory-sheet.md#subgraph-node-orch-h4b) | [4 — GSF → DDD](../epilogue/multiscale.md#script-audit-trail-gsf); [4b — FE² at notch](../epilogue/multiscale.md#script-audit-trail-h4b-fe2) |
| **All — orchestrated chain** | *(aggregates rows 1–4b)* | `multiscale_export.yaml` via [`parse_multiscale_workflow.sh`](../../scripts/parse_multiscale_workflow.sh) | Epilogue Act VI reunion; [preface row 16](../preface.md#skill-navigation-row-16) | [`orch`: **OUT**](../appendix/memory-sheet.md#subgraph-node-orch-out) | [All — orchestrated chain](../epilogue/multiscale.md#script-audit-trail-orchestrated-chain) |

Each row above maps to a parser in the [epilogue script audit trail](../epilogue/multiscale.md#script-audit-trail-parse-scripts-handshakes) — the **[All — orchestrated chain row](../epilogue/multiscale.md#script-audit-trail-orchestrated-chain)** is what [`parse_multiscale_workflow.sh`](../../scripts/parse_multiscale_workflow.sh) runs in dependency order to emit `multiscale_export.yaml` from this table's artifacts; the [script audit trail row-level audit](../epilogue/multiscale.md#script-audit-trail-row-level-audit) and [memory sheet subgraph node audit](../appendix/memory-sheet.md#subgraph-node-audit-table) are the bidirectional mirrors of the **Act VI subgraph node** and **Script audit trail row** columns above — return to this pedigree table from those rows when artifact-to-script mapping needs the upstream workflow context.

**Pedigree ↔ parser step mapping (IX.3 → script audit trail → preface row 16).** {#ix3-pedigree-parser-step-mapping} The table below stitches the epilogue pedigree rows above to the [script audit trail](../epilogue/multiscale.md#script-audit-trail-parse-scripts-handshakes) parser rows and the [preface row 16 skill checkpoint](../preface.md#skill-navigation-row-16) / [epilogue Act VI workflow exam step audit](../epilogue/multiscale.md#act-vi-workflow-exam-step-audit) competence steps — use it when the pedigree table and orchestrated chain row feel disconnected:

| IX.3 pedigree row | Act VI subgraph node | Script audit trail parser | Script audit trail row ↔ subgraph node audit | Preface row 16 step ↔ [sources pedigree path step](../appendix/sources.md#parameter-pedigree-path-act-vi-reading-order) | Step audit row |
|-------------------|----------------------|---------------------------|---------------------------------------------|------------------------------------------------------------------------------------------------------------------------|----------------|
| **1 — DFT → continuum** | `act6`: DFT→MD→DDD→FEM; [`orch`: H1](../appendix/memory-sheet.md#subgraph-node-orch-h1) | [`parse_dft_workflow.sh`](../../scripts/parse_dft_workflow.sh), [`parse_elastic.sh`](../../scripts/parse_elastic.sh) | [1 — DFT → FEM](../epilogue/multiscale.md#script-audit-trail-h1-dft-fem) ↔ [`act6`: DFT](../appendix/memory-sheet.md#subgraph-node-act6-dft) · [`orch`: H1](../appendix/memory-sheet.md#subgraph-node-orch-h1) | [Step 2 — Foundation folder](../preface.md#row-16-step-2) ↔ [sources steps 1, 5](../appendix/sources.md#parameter-pedigree-path-act-vi-reading-order) | Row 2 |
| **2 — Joule → CHT** | [`orch`: H2](../appendix/memory-sheet.md#subgraph-node-orch-h2) | [`parse_cht.sh`](../../scripts/parse_cht.sh) | [2 — Joule ↔ CHT](../epilogue/multiscale.md#script-audit-trail-h2-cht) ↔ [`orch`: H2](../appendix/memory-sheet.md#subgraph-node-orch-h2) | [Step 1](../preface.md#row-16-step-1) + [Step 4](../preface.md#row-16-step-4) (\(\Delta T\) pedigree) ↔ [preface rows 8, 13](../preface.md#skill-navigation-row-8) (orchestration, not foundation-path step) | Rows 1, 4 |
| **3 — thermal → mechanical** | `act6`: DFT (phonon); [`orch`: H3](../appendix/memory-sheet.md#subgraph-node-orch-h3) | [`parse_alpha.sh`](../../scripts/parse_alpha.sh) | [3 — Thermal → FEM](../epilogue/multiscale.md#script-audit-trail-h3-alpha) ↔ [`orch`: H3](../appendix/memory-sheet.md#subgraph-node-orch-h3) | [Step 1](../preface.md#row-16-step-1) + [Step 4](../preface.md#row-16-step-4) ↔ [preface row 13](../preface.md#skill-navigation-row-13) · [sources step 1](../appendix/sources.md#parameter-pedigree-path-act-vi-reading-order) (phonon export) | Rows 1, 4 |
| **4a — rate hardening** | [`act6`: DDD](../appendix/memory-sheet.md#subgraph-node-act6-ddd); [`orch`: H4a](../appendix/memory-sheet.md#subgraph-node-orch-h4a) | [`parse_rate.sh`](../../scripts/parse_rate.sh), [`parse_lifetime.sh`](../../scripts/parse_lifetime.sh) | [4a — DDD → FEM rate](../epilogue/multiscale.md#script-audit-trail-h4a-rate) · [MD lifetime](../epilogue/multiscale.md#script-audit-trail-md-lifetime) ↔ [`orch`: H4a](../appendix/memory-sheet.md#subgraph-node-orch-h4a) | [Step 1](../preface.md#row-16-step-1) + [Step 4](../preface.md#row-16-step-4) (\(\tau_{\text{ph}}\) at \(T_w\)) ↔ [sources steps 3, 4](../appendix/sources.md#parameter-pedigree-path-act-vi-reading-order) · [preface row 14](../preface.md#skill-navigation-row-14) | Rows 1, 4 |
| **4b — notch localization** | `act6`: DFT (GSF); [`orch`: H4b](../appendix/memory-sheet.md#subgraph-node-orch-h4b) | [`parse_fe2.sh`](../../scripts/parse_fe2.sh), [`parse_gsf.sh`](../../scripts/parse_gsf.sh) | [4b — FE² at notch](../epilogue/multiscale.md#script-audit-trail-h4b-fe2) · [GSF](../epilogue/multiscale.md#script-audit-trail-gsf) ↔ [`orch`: H4b](../appendix/memory-sheet.md#subgraph-node-orch-h4b) | [Step 1](../preface.md#row-16-step-1) (understood) ↔ [sources step 1](../appendix/sources.md#parameter-pedigree-path-act-vi-reading-order) (GSF) · [preface row 15](../preface.md#skill-navigation-row-15) | Row 1 |
| **All — orchestrated chain** | [`orch`: OUT](../appendix/memory-sheet.md#subgraph-node-orch-out) | [`parse_multiscale_workflow.sh`](../../scripts/parse_multiscale_workflow.sh) | [All — orchestrated chain](../epilogue/multiscale.md#script-audit-trail-orchestrated-chain) ↔ [`orch`: OUT](../appendix/memory-sheet.md#subgraph-node-orch-out) | [Step 3 — Orchestrated chain](../preface.md#row-16-step-3) ↔ [sources step 6](../appendix/sources.md#parameter-pedigree-path-act-vi-reading-order) | Row 3 |

**Three-way round-trip audit.** Each row above stitches [IX.3 epilogue pedigree table](#ix3-epilogue-pedigree-table) → [script audit trail row-level audit](../epilogue/multiscale.md#script-audit-trail-row-level-audit) → [memory sheet subgraph node audit](../appendix/memory-sheet.md#subgraph-node-audit-table) → [epilogue Act VI foundation minimal artifact](../epilogue/multiscale.md#act-vi-foundation-minimal-artifact) → [prologue row 16 preview steps](../prologue/00-many-scales.md#row-16-preview-steps) → [sources parameter pedigree path](../appendix/sources.md#parameter-pedigree-path-act-vi-reading-order). Return to the **Preface row 16 step ↔ sources pedigree path step** column when foundation-path reading order and orchestration steps feel disconnected; return to the **Script audit trail row ↔ subgraph node audit** column when any one leg feels isolated.

Return to the [All — orchestrated chain row](../epilogue/multiscale.md#script-audit-trail-orchestrated-chain) for the parser inputs and yaml exports this mapping assumes; the [memory sheet Act VI baby picture](../appendix/memory-sheet.md#act-vi-baby-picture-me-412-coupling-ladder) draws the `act6` and `orch` subgraphs this table names row-by-row — return there when pedigree rows feel abstract without the foundation → orchestration diagram; the [sensitivity worksheet closing](../epilogue/multiscale.md#worked-example-sensitivity-ranks) (row 16 orchestration stitch) is the quantitative audit that should precede step audit row 3; the [prologue row 16 closing stitch](../prologue/00-many-scales.md#row-16-closing-stitch) reunites the loop in narrative time when step audit rows 1–4 all pass; the [IX.3 Bridge closing round-trip](#bridge-to-the-epilogue-closing) is the Part IX mirror of that prologue stitch when mathematical order and laboratory time diverge on the same afternoon.

The [preface epilogue hinges Act VI row](../preface.md#epilogue-continuity-hinges) names the same stitch in competence time; the [prologue row 16 preview](../prologue/00-many-scales.md#what-you-should-be-able-to-do-after-the-prologue) (minimal artifact column) and [workflow exam Act VI row](../epilogue/multiscale.md#what-you-should-be-able-to-do-after-the-book) (same minimal artifact column) sketch and audit the competence loop bidirectionally; the [memory sheet one-page recap Act VI column](../appendix/memory-sheet.md#one-page-copper-wire-recap) compresses it for index-card review; the [sensitivity worksheet closing](../epilogue/multiscale.md#worked-example-sensitivity-ranks) (row 16 orchestration stitch) proves Handshakes 1–4b ran in dependency order after the partial-derivative audit — return there from the one-page recap when the Act VI column feels abstract without the quantitative audit trail.

**Handshake 4 split (4a bulk vs 4b notch).** Rows 4a and 4b share the same `hardening.yaml` archive but answer different Acts on the prologue wire: **4a** extrapolates DDD strain rate to lab grip speed for Act IV hardening ([epilogue 4a](../epilogue/multiscale.md#4a--ddd-strain-rate-to-quasi-static-load-cell-act-iv--hardening)); **4b** activates only when the optional notch demands FE² at the root ([epilogue 4b](../epilogue/multiscale.md#4b--when-continuum-fails-at-the-notch-md-subdomain-act-v--notch)). Complete 4a's rate pedigree before opening 4b — scalar \(H\) from bulk calibration is necessary but not sufficient for notch localization. The [epilogue opening hinge](../epilogue/multiscale.md#opening-hinge-ix3-to-epilogue) maps these exports onto the wire specimen in workflow time.
...
Read the epilogue's [**opening hinge from Part IX**](../epilogue/multiscale.md#opening-hinge-ix3-to-epilogue) first for the export table mapped onto the wire specimen — including the Handshake 4a/4b split for Act IV hardening vs Act V localization; then the general coupling patterns (partitioned CHT, sequential homogenization, concurrent FE²). When mathematical order (I→IX) and laboratory time diverge, the [preface six-act reunion row](../preface.md#epilogue-continuity-hinges) and the epilogue's [**Lab act reunion: six acts, one afternoon**](../epilogue/multiscale.md#lab-act-reunion-six-acts-one-afternoon) reunite them — Act VI (this chapter's foundation archive and [quasiharmonic \(\alpha\) Lab act](#lab-act-quasiharmonic-alpha-handshake-3-pedigree)) supplies every parameter Acts I–V already consumed. That reunion section links back here for the pedigree checklist; [Part IX opening skill checkpoint](../part09-dft/00-opening.md#what-you-should-be-able-to-do-after-part-ix) lists [`parse_alpha.sh`](../../scripts/parse_alpha.sh) as the IX.3 competence artifact beside `parse_elastic.sh`. Complete [preface row 14](../preface.md#skill-navigation-row-14) (Handshake 4a rate extrapolation) before [row 15](../preface.md#skill-navigation-row-15) (Handshake 4b FE² notch) when the optional micro-notch is present.

{#bridge-to-the-epilogue-closing} **Bridge closing round-trip.** The [epilogue Act VI reunion paragraph](../epilogue/multiscale.md#lab-act-reunion-six-acts-one-afternoon) is the workflow-time mirror of this Bridge section — read that paragraph when the foundation checklist and epilogue pedigree table above feel like isolated audit rows without laboratory time; return here when the reunion paragraph names Act VI foundation but the six-row scale-boundary handshake table is missing. The [prologue row 16 closing stitch](../prologue/00-many-scales.md#row-16-closing-stitch) is the narrative-time mirror of this Bridge closing — read it first when Act VI foundation runs in parallel with Acts I–V but mathematical order says Part IX → epilogue; return here when that closing stitch sends you back to the foundation checklist and [pedigree ↔ parser step mapping](#ix3-pedigree-parser-step-mapping). The [preface row 16 skill checkpoint](../preface.md#skill-navigation-row-16) and [epilogue workflow exam Act VI step audit](../epilogue/multiscale.md#act-vi-workflow-exam-step-audit) map the same four competence steps to this Bridge's artifacts; the [memory sheet Act VI baby picture closing paragraph](../appendix/memory-sheet.md#act-vi-baby-picture-closing) and [sources parameter pedigree path](../appendix/sources.md#parameter-pedigree-path-act-vi-reading-order) draw the foundation → orchestration diagram when both clocks diverge.
