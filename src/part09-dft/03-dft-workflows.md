...
This Lab act is the **upstream half** of epilogue [Handshake 3](../../epilogue/multiscale.md#handshake-3--thermal-strain--mechanical-stiffness-part-vi--iv): archive quasiharmonic \(\alpha(300\,\text{K})\) with the same pedigree as \(C_{ij}\) before the load cell reads fixed-grip thermal stress. The [preface Joule heat → fixed-grip stress row](../../preface.md#epilogue-continuity-hinges) and [preface row 13 skill checkpoint](../../preface.md#skill-navigation-row-13) list Handshake 2 → this Lab act → Handshake 3 in workflow order; [memory sheet row 13](../../appendix/memory-sheet.md#continuity-hinges-master-map) is the one-page map when CHT converges but handbook \(\alpha\) still sits in the FEM deck. When mathematical order (Part IX after Parts IV–V) and narrative time (Act III on the load cell) diverge, read [IX.3's Bridge Act III reunion paragraph](#bridge-act-iii-reunion-handshake-3) beside the epilogue's [Act III reunion paragraph](../../epilogue/multiscale.md#act-iii-reunion-handshake-3) — the Lab act supplies \(\alpha\); the bridge closes the same stitch the [prologue row 13 preview](../../prologue/00-many-scales.md#prologue-row-13-preview) named before Part I.
...
**Act III reunion (Handshake 3 pedigree).** {#bridge-act-iii-reunion-handshake-3}

The [prologue row 13 preview](../../prologue/00-many-scales.md#prologue-row-13-preview) named Handshakes 2 and 3 before Part I; this chapter's [quasiharmonic \(\alpha\) Lab act](#lab-act-quasiharmonic-alpha-handshake-3-pedigree) is the **upstream half** of the Act III stitch the epilogue reunites in workflow time. Handshake 2 (Parts IV–V CHT) sets \(\Delta T\); this Lab act sets \(\alpha\); Handshake 3 (Part VI fixed-grip return mapping) sets \(\alpha\Delta T\) on the load cell. When narrative time (Act III — displacement ramps on heated grips) and mathematical order (Part IX after Parts IV–V) diverge on the same afternoon, read the epilogue's [Act III reunion paragraph](../../epilogue/multiscale.md#act-iii-reunion-handshake-3) beside this bridge — it states the thermal pre-stress stitch in one breath; the [sensitivity derivation worksheet](../../epilogue/multiscale.md#worked-example-sensitivity-ranks) (Handshake 3 paragraph) proves why \(\alpha\) dominates fixed-grip stress once \(\Delta T\) is set. Do not conflate Handshake 2 (\(\Delta T\)) with Handshake 3 (\(\alpha\Delta T\)) — [memory sheet row 13](../../appendix/memory-sheet.md#continuity-hinges-master-map) is the one-page map; [`parse_alpha.sh`](../../scripts/parse_alpha.sh) is the competence artifact that closes the loop from this chapter to the load cell.
