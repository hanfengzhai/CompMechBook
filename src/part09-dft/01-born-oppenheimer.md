...
When **this Bridge names Kohn–Sham but Born–Oppenheimer still feels like a new course** after row 35 restored yaml handoff tables, read the [coarse-graining → electronic audit reunion index](../appendix/sources.md#coarse-graining-electronic-audit-reunion-index-row-36) (row 36) at the [preface coarse-graining → electronic audit thread](../preface.md#reading-rhythm) before re-reading [IX.0](00-opening.md#bridge). Row 36 reunites **handoff tables with SCF audit gates**; row 35 remains the audit when the [pedigree checklist](../part08-md/03-ab-initio-and-coarse-graining.md#pedigree-checklist-before-the-epilogue) still lacks DFT evidence columns.

When **Murnaghan \(a_0\) and \(B_0\) exist but IX.2 still feels like standalone quantum chemistry** — BO/HK theorems are understood but \(\mathbf{H}[\rho]\mathbf{c}_n = \epsilon_n \mathbf{S}\mathbf{c}_n\) feels disconnected from Part I eigenvalues — read the [Born–Oppenheimer → Kohn–Sham reunion index](../appendix/sources.md#born-oppenheimer-kohn-sham-reunion-index-row-38) (row 38) and the [preface Born–Oppenheimer → Kohn–Sham thread](../preface.md#reading-rhythm) before opening [IX.2](02-kohn-sham.md). Row 38 reunites **BO/HK theorems with SCF implementation**; row 37 remains the audit when SCF logs exist but theorem vocabulary is still missing.

When **the cutoff-sweep Lab act converged but IX.3 still feels like standalone coursework** — SCF is understood but `cu.foundation/` has no archived deck — read the [Kohn–Sham → DFT workflows reunion index](../appendix/sources.md#kohn-sham-dft-workflows-reunion-index-row-39) (row 39) at the [preface reading rhythm](../preface.md#reading-rhythm) before opening [IX.3](03-dft-workflows.md). Row 39 names the **Kohn–Sham → DFT workflows** stitch when the fixed-point loop closed but reproducible archives still feel like a new course.
