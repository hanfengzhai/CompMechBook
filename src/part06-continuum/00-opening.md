...
| CHT export ([V.4 parser checkpoint](../part05-fvm/04-navier-stokes-cfd.md#parser-checkpoint-archive-cht-export-yaml)) | `cht_export.yaml` from [`parse_cht.sh`](../../scripts/parse_cht.sh); converged \(T_w \approx 379\,\text{K}\) (Ra/Nu Lab act) or fixture audit at \(h=15\) | Single wall temperature for both twins; \(\varepsilon_{\text{th}} = \alpha(T_w)\Delta T\) in virtual work | [VI.3](03-variational-elasticity.md); Part VII \(M(\tau, T_w)\) |
