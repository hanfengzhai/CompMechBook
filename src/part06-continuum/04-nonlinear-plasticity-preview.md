...
Turn the page when the mesh is fine enough but the physics still wrong — that is the hinge between continuum and mesoscale. If the return-mapping Lab act above reproduces the load cell knee at one strain rate but fails when the grip ramp slows, archive `viscoplastic.yaml` and descend: the missing physics is in line motion, not in another Gauss point. If J₂ internal variables still feel like fitted scalars rather than stored forest geometry, continue to [VII.1's opening hinge](../part07-defects/01-defect-taxonomy.md#defect-taxonomy-opening-hinge-plasticity-to-forest), where Burgers vector \(\mathbf{b}\) replaces \(\alpha\) as the primary object for the first time since Part VI began.
