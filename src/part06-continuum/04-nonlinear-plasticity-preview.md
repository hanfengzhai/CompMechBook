...
**Intermission → descent reunion (row 32).** If row 31 restored virtual work but \(J_2\) phenomenology and Part VII's dislocation forest still feel like separate subjects, read the [intermission → descent reunion index](../appendix/sources.md#intermission-descent-reunion-index-row-32) — the meta stitch when the knee is computable but unexplained. Start at the [intermission](#intermission-ascent-ends-descent-begins) above (recite the [plot spine one line](#plot-spine-one-line) aloud — row 20 midpoint gate), run the [return-mapping Lab act](#lab-act-return-mapping-on-the-load-cell-knee-act-iv-hardening), read the [three descent signals](#three-signals-to-descend-from-part-vi-to-part-vii), then open [VII.0 Scene](../part07-defects/00-opening.md#scene). Row 32 does not replace row 31 — it reunites **fitted \(H\) with forest density \(\rho\)** when virtual work closed the load cell but cold-drawn history never entered the constitutive story.
