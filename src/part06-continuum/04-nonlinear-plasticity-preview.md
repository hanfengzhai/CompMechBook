...
If you have read linearly since the [preface ascent preview chain](../../preface.md#ascent-preview-chain) and [continuity hinges](../../preface.md#continuity-hinges-ascent-descent), Part VI's [midpoint anchor](00-opening.md#midpoint-ascent-complete-descent-ahead) marked the turn from mathematical climb to scale descent. Whether you walked through [IV.5 Door A](../part04-fem/05-convergence.md#bridge-two-doors-from-here) (FVM cooling in **Act II**) or Door B (FEM straight to continuum), you now share one tensor vocabulary — Cauchy stress, virtual work, return-mapping — and one honest limit: smooth fields cannot carry cold-drawn history forever.

This chapter is the last continuum stop before that hinge closes: it admits that cold-drawn copper, notch roots, and path-dependent hardening **break** the smooth fields FEM and FVM trusted in Parts IV–V. The conjugate heat transfer loop from Part V and the Céa-certified mesh from Part IV still govern **Act II–III** in the linear regime; what changes here is **Act IV**, where the load cell knee asks for history variables no elliptic energy functional supplies.

The [preface descent preview chain](../../preface.md#descent-preview-chain) names what follows — dislocation forest, atomic lattice, electron density — without asking you to leave the wire on the bench. The specimen does not change; only the state variable does. Part VI's J₂ return-mapping loop can fit \(H\) and \(\sigma_{y0}\) from the load cell knee; it cannot explain **why** the curve bent. That explanation lives in Burgers vectors and line motion — the opening scene of [Part VII](../part07-defects/00-opening.md). Read the remainder of this chapter as the continuum's honest farewell: geometric and material nonlinearity exhaust what a field can say before defects take the narrative.
...
Part VII opens on the same bench with the same specimen at the yield point: polished copper showing faint **slip lines** on {111} planes — the visible trace of dislocation motion that J₂ plasticity homogenized into a scalar \(\alpha\). The [intermission above](#intermission-ascent-ends-descent-begins) named the turn; Part VII's [opening Scene](../part07-defects/00-opening.md#scene) picks up the load cell trace where the return-mapping Lab act left off — linear climb, then knee, then hardening — and asks what moves underneath the mesh. Remember also that the drawn wire is not a single crystal: cold drawing leaves a **polycrystal with grain boundaries** that homogenized \(H\) cannot see — the spool of wire in [VII.3](../part07-defects/03-polycrystal-and-fem-handoff.md) is the same specimen at a finer organizational scale. [VII.1](../part07-defects/01-defect-taxonomy.md) names the defect catalog; [VII.2](../part07-defects/02-dislocation-dynamics.md) follows the forest as it moves, multiplies, and tangles under load.
