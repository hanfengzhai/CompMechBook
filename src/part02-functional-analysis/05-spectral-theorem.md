...
## Writings canonical hinge (Part II → Part III) {#writings-canonical-hinge-ii5-to-iii0}

This chapter is the last file in the **Functional Analysis Notes** subtree ([`writings/functional-analysis/`](../../writings/functional-analysis/)); the next page of the continuous book is the **PDE Notes** subtree ([`writings/pde/`](../../writings/pde/)), beginning at [Part III opening](../part03-pdes/00-opening.md). The numbering repeats the ME 412 habit: Part II ends at `05-spectral-theorem.md`; Part III opens at `00-opening.md` with [**Closing the arc from Part II**](../part03-pdes/00-opening.md#closing-the-arc-from-part-ii) — the same four concept-map questions (object, structure, theorem, what breaks), now answered with **equations on \(\Omega\)** instead of abstract spaces alone.

**One continuous sentence (read aloud before Part III):** the tap-test eigenvalues from the Lab act above and the Lax–Milgram bilinear form \(a(u,v)\) are still the copper wire; Part III does not introduce new physics — it writes the **strong and weak PDEs** whose solutions live in the \(H^1\) and \(L^2\) rooms Part II built, starting at the grip corner where classical \(C^2\) smoothness fails.

The unified mdBook [`src/SUMMARY.md`](../SUMMARY.md) syncs these canonical files — edit under `writings/<part>/chapters/`, run `./scripts/sync-writings.sh`, then `mdbook build`. If Part III ever feels like a separate PDE course after this chapter, run `./scripts/sync-writings.sh --check` and read the [II.5 → III.0 analysis–PDE reunion (row 47)](../appendix/sources.md#rows17-46-ii5-iii0-analysis-pde-reunion-index-row-47) — the break is usually skipping this hinge and the [Writings canonical landing](../part03-pdes/00-opening.md#writings-canonical-landing-ii5-to-iii0), not missing PDE prerequisites.
