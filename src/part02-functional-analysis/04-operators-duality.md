...
**Scale-boundary handshake (II.4 → II.5 → Part III).**

| Operator export (this chapter) | Spectral consumer ([II.5](05-spectral-theorem.md)) | PDE consumer (Part III) | Failure mode |
|--------------------------------|-----------------------------------------------------|-------------------------|--------------|
| Dual loads \(\ell \in H^*\) via Riesz | Self-adjoint compact operators | Point forces and Neumann flux in [III.2](../part03-pdes/02-weak-form.md) | Load lumping without weak\* convergence |
| Bounded operator \(A: H \to H\) | Spectral theorem: real eigenvalues, orthogonal modes | \(-\Delta\), heat operator, elasticity bilinear form | Treating convection as self-adjoint without adjoint audit |
| Weak\* convergence \(\ell_N \to \ell\) | Rayleigh–Ritz modal bounds | Lax–Milgram stability under mesh refinement | Midspan displacement oscillating as \(N\) grows |
| Compact embedding \(H^1 \hookrightarrow L^2\) | Eigenvalues accumulate at zero | Rellich–Kondrachov in [III.3](../part03-pdes/03-sobolev-spaces.md) | Assuming \(C^2\) when only \(H^1\) membership holds |
| Galerkin projector \(P_h: H \to V_h\) | Best approximation in energy norm | Part IV assembly ([IV.2](../part04-fem/02-galerkin-assembly.md)) | Different connectivity for thermal vs mechanical DOFs |

The distributed-body-force Lab act above is the operational version of this handshake: nodal weights \(w_i\) must converge weak\* to \(\int (\rho g A) v\, dx\) before Part IV's scatter into \(\mathbf{f}\) is trustworthy. When sensitivity analysis asks how tip displacement responds to a perturbation in grip load, the adjoint solution is the duality pattern this chapter named — the same adjoint-side logic Part V will need for non-symmetric convection operators.

On the copper wire, the stiffness operator from Part I's spring network becomes a differential operator in the limit; its eigenfunctions are standing-wave patterns along the bar, its eigenvalues are squared natural frequencies. Buckling searches for \(\lambda\) where \(\mathbf{K} - \lambda \mathbf{K}_g\) loses invertibility — the same Fredholm logic previewed above. [II.5](05-spectral-theorem.md) states the spectral theorem explicitly and closes Part II with the **well-posedness triangle** that hands off to Part III: strong forms for intuition, weak forms for computation, Sobolev spaces for regularity.

Return to the [prologue](../../prologue/00-many-scales.md): **Act III — Pulling** will ramp grip displacement on the load cell, but the operator cannot trust that curve until the **Galerkin projector** and **dual loads** defined here make mesh refinement honest. Part I's concentrated nodal forces were finite-dimensional shadows of functionals \(\ell(v)\); Part IV's assembly will scatter those loads into \(\mathbf{f}\) only because Riesz representation and weak\* convergence justify the limit. The [preface ascent continuity hinges](../preface.md#ascent-continuity-hinges) name Part II → Part III as the **function spaces → PDEs** turn at [II.5](05-spectral-theorem.md#bridge-to-part-iii); this chapter supplies the dual loads and operator stability that hinge assumes.

Turn the page when operator language feels natural but spectra still look like matrix diagonalization — [II.5](05-spectral-theorem.md) is where infinite-dimensional modes acquire the same Rayleigh–Ritz bounds Part I.3 previewed on the spring chain.
