...
> **V.4 — Act II — Discretization:** Navier–Stokes and conjugate heat transfer set \(T_w\) — the temperature every descent rung inherits.
...
## Writings canonical hinge (Part V → Part VI) {#writings-canonical-hinge-v4-to-vi0}

This chapter is the last file in the **FVM Notes** subtree ([`writings/fvm/`](../../writings/fvm/)); the next page of the continuous book is the **Continuum Mechanics Notes** subtree ([`writings/continuum/`](../../writings/continuum/)), beginning at [Part VI opening](../part06-continuum/00-opening.md). Part V ends at `04-navier-stokes-cfd.md`; Part VI opens at `00-opening.md` with [**Writings canonical landing**](../part06-continuum/00-opening.md#writings-canonical-landing-v4-to-vi0) and [**Closing the arc from Parts IV and V**](../part06-continuum/00-opening.md#closing-the-arc-from-parts-iv-and-v) — the same copper wire, same archived \(T_w\) from [`fixtures/cht_export.yaml`](../../fixtures/cht_export.yaml), but the load cell's force–displacement curve now acquires **Cauchy stress and virtual work** behind the sparse systems Parts IV and V already solved.

**One continuous sentence (read aloud before Part VI):** the Picard loop and face-flux balances above were never a separate CFD finale — they are the **transport half** of Schematic 14 that reunites with Galerkin assembly in continuum tensor language before the wire yields in Acts III–IV.

The unified mdBook [`src/SUMMARY.md`](../SUMMARY.md) syncs these canonical files — edit under `writings/<part>/chapters/`, run `./scripts/sync-writings.sh`, then `mdbook build`. If Part VI ever feels like a separate elasticity course after this chapter, run `./scripts/sync-writings.sh --check` and read the [V.4 → VI.0 FVM–continuum reunion (row 50)](../appendix/sources.md#rows17-49-v4-vi0-fvm-continuum-reunion-index-row-50) — the break is usually skipping this hinge and the [Writings canonical landing](../part06-continuum/00-opening.md#writings-canonical-landing-v4-to-vi0), not missing tensor calculus prerequisites. If you took [IV.5 Door B](../part04-fem/05-convergence.md#bridge-two-doors-from-here) and skipped Part V on first reading, land directly at Part VI opening — row 50 still applies when **FEM stiffness feels disconnected from \(\boldsymbol{\sigma}\)**.
...
Turn the page when sparse linear systems and face fluxes feel like the whole story — continuum mechanics is what makes \(\mathbf{K}\mathbf{U}=\mathbf{F}\) a force-balance statement rather than an array exercise. If the CHT loop converges but the load cell reading still lacks a stress tensor behind it, Part VI is the missing chapter, not another flux limiter. In the canonical tree, continue at [`writings/continuum/chapters/00-opening.md`](../../writings/continuum/chapters/00-opening.md) — read it as the **midpoint reunion** after Door A, not a course change. The [Writings canonical hinge](#writings-canonical-hinge-v4-to-vi0) above names the file boundary: FVM Notes end here; [Elasticity Notes](https://hanfengzhai.github.io/file/elasticity_notes.pdf) begin on the **same wire** where the twin Schematic 14 ladders meet in Cauchy stress.
