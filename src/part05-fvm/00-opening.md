...
If you arrived through [IV.5 Door A](../part04-fem/05-convergence.md#bridge-two-doors-from-here), Part IV's closing checkpoint just certified Céa's lemma on the solid mesh and named two continuations. **Door A** is this part — the air around the wire, not another stiffness matrix. The finite volume method discretizes flux balances directly. When the copper wire heats the surrounding air, convection carries energy away at a rate governed by the Navier–Stokes equations and boundary-layer physics that FEM conduction alone cannot resolve — even when Robin BC \(q = h(T_w - T_\infty)\) on the wire surface made the solid run look complete. Part V builds the FVM toolkit: integral conservation laws, one-dimensional schemes, Riemann solvers for shocks and contact discontinuities, and the CFD pipeline for incompressible and compressible flow.
