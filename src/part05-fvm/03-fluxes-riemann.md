...
**Scale-boundary handshake (V.3 → V.4 → Part VI).**

| Riemann export (this chapter) | Navier–Stokes consumer ([V.4](04-navier-stokes-cfd.md)) | Wire lab act | Failure mode |
|-------------------------------|-----------------------------------------------------------|--------------|--------------|
| Godunov/Roe/HLLC face flux \(F^*_{i+1/2}\) | Viscous + heat fluxes regularize shocks | Act II low-Re cooling around wire | Entropy violation (expansion shock) |
| MUSCL + limiters for \(O(\Delta x^2)\) | Reynolds number: advection vs diffusion | Boundary-layer Nusselt on heated wire | \(\rho<0\) or \(p<0\) without limiters |
| Sod shock-tube patch test | Natural-convection CHT loop with Part IV FEM | Thermocouple stops climbing when flux matches | Wrong ghost cells at wire surface |
| CFL limit from wave speeds | SIMPLE/PISO pressure–velocity splitting | Timestep audit before Picard coupling | Violating CFL on stiff source terms |
| Discrete conservation at faces | Robin BC \(-k\partial T/\partial n = h(T_w-T_\infty)\) | [IV.5 Door A](../part04-fem/05-convergence.md#bridge-two-doors-from-here) discretization fork | Flux/temperature mismatch at solid–fluid interface |

The [preface ascent continuity hinges](../preface.md#ascent-continuity-hinges) name [IV.5](../part04-fem/05-convergence.md#bridge-two-doors-from-here) / [V.4](04-navier-stokes-cfd.md#bridge-to-part-vi) as the **discretization fork → continuum** turn; this chapter is the hyperbolic rung on Door A — conservation at faces before viscosity and pressure coupling enter in Navier–Stokes.

Turn the page when Sod passes but the wire still runs hot — that is the signal to add viscosity, conduction, and the shared continuum vocabulary Part VI will name. If the shock tube is exact but wall temperature disagrees with the solid mesh, the fault is the **interface handshake**, not another Riemann solver — [V.4](04-navier-stokes-cfd.md) is where flux balances meet FEM conduction in Act II.
