...
## Closing the arc from VII.2 (DDD complete, homogenization enters) {#polycrystal-opening-hinge-ddd-to-homogenization}

If you walked through [VII.2's Bridge](02-dislocation-dynamics.md#bridge-to-vii3) and the [Lab act: read the hardening bend from forest density](02-dislocation-dynamics.md#lab-act-read-the-hardening-bend-from-forest-density-act-iv), the copper wire has a converged OpenDiS run — segment network, \(\tau(\gamma)\), \(\rho(\gamma)\), link-length histograms, Taylor fit with MD-calibrated mobility — but **Act IV** on the spool still disagrees with experiment because the wire is not one orientation. [Part VII opening — Chapter guide](../part07-defects/00-opening.md#chapter-guide) lists VII.3 as the rung where polycrystal homogenization enters; [VII.2's opening hinge](02-dislocation-dynamics.md#ddd-opening-hinge-taxonomy-to-motion) replaced static \(\rho\) labels with Peach–Köhler motion; this chapter is the **third and closing rung inside Part VII** — crystal plasticity and FEM handoff before Part VIII resolves line cores.

| VII.2 (single-crystal DDD on the wire) | VII.3 (polycrystal homogenization on the wire) |
|----------------------------------------|------------------------------------------------|
| One slip system, one crystal orientation | Thousands of grains twisted by drawing dies |
| Segment network under Peach–Köhler forces | Texture, Schmid factors, grain-boundary barriers |
| \(\tau(\gamma)\), \(\rho(\gamma)\) as simulation output | Internal state variables on DAMASK/FEM Gauss points |
| Taylor fit \(\tau = \tau_0 + \alpha\mu b\sqrt{\rho}\) | Voce / crystal plasticity \(h_0\), \(g_{\text{sat}}\) calibration |
| OpenDiS export yaml for one RVE | OpenDiS → DAMASK → polycrystal FEM pipeline |
| DDD at \(\dot\varepsilon \sim 10^3\,\text{s}^{-1}\) | Rate extrapolation to lab \(\dot\varepsilon \sim 10^{-3}\,\text{s}^{-1}\) ([Handshake 4a](../epilogue/multiscale.md#4a--ddd-strain-rate-to-quasi-static-load-cell-act-iv--hardening)) |
| Single-crystal hardening matches one orientation | Bulk spool test requires texture average and EBSD |
| FE² preview when one RVE is insufficient | FE² at notch Gauss points when scalar \(H\) fails ([Step 4](#step-4--when-offline-calibration-fails-fe-at-the-notch)) |

VII.2 answered *how* lines move, multiply, and export forest statistics on one crystal; this chapter asks *where* those statistics land on **Part IV's mesh** — polycrystal texture, drawing fiber, grain-boundary transmission, and the rate handshake that lets DDD timesteps speak to the load cell's grip speed. When the [preface continuity hinge](../preface.md#continuity-hinges-ascent-descent) lists **taxonomy → dislocation dynamics** at [VII.2's opening hinge](02-dislocation-dynamics.md#ddd-opening-hinge-taxonomy-to-motion), and [VII.2's Bridge](02-dislocation-dynamics.md#bridge-to-vii3) named texture as the missing physics after OpenDiS converges, this section is where single-crystal movies yield to homogenized internal variables for the first time since Part VII began.

The specimen on the bench has not moved since [VII.2's Scene](02-dislocation-dynamics.md#scene-the-forest-grows). The load cell still records the post-yield hardening knee; OpenDiS still archives `ddd_cu_fr300K/` beside `mobility_cu_screw_300K.yaml`. What changes on this page is **organization**: the cold-drawn spool is thousands of crystals, not one RVE — DDD statistics must survive drawing dies, grain boundaries, and strain-rate extrapolation before Part VI's return mapping or Part IV's tensile run can inherit them without refitting \(H\) by hand.
...
If you have read linearly since [Part VI's intermission](../part06-continuum/04-nonlinear-plasticity-preview.md#intermission-ascent-ends-descent-begins) and the [preface descent preview chain](../preface.md#descent-preview-chain), the copper wire has changed state variable three times on the descent without leaving the bench: Burgers vectors and forest density \(\rho\) in [VII.1](../part07-defects/01-defect-taxonomy.md), Peach–Köhler forces and Taylor hardening in [VII.2](../part07-defects/02-dislocation-dynamics.md), and now polycrystal texture and the OpenDiS → DAMASK → FEM export pipeline in this chapter — after [VII.2's opening hinge](02-dislocation-dynamics.md#ddd-opening-hinge-taxonomy-to-motion) made the forest computable and [this chapter's opening hinge](#polycrystal-opening-hinge-ddd-to-homogenization) makes it **exportable** to Part IV's mesh. Part VII is the last stop where **line defects** remain the protagonist — statistics on an elastic medium, mobility laws with cutoffs, hardening curves that Part VI's \(J_2\) fit could only approximate.
