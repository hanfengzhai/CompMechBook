...
**Act V reunion (Handshake 4b pedigree).** {#bridge-act-v-reunion-handshake-4b}

The [prologue row 15 preview](../../prologue/00-many-scales.md#prologue-row-15-preview-handshake-4b) named notch localization before Part I; this chapter's [Step 4 FE² procedure](#step-4--when-offline-calibration-fails-fe-at-the-notch) and [preface row 14](../../preface.md#skill-navigation-row-14) rate extrapolation are the **upstream halves** of the Act V stitch the epilogue reunites in workflow time. Handshake 4a sets bulk \(\tau_{\text{lab}}\); this Step 4 asks whether scalar \(H\) suffices at the notch root; Handshake 4b (epilogue FE² worked example) quantifies the 10–15% uplift when pile-ups matter. When narrative time (Act V — optional stress concentrator) and mathematical order (Part VII before epilogue) diverge on the same afternoon, read the epilogue's [Act V reunion paragraph](../../epilogue/multiscale.md#act-v-reunion-handshake-4b) beside this bridge — it states the notch stitch in one breath; the [Handshake 4b opening paragraph](../../epilogue/multiscale.md#handshake-4b-opening) is the downstream workflow mirror of the [prologue row 15 preview](../../prologue/00-many-scales.md#prologue-row-15-preview-handshake-4b); the [sensitivity derivation worksheet](../../epilogue/multiscale.md#worked-example-sensitivity-ranks) (Handshake 4b paragraph) proves why scalar \(H\) under-predicts root stress by 10–15%. Do not conflate bulk hardening adequacy with notch-root localization — [memory sheet row 15](../../appendix/memory-sheet.md#continuity-hinges-master-map) is the one-page map; [`parse_fe2.sh`](../../scripts/parse_fe2.sh) is the competence artifact that closes the loop from this chapter to the notch-root answer. Upstream rate extrapolation must come from [row 14](../../preface.md#skill-navigation-row-14) first — FE² inherits the same `hardening.yaml` that Handshake 4a calibrated.
...
## Intermission: mesoscale ends, atomistics begin {#intermission-mesoscale-ends-atomistics-begin}

If you have read linearly since [Part VI's intermission](../part06-continuum/04-nonlinear-plasticity-preview.md#intermission-ascent-ends-descent-begins), Part VII was the first descent — forest statistics replacing phenomenological \(H\). This chapter is the **last mesoscale stop** before atoms enter explicitly: it exports hardening laws and mobility yaml the crystal plasticity deck consumes, and names every quantity that still hides in **core structure** DDD cannot resolve.

The three signals that brought you from VI.4 reappear here with mesoscale vocabulary answered and atomistic vocabulary deferred: **history** (OpenDiS link statistics replace fitted \(H\)); **rate** (the [rate handshake](#scale-boundary-handshake-ddd-strain-rate-to-quasi-static-fem) extrapolates DDD to lab grip speed); **notch** ([Step 4 FE²](#step-4--when-offline-calibration-fails-fe-at-the-notch) asks whether scalar hardening suffices at stress concentrators). Part VIII resolves the deferred signal — core width, \(\gamma_{\text{sf}}\), bond breaking at grain boundaries — without asking you to leave the wire on the bench. The specimen does not change; only the state variable does.

When mobility yaml or Peierls stress still feel like magic numbers after this chapter, the plot turns downward again: [Part VIII's second descent rung](../part08-md/00-opening.md#second-descent-rung-vii-midpoint-reunion) is where line cutoffs become vibrating nuclei.
