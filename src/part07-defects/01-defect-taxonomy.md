...
The [preface descent continuity hinges](../preface.md#descent-continuity-hinges) name [VI.4 → VII](../part06-continuum/04-nonlinear-plasticity-preview.md#bridge-to-part-vii) as the **phenomenology → pedigree** turn; this chapter is where that turn becomes a **geometric inventory** before [VII.2](02-dislocation-dynamics.md) makes the forest computable. When the taxonomy table feels like metallurgy notes rather than simulation input, reread the Act IV Lab act above — slip traces on cold-drawn copper are the physical objects the next chapter will integrate.

Turn the page when \(\rho\) is a label on an input deck rather than a count of moving lines — dislocation dynamics is where the forest becomes computable geometry. Read the [VII.2 opening hinge](02-dislocation-dynamics.md#opening-hinge-vii1-to-vii2) when the taxonomy table feels complete but OpenDiS still waits for segment networks rather than defect names.
